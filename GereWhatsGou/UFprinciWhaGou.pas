...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, inifiles,System.Math,Winapi.TlHelp32,Winapi.ShellAPI,
    //############ ATENCAO AQUI ####################
...
type
  TFprinciWhaGou = class(TForm)
    ...
    procedure tmCheckOnlineTimer(Sender: TObject);
    procedure WPPConnect1RetErrorWhiteScreen(Sender: TObject; Response: string);
...
begin
 if not WPPConnect1.Auth(false) then
  Begin
    ...
    WPPConnect1.FormQrCodeType := TFormQrCodeType.Ft_Http;
...
begin
  ...
  try
    ...
    MensagensEnviadas.Lines.Add('Mensagem de teste enviada: '+'6692350049');

    if vpTelefoneTeste<>'' then
    begin
    WPPConnect1.setKeepAlive('true'); //Marca como Online
    WPPConnect1.markIsComposing(vpTelefoneTeste, '1000'); //Digitando 5 Segundos
    WPPConnect1.SendTextMessageEx(vpTelefoneTeste, 'Mensagem de teste', options, '124');
    MensagensEnviadas.Lines.Add('Mensagem de teste enviada: '+vpTelefoneTeste);
    end;
...
begin
  if not WPPConnect1.Auth(False) then
  Begin
    WPPConnect1.FormQrCodeType :=Ft_Desktop;
    WPPConnect1.FormQrCodeStart;
  End
  else
  begin
...
begin
  ...
  if (TWPPConnect(Sender).Status = Inject_Initialized) or
     (TWPPConnect(Sender).Status = Inject_IsReady) or
     (TWPPConnect(Sender).Status = Server_Connected) then
  begin
...
//  Label3.Visible := False;
  ...
  mmErros.Lines.Add(Label3.Caption);
...
begin
   mmErros.Lines.Add( '534 Sessão '+WhatsAppWebVersion.Name+' ' +WhatsAppWebVersion.WAVersion);
end;
  ...
  begin
    ...
    begin
      ...
      begin
       ...
       // WPPConnect1.SendTextMessage(vlTelefone,  cfg.FieldByName('cfgmgoumensa'+lowercase(aTipoNotificacao) + vlIrandomico).AsString, options,vpCznChave+vlTempoDigitacao.ToString {'123'});
...
procedure TFprinciWhaGou.tmCheckOnlineTimer(Sender: TObject);
begin
  tmCheckOnline.Enabled := False;
...
begin
  ...
  begin
    ...
    begin
      DtIni := IncMonth(DtFim, -1); // há 1 mes
    end
    else if RGMensagens.ItemIndex=2 then
    begin
      DtIni := IncMonth(DtFim, -3); // há 3 meses
    end
    else if RGMensagens.ItemIndex=3 then
    begin
      DtIni := IncMonth(DtFim, -6); // há 6 meses
    end;
...
Begin
  ...
  try
    if (WPPConnect1.MyNumber = Null) or (WPPConnect1.MyNumber = '')  then
    Begin
      Exit;
    End
    else
...
procedure TFprinciWhaGou.CriarArquivoBAT_ReiniciaAplicacao;
var
  nomearq: string;
  NomeAplicacao: string;
  arq: TextFile;
