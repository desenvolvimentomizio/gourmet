...
begin
    ...
    begin
      ...
      begin
        ...
        vlDataHoraSaida:=  datetimetostr(now());// consulta.FieldByName('recdthorasaida').AsString;

        vlRecCodigo :='0'; // consulta.Fields[1].AsString;
        vlDiferenca := '00:00:00'; //consulta.Fields[0].AsString;

        vlHoras:=copy(vlDiferenca,1,pos(':',vlDiferenca)-1);

        vlMinutos:=copy(vlDiferenca,pos(':',vlDiferenca)+1,2);
        vlMinutos:=sonumeros(vlMinutos);
        vldias := '0';//sonumeros(consulta.Fields[2].AsString);
