...
procedure doSaveLog(aPath:String; Msg: string);
...
begin
  ...
  try
    ...
    try
      ...
      if fileexists(aPath + 'logs\loggere_' + vlNomeArqlog + '.log') then
      begin
        loLista.LoadFromFile(aPath + 'logs\loggere_' + vlNomeArqlog + '.log');
      end;
    ...
    try
      loLista.SaveToFile(aPath+ 'logs\loggere_' + vlNomeArqlog + '.log');
    except
    end;
    loLista.Free;
  end;
