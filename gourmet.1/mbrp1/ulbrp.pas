...
type
  Tlbrp = class(Tlbrf)
    ...
    procedure frabrpActAtualizarExecute(Sender: TObject);
  private
...
function Pagar(pCargaFrame: TCargaFrame; acesso: Tacesso; pEtdCodigo: String; vrfichave: string): string;
var
  lfram: Tfrabrp;
  vch: string;
begin
  ...
  try
    ...
    begin
      ...
      if vrfichave <> '' then
      begin
        lfram.uqtabela.Filter := 'etdcodigo = ' + pEtdCodigo + ' and rfichave=' + vrfichave;
 ...
 if lfram.uqtabela.RecordCount = 1 then
    begin
      lfram.SelecionarTodosBaixa;
      lfram.calculartotais;
      lfram.somarbaixa;
...
procedure Tlbrp.frabrpActAtualizarExecute(Sender: TObject);
begin
  inherited;
  frabrp.ActAtualizarExecute(Sender);
