...
Function GetDataCountFromIndexSearchDB(nCount : DWORD): DWORD;
...
begin
        if not (@NBioAPI_InitIndexSearchEngine_Func = nil) then
                result := NBioAPI_InitIndexSearchEngine_Func(g_hBSP)
        else
...
begin
        if not (@NBioAPI_GetDataCountFromIndexSearchDB_Func = nil) then
                result := NBioAPI_GetDataCountFromIndexSearchDB_Func(g_hBSP, @nCount)
        else
                result := NBioAPIERROR_INVALID_HANDLE;
end;
