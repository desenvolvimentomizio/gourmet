...
Begin
  ...
  Begin
    ...
    Begin
      ...
      vdataar := copy(vdataar, 1, 2) + '/' + copy(vdataar, 3, 2) + '/' +
        copy(vdataar, 5, 4);
    ...
    Begin
      ...
      vdatabx := copy(vdatabx, 1, 2) + '/' + copy(vdatabx, 3, 2) + '/' +
        copy(vdatabx, 5, 4);
      ...
      Begin
        vobldespcartorio :=
          trim(floattostr(strtofloat(vobldespcartorio) / 100));
      End
      Else
      Begin
        vobldespcartorio := '0';
      ...
      rfi.close;
      rfi.SQL.clear;
      rfi.SQL.add('select * from rfi where bol.bolnossonumero=' + chr(39) +
        vnossonume + chr(39));
      rfi.open;
      ...
      rfi.SQL.clear;
      rfi.SQL.add
        ('SELECT bol.rfichave, bol.bolchave, bol.gebcodigo,  v_rfi.etdcodigo, v_rfi.etdidentificacao, ');
      rfi.SQL.add
        ('bol.bolnossonumero, v_rfi.rfinumero, v_rfi.rfivencimento, v_rfi.rfivalor, bol.sblcodigo, sbl.sblidentificacao,');
      rfi.SQL.add
        ('v_rfi.carcodigo, car.caridentificacao, v_rfi.rfisaldocapital,  v_rfi.rfisaldogeral ');
      rfi.SQL.add('FROM bol, v_rfi, sbl,  car ');
      rfi.SQL.add
        (' WHERE bol.rfichave = v_rfi.rfichave AND bol.sblcodigo = sbl.sblcodigo AND v_rfi.carcodigo = car.carcodigo ');
      rfi.SQL.add
        ('AND v_rfi.srfcodigo = 0 AND bol.sblcodigo <> 9 AND v_rfi.flacodigo = '
      ...
      begin
        vrfichave := '0';
        erros.lines.add('Registro não localizado: ' + vnossonume + ' Arquivo: '
          + narq + ' - ' + voblnumret)
      end;
      ...
      Begin
        ...
        Begin
          ...
          begin
            ...
            lteltetotal.AsCurrency := ltelteprincipal.AsCurrency +
              lteltejuros.AsCurrency - lteltedesconto.AsCurrency;
            ...
            begin
              consulta.close;
              consulta.SQL.Text :=
                'select etdidentificacao from etd where etdcodigo=' +
                rfietdcodigo.AsString;
              consulta.open;
              if not consulta.IsEmpty then
              begin
                ccoccofavorecido.AsString :=
                  consulta.Fieldbyname('etdidentificacao').AsString;
              end
              else
              begin
                ccoccofavorecido.AsString := '';
      ...
      consulta.close;
      consulta.SQL.Text :=
        'select oblchave from obl,bol where obl.bolchave=bol.bolchave and ' +
        'bol.rfichave=' + vrfichave + ' and obldata=' + chr(39) +
        FormatDateTime('yyyy-mm-dd', strtodate(vdatabx)) + chr(39) +
        ' and oblvalor=' + BuscaTroca(vvalorbx, ',', '.') + ' and oblnumret=' +
        chr(39) + voblnumret + chr(39) + ' and oblnossonume=' + chr(39) +
        vnossonume + chr(39);
      consulta.open;
      ...
      begin
        ...
        if oto.Locate('oblchave;tobcodigo',
          VarArrayOf([chaveobl, buscatob(vtipoliq, vbco)]), []) then
        begin
          ...
          Begin
            if omo.Locate('oblchave;mobcodigo',
              VarArrayOf([chaveobl, buscamob(vmodoliq1, vbco, vtipoliq)]), [])
            then
            begin
              pode := false;
              break;
          ...
          Begin
            if omo.Locate('oblchave;mobcodigo',
              VarArrayOf([chaveobl, buscamob(vmodoliq2, vbco, vtipoliq)]), [])
            then
            begin
              pode := false;
              break;
          ...
          Begin
            if omo.Locate('oblchave;mobcodigo',
              VarArrayOf([chaveobl, buscamob(vmodoliq3, vbco, vtipoliq)]), [])
            then
            begin
              pode := false;
              break;
          ...
          Begin
            if omo.Locate('oblchave;mobcodigo',
              VarArrayOf([chaveobl, buscamob(vmodoliq4, vbco, vtipoliq)]), [])
            then
            begin
              pode := false;
              break;
          ...
          Begin
            if omo.Locate('oblchave;mobcodigo',
              VarArrayOf([chaveobl, buscamob(vmodoliq5, vbco, vtipoliq)]), [])
            then
            begin
              pode := false;
              break;
      ...
      begin
        ...
        Try
          ...
          Try
            ...
            oto.post;
          Except
            erros.lines.add('Registro com nosso numero: ' + vnossonume +
              ' não foi localizado! Liq:' + vtipoliq + ' Bco: ' + vbco);
          End;
        ...
        Except
          ...
          erros.lines.add('Registro não localizado: ' + vnossonume +
            ' Arquivo: ' + narq + ' - ' + voblnumret)
  ...
  Begin
    ...
    Begin
      ...
      Begin
        ...
        vdataar := copy(vdataar, 7, 2) + '/' + copy(vdataar, 5, 2) + '/' +
          copy(vdataar, 1, 4);
      ...
      Begin
        ...
        vdataar := copy(vdataar, 1, 2) + '/' + copy(vdataar, 3, 2) + '/20' +
          copy(vdataar, 5, 2);
      End;
    End;
    ...
    Begin
      ...
      rfibol.SQL.clear;
      rfibol.SQL.Text :=
        'select * from rfi,bol WHERE bol.rfichave=rfi.rfichave and  bol.bolnossonumero like ('
      ...
      rfibol.SQL.clear;
      rfibol.SQL.Text :=
        'select * from rfi,bol WHERE bol.rfichave=rfi.rfichave and  '
        + ' (rfi.rfinumero='+QuotedStr(vcNumeroTitulo) +') ';
      rfibol.open;
      ...
      vdatabx := copy(vdatabx, 1, 2) + '/' + copy(vdatabx, 3, 2) + '/20' +
        copy(vdatabx, 5, 2);
        ...
        Begin
          ...
          Begin
            ...
            begin
              ...
              lte.Append;
              lteltedata.AsString := vdatabx;
              ltelteprincipal.AsCurrency := strtofloat(vvalorbx) -
                (strtofloat(voblvalorjuros) + strtofloat(voblmulta));
              lteltejuros.AsCurrency := strtofloat(voblvalorjuros) +
                strtofloat(voblmulta);
              lteltedesconto.AsString := voblvalordesc;
              lteltetotal.AsCurrency := ltelteprincipal.AsCurrency +
                lteltejuros.AsCurrency - lteltedesconto.AsCurrency;
              ltelteextenso.AsString := '';
              ...
              mfitmfcodigo.AsInteger := tmfRecebimento;
              mfimoecodigo.AsInteger := 1;
              mfimfivalor.AsCurrency := strtofloat(vvalorbx) -
                (strtofloat(voblvalorjuros) + strtofloat(voblmulta));
              mfimfidata.AsString := vdatabx;
              mfimfihistorico.AsString := 'Recebimento Valor principal';
              mfimfivalorori.AsCurrency := strtofloat(vvalorbx) -
                (strtofloat(voblvalorjuros) + strtofloat(voblmulta));
              mfimfiparcela.AsInteger := 1; // rfirfiparcela.AsInteger;
              mfiflacodigo.AsInteger := acesso.Filial;
              mfimfisituacao.AsInteger := 0;
              ...
              begin
                ...
                mfitmfcodigo.AsInteger := tmfRecebimentoJuros;
                mfimoecodigo.AsInteger := 1;
                mfimfivalor.AsCurrency := strtofloat(voblvalorjuros) +
                  strtofloat(voblmulta);
                mfimfidata.AsString := vdatabx;
      ...
      begin
        ...
        If (vtipoliq = '06') Or (vtipoliq = '17') Or (vtipoliq = '15') Or
          (vtipoliq = '16') Then
        Begin
      ...
      consulta.SQL.add
        (' INNER JOIN rfi ON rfi.rfichave=bol.rfichave AND  rfi.rfichave = ' +
        vrfichave);
      consulta.SQL.add(' AND rfi.rfivalor = ' + BuscaTroca(vvalorbx, ',', '.') +
        ' LEFT JOIN obl ON  obl.bolchave = bol.bolchave ');
      consulta.SQL.add(' AND obldata = ' +
        QuotedStr(FormatDateTime('yyyy-mm-dd', strtodate(vdatabx))) +
        ' AND oblnumret = ' + QuotedStr(voblnumret) + ' AND oblnossonume = ' +
        QuotedStr(vnossonume));
      ...
      begin
        ...
        if oto.Locate('oblchave;tobcodigo',
          VarArrayOf([bolchave, buscatob(vtipoliq, vbco)]), []) then
        begin
          ...
          Begin
            if omo.Locate('oblchave;mobcodigo',
              VarArrayOf([bolchave, buscamob(vmodoliq1, vbco, vtipoliq)]), [])
            then
            begin
              pode := false;
              break;
          ...
          Begin
            if omo.Locate('oblchave;mobcodigo',
              VarArrayOf([bolchave, buscamob(vmodoliq2, vbco, vtipoliq)]), [])
            then
            begin
              pode := false;
              break;
          ...
          Begin
            if omo.Locate('oblchave;mobcodigo',
              VarArrayOf([bolchave, buscamob(vmodoliq3, vbco, vtipoliq)]), [])
            then
            begin
              pode := false;
              break;
          ...
          Begin
            if omo.Locate('oblchave;mobcodigo',
              VarArrayOf([bolchave, buscamob(vmodoliq4, vbco, vtipoliq)]), [])
            then
            begin
              pode := false;
              break;
          ...
          Begin
            if omo.Locate('oblchave;mobcodigo',
              VarArrayOf([bolchave, buscamob(vmodoliq5, vbco, vtipoliq)]), [])
            then
            begin
              pode := false;
              break;
      ...
      begin
        ...
        Begin
          omo.close;
          omo.FilterSQL := 'oblchave=' + obloblchave.AsString +
            ' and mobcodigo=' + QuotedStr(buscamob(vmodoliq1, vbco, vtipoliq));
          omo.open;
        ...
        Begin
          ...
          omo.close;
          omo.FilterSQL := 'oblchave=' + obloblchave.AsString +
            ' and mobcodigo=' + QuotedStr(buscamob(vmodoliq2, vbco, vtipoliq));
          omo.open;
        ...
        Begin
          omo.close;
          omo.FilterSQL := 'oblchave=' + obloblchave.AsString +
            ' and mobcodigo=' + QuotedStr(buscamob(vmodoliq3, vbco, vtipoliq));
          omo.open;
        ...
        Begin
          omo.close;
          omo.FilterSQL := 'oblchave=' + obloblchave.AsString +
            ' and mobcodigo=' + QuotedStr(buscamob(vmodoliq4, vbco, vtipoliq));
          omo.open;
        ...
        Begin
          omo.close;
          omo.FilterSQL := 'oblchave=' + obloblchave.AsString +
            ' and mobcodigo=' + QuotedStr(buscamob(vmodoliq5, vbco, vtipoliq));
          omo.open;
...
Begin
  ...
  Begin
    ...
    Begin
      consulta.close;
      consulta.SQL.Text := 'select mobcodigo from mob where mobreferencia=' +
        chr(39) + cod + chr(39) + ' and bcocodigo=' + chr(39) + vbcocodigo
        + chr(39);
      // chr(39) + ' and mobtiporetorno=' + chr(39) + 'A' + chr(39);
      consulta.open;
    End;
    If (vtip = '06') Or (vtip = '09') Or (vtip = '17') Then
    Begin
      consulta.close;
      consulta.SQL.Text := 'select mobcodigo from mob where mobreferencia=' +
        chr(39) + cod + chr(39) + ' and bcocodigo=' + chr(39) + vbcocodigo
...
Begin
  ...
  consulta.close;
  consulta.SQL.Text := 'select tobcodigo from tob where tobreferencia=' +
    chr(39) + cod + chr(39) + ' and bcocodigo=' + chr(39) + vbcocodigo
    + chr(39);
  consulta.open;
