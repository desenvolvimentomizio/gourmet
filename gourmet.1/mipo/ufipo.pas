...
type
  ...
  public
    ...
    vpNotaObrigatoria:Boolean;
    vpValorNota:Currency;
...
function imprimeorc(AOwner: TComponent; conexao: tuniconnection; vchave: string; vTrmCodigo: string; vusrcodigo: string; vlImpAtivos: string; vValorNota: Currency): string;
var
  vch: string;
  hWindow: HWND;
begin
  try
    ...
    if copy(vlImpAtivos,6,1)='1' then
      fipo.vpNotaObrigatoria:=True
    else
      fipo.vpNotaObrigatoria:=False;


    fipo.vpImpAtivos := vlImpAtivos;

    fipo.vorcchave := vchave;
    fipo.vpValorNota := vValorNota;
...
begin
  ...
  if vpNotaObrigatoria then
  begin
    if (not sbNFE.Down)  and (not sbNFCe.Down) and (not sbNFCeI.Down) then
    begin
      ShowMessage('ATENÇÃO !!'+#13+'É necessário definir o tipo de Nota Fiscal!');
      exit;
...
begin
  ...
  begin
    ...
    sbNFCei.Enabled := Plnfce.Visible;
    sbNFe.Enabled := plnota.Visible;
  end;
