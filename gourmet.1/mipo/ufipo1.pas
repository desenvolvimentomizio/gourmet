...
begin
  ...
  begin
    ...
    if ((FindWindow(PChar('Tfprincirest'), nil)) <> 0) or ((FindWindow(PChar('TFprinciVar'), nil)) <> 0) or ((FindWindow(PChar('TFprinciCax'), nil)) <> 0) then
    begin
...
begin
  ...
  consulta.SQL.Add('AND ito.stocodigo IN (1, 2, 3,5) ');
  ...
  if (vlTotalOrcamento = vlTotalServico) and (vlTotalOrcamento = 0) then
  begin
    sbNFCe.Enabled := False;
    sbNFCeI.Enabled := False;
  end;
