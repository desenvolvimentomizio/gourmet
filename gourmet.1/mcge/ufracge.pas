...
type
  Tfracge = class(Tfrabase)
    ...
    ccgcedcodigo: TIntegerField;
    ActApuracaoCX: TAction;
    mda: TUniQuery;
    mdamodalidade_id: TIntegerField;
    mdamodalidade_nome: TStringField;
    mdamodalidade_valor: TFloatField;
    cfgctb: TUniQuery;
    cfgcfgcodigo: TIntegerField;
    cre: TUniQuery;
    cretotal: TCurrencyField;
    qdias: TUniQuery;
    rfi: TUniQuery;
    rfimodalidade_valor: TCurrencyField;
    ...
    procedure ActApuracaoCXExecute(Sender: TObject);
  private
...
procedure Tfracge.ActApuracaoCXExecute(Sender: TObject);
var
  vPrimeiroDia: string;
  vUltimoDia: string;
...
begin
  ...
  cfgctb.Close;
  cfgctb.ParamByName('cfgcodigo').AsInteger := cfgcfgcodigo.AsInteger;
  cfgctb.Open;
  ...
  begin
    ...
    begin
      try
        ...
        consulta.Close;
        consulta.SQL.Text := 'select  cgedatafinal ultimodia  , cgedatainicial  primeirodia from cge where cgechave=' + uqtabelacgechave.AsString;
        consulta.Open;

        if strtodatetime(vldia) < strtodatetime(consulta.FieldByName('primeirodia').AsString) then
        begin
        ...
        qdias.Close;
        qdias.SQL.Text := 'select  year(' + QuotedStr(FormatDateTime('yyyy-m-d', strtodate(vldia))) + ' ), LAST_DAY(' + QuotedStr(FormatDateTime('yyyy-m-d', strtodate(vldia))) +
          ') ultimodia ' + ' , CAST(DATE_FORMAT(' + QuotedStr(FormatDateTime('yyyy-m-d', strtodate(vldia))) + ' ,' + QuotedStr('%Y-%m-01') + ') as DATE) primeirodia ';
        qdias.Open;

        if consulta.FieldByName('ultimodia').AsDateTime < qdias.FieldByName('ultimodia').AsDateTime then
        begin
          vUltimoDia := consulta.FieldByName('ultimodia').AsString;
        end
        else
        begin
          vUltimoDia := qdias.FieldByName('ultimodia').AsString;
        end;
        ...
        consulta.SQL.Text :=
          ...
          + QuotedStr('cco') + ' and mcgdata BETWEEN :dtinicial AND :dtfinal and ccgcodigo=:ccgcodigo GROUP BY ccgcodigo';

        // ' and mcgcompetencia=:mcgcompetencia and ccgcodigo=:ccgcodigo GROUP BY ccgcodigo';

        // consulta.ParamByName('mcgcompetencia').AsString := formatfloat('00', vlMeses) + '-' + copy(uqtabelacgeano.AsString, 3, 2);

        consulta.ParamByName('dtinicial').asdate := vPrimeiroDia.ToDouble;
        consulta.ParamByName('dtfinal').asdate := vUltimoDia.ToDouble;
    ...
    // apuração recebimento das vendas
    ...
    begin
     ...
     if vPrimeiroDiaHora='' then
     begin
     consulta.Close;
     consulta.SQL.Text:='select ccxinicio from ccx order by ccxinicio limit 1';
     consulta.Open;

       vPrimeiroDiaHora:=consulta.FieldByName('ccxinicio').AsString;
      ...
      begin
        ...
        { if mdamodalidade_id.AsInteger = mdaCartaoDebito then
          begin
          cgedcg.FieldByName('ccgcodigo').AsInteger := cfgctb.FieldByName('cfgctbpcgcartaod').AsInteger;
          end;

          if mdamodalidade_id.AsInteger = mdaCartaoCredito then
          begin
          cgedcg.FieldByName('ccgcodigo').AsInteger := cfgctb.FieldByName('cfgctbpcgcartao').AsInteger;
          end;

          if mdamodalidade_id.AsInteger = mdaAFaturar then
          begin
          cgedcg.FieldByName('ccgcodigo').AsInteger := cfgctb.FieldByName('cfgctbpcgafaturar').AsInteger;
          end;

          if mdamodalidade_id.AsInteger = mdaDoacao then
          begin
          cgedcg.FieldByName('ccgcodigo').AsInteger := cfgctb.FieldByName('cfgctbpcgdoacao').AsInteger;
          end;

          if mdamodalidade_id.AsInteger = mdaChequeTerceiros then
          begin
          cgedcg.FieldByName('ccgcodigo').AsInteger := cfgctb.FieldByName('cfgctbpcgcheque').AsInteger;
          end; }

        if (ccgccgcodigo.AsInteger = cfgctb.FieldByName('cfgctbpcgdinheiro').AsInteger) then
        begin

          mda.Close;
          mda.ParamByName('inicial').asdate := strtodate(vPrimeiroDia); // uqtabelacgedatainicial.AsFloat;
          mda.ParamByName('final').asdate := strtodate(vUltimoDia); // uqtabelacgedatafinal.AsFloat;
          mda.Filter := 'modalidade_id=' + mdadinheiro.ToString;
          mda.Filtered := true;
          mda.Open;

          // fim do movimenta de mes pelo periodo de caixa
          if not mda.IsEmpty then
          begin
            vlValorRealizado := mdamodalidade_valor.AsCurrency;
          end
          else
            vlValorRealizado := 0;
          ...
          begin
            ...
            cgedcg.Append;
            cgedcg.FieldByName('cgechave').AsInteger := uqtabelacgechave.AsInteger;
            cgedcg.FieldByName('ccgcodigo').AsInteger := cfgctb.FieldByName('cfgctbpcgdinheiro').AsInteger;
            cgedcg.FieldByName('dcgprevisao').AsFloat := vlValorPrevisto;
            cgedcg.FieldByName('dcgmes').AsInteger := vlMeses;
            cgedcg.FieldByName('dcgano').AsInteger := uqtabelacgeano.AsInteger;
            cgedcg.FieldByName('dcgrealizado').AsFloat := vlValorRealizado;
        ...
        if (ccgccgcodigo.AsInteger = cfgctb.FieldByName('cfgctbpcgcartaod').AsInteger) then
        begin

          rfi.Close;
          rfi.ParamByName('inicial').asdate := strtodate(vPrimeiroDia);;
          rfi.ParamByName('final').asdate := strtodate(vUltimoDia);
          rfi.ParamByName('mdacodigo').AsInteger := mdaCartaoDebito;
          rfi.Open;

          // fim do movimenta de mes pelo periodo de caixa
          if not mda.IsEmpty then
          begin
            vlValorRealizado := rfimodalidade_valor.AsCurrency;
          end
          else
            vlValorRealizado := 0;
        ...
        if (ccgccgcodigo.AsInteger = cfgctb.FieldByName('cfgctbpcgcartao').AsInteger) then
        begin

          rfi.Close;
          rfi.ParamByName('inicial').asdate := strtodate(vPrimeiroDia);;
          rfi.ParamByName('final').asdate := strtodate(vUltimoDia);
          rfi.ParamByName('mdacodigo').AsInteger := mdaCartaoCredito;
          rfi.Open;
        ...
        if (ccgccgcodigo.AsInteger = cfgctb.FieldByName('cfgctbpcgconvenio').AsInteger) then
        begin

          cre.Close;
          cre.ParamByName('dtinicial').asdate := strtodate(vPrimeiroDia); // uqtabelacgedatainicial.AsFloat;;
          cre.ParamByName('dtfinal').asdate := strtodate(vUltimoDia); // uqtabelacgedatafinal.AsFloat;
          cre.Open;

          // fim do movimenta de mes pelo periodo de caixa
          if not cre.IsEmpty then
          begin
            vlValorRealizado := cretotal.AsCurrency;
          end
          else
            vlValorRealizado := 0;
          ...
          begin
            ...
            cgedcg.Append;
            cgedcg.FieldByName('cgechave').AsInteger := uqtabelacgechave.AsInteger;
            cgedcg.FieldByName('ccgcodigo').AsInteger := cfgctb.FieldByName('cfgctbpcgconvenio').AsInteger;
            cgedcg.FieldByName('dcgprevisao').AsFloat := vlValorPrevisto;
            cgedcg.FieldByName('dcgmes').AsInteger := vlMeses;
            cgedcg.FieldByName('dcgano').AsInteger := uqtabelacgeano.AsInteger;
            cgedcg.FieldByName('dcgrealizado').AsFloat := vlValorRealizado;
        ...
        { if (ccgccgcodigo.AsInteger = cfgctb.FieldByName('cfgctbpcgdoacao').AsInteger) then
          begin


          mda.Close;
          mda.ParamByName('inicial').AsDate := uqtabelacgedatainicial.AsFloat;
          mda.ParamByName('final').AsDate := uqtabelacgedatafinal.AsFloat;
          mda.Filter := 'modalidade_id=' + mdadoacao.tostring;
          mda.Filtered := true;
          mda.Open;
        ...
        { if (ccgccgcodigo.AsInteger = cfgctb.FieldByName('cfgctbpcgafaturar').AsInteger) then
          begin
          ...
          mda.ParamByName('inicial').AsDate := uqtabelacgedatainicial.AsFloat;
          mda.ParamByName('final').AsDate := uqtabelacgedatafinal.AsFloat;
          mda.Filter := 'modalidade_id=' + mdaafaturar.tostring;
          mda.Filtered := true;
          mda.Open;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        { if ccg.Locate('ccgcodigo',acgccgcodigo.AsInteger,[]) then
          begin

          acg.Edit;
          acg.FieldByName('cedcodigo').AsInteger := ccgcedcodigo.AsInteger;
          acg.Post;
          end; }
