unit ufratcc;

interface

uses
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes, Vcl.Graphics,
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, ufrabase, Data.DB, System.ImageList,
  Vcl.ImgList, Vcl.ExtCtrls, VirtualTable, MemDS, DBAccess, Uni, Vcl.Menus,
  System.Actions, Vcl.ActnList, Vcl.StdCtrls, Vcl.Buttons, Vcl.Grids,
  Vcl.DBGrids, Vcl.Imaging.jpeg, Vcl.Mask, Vcl.DBCtrls, Vcl.Imaging.pngimage,uPegaBase;

type
  Tfratcc = class(Tfrabase)
    uqtabelatcccodigo: TIntegerField;
    uqtabelatccidentificacao: TStringField;
    procedure ActAlterarExecute(Sender: TObject);
    procedure ActIncluirExecute(Sender: TObject);
  private
    { Private declarations }
  public
    { Public declarations }
  end;

var
  fratcc: Tfratcc;

  // Início ID do Módulo fratcc
const
  vPlIdMd = '00.00.00.000-01';
  vPlTitMdl = 'Tipos de Contratos';

  // Fim ID do Módulo fratcc



implementation

uses
  uftcc;

{$R *.dfm}


type
  TCellGrid = class(TCustomGrid);

function formuFrame(pCargaFrame: TCargaFrame): string;
begin
  pCargaFrame.IdModulo := vPlIdMd;
  pCargaFrame.Titulo := vPlTitMdl;
  fratcc := Tfratcc.Create(pCargaFrame);
end;

procedure defineacesso(pCargaFrame: TCargaFrame);
begin
  pCargaFrame.Titulo := vPlTitMdl;
  fratcc := Tfratcc.Create(pCargaFrame);
  try
    fratcc.CriaAcoesDeAcesso;
  finally
    fratcc.Free;
  end;
end;

exports defineacesso, formuFrame;



procedure Tfratcc.ActAlterarExecute(Sender: TObject);
begin
  inherited;
 CriaFormulario(Tftcc, uqtabelatcccodigo.AsString , '');
end;

procedure Tfratcc.ActIncluirExecute(Sender: TObject);
begin
  inherited;
 CriaFormulario(Tftcc, '' , '');
end;

end.
