...
begin
  ...
  // except
  // zcone.Rollback;
  // ShowMessage('FALHA: Importações não realizadas!');
...
begin
  ...
  // except
  // zcone.Rollback;
  // ShowMessage('FALHA: Exportações não realizadas!');
...
//    self.relweb.Params[0].AsString := vlRadCNPJ;
    ...
    SBIncluirUm.Enabled := false;
    SBIncluirTodos.Enabled := false;
    SBExcluirUn.Enabled := false;
    SBExcluirTodos.Enabled := false;
    self.bReconectar.Visible := true;
...
begin
 ...
 { if not vCustomi then
  ...
  begin}
    vpRelCodigoInt := strtoint(copy(self.rlrelcodigo.AsString, 1, 3));
  ...
  if vlNovoCodigo then
    relw.Fieldbyname('relcodigo').AsString := formatfloat('000', vpRelCodigoInt) + vlRadCNPJ
  else
