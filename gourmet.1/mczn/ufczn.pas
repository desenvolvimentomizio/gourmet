...
begin
  ...
  begin
    // ajustar modalidades para vendas do ripo refeicao

    mesvenda.Close;
    mesvenda.Close;
    mesvenda.ParamByName('inicio').Asfloat := cznabertura.Field.AsDateTime;
    mesvenda.ParamByName('final').Asfloat := cznfechamento.Field.AsDateTime;
    mesvenda.Open;



    while not mesvenda.Eof do
    begin

      mesrefeicao.Close;
      mesrefeicao.ParamByName('meschave').AsInteger:=mesvenda.FieldByName('meschave').AsInteger+1;
      mesrefeicao.ParamByName('mesvalor').AsCurrency:=mesvenda.FieldByName('mesvalor').AsCurrency;
      mesrefeicao.Open;

      if not mesrefeicao.IsEmpty then
      begin
        dtlvenda.Close;
        dtlvenda.ParamByName('meschave').AsInteger:=mesvenda.FieldByName('meschave').AsInteger;
        dtlvenda.Open;

        if not dtlvenda.IsEmpty then
        begin
          mesrefeicao.Edit;
          mesrefeicao.FieldByName('mdacodigo').AsString:=dtlvenda.FieldByName('mdacodigo').AsString;
          mesrefeicao.Post;
        end;
      end;
      mesvenda.Next;
    ...
    // registra o itmchave da venda no imp para cada colaborador

    imp.Close;
    imp.ParamByName('inicio').Asfloat := cznabertura.Field.AsDateTime;
    imp.ParamByName('final').Asfloat := cznfechamento.Field.AsDateTime;
    imp.Open;

    pbProcesamento.Visible := true;
    pbProcesamento.Max := imp.RecordCount;
    ...
    begin
      ...
      consulta.Close;
      consulta.SQL.Text := 'select itmchave from itm where itochave=' + impitochave.AsString;
      consulta.Open;

      if not consulta.IsEmpty then
      begin
        imp.Edit;
        impitmchave.AsInteger := consulta.FieldByName('itmchave').AsInteger;
        imp.post;
      end;
    ...
    consulta.Close;
    consulta.SQL.Text := 'delete from olt where orcchave in (select orcchave from orc where orcdataencerr<' +
      QuotedStr(ajustadata(datetostr(IncDay(now() - 7)))) + ')';
    consulta.ExecSQL;

    consulta.Close;
    consulta.SQL.Text := 'delete from orc where stocodigo=3 and orcdataencerr<' +
      QuotedStr(ajustadata(datetostr(IncDay(cznfechamento.Field.AsDateTime - 7))));
    consulta.ExecSQL;

    consulta.Close;
    consulta.SQL.Text := 'delete from orc where stocodigo=1';
    consulta.ExecSQL;

    consulta.Close;
    consulta.SQL.Text := 'delete from orc where orctotalav=0';
    consulta.ExecSQL;

    consulta.Close;
    consulta.SQL.Text := 'delete from orc where stocodigo=88 and orcdataencerr is not null and orcdataencerr<' +
      QuotedStr(ajustadata(datetostr(IncDay(cznfechamento.Field.AsDateTime - 7))));
    consulta.ExecSQL;

    consulta.Close;
    consulta.SQL.Text := 'delete from unm where orcchave not in (select orcchave from orc)';
    consulta.ExecSQL;
    ...
    consulta.Close;
    consulta.SQL.Text := 'delete from olt where orcchave not in (select orcchave from orc)';
    consulta.ExecSQL;

    consulta.Close;
    consulta.SQL.Text := 'delete from imm where itochave not in (select itochave from ito, orc where ito.orcchave=orc.orcchave)';
    consulta.ExecSQL;

    consulta.Close;
    consulta.SQL.Text := 'SELECT cfgmgoupedidelivery,cfgmgounumepedint, cfgmgoupedideliveryinicial FROM cfgmgou ';
    consulta.Open;

    consulta.Edit;
    consulta.FieldByName('cfgmgoupedidelivery').AsInteger := consulta.FieldByName('cfgmgoupedideliveryinicial').AsInteger;
    consulta.post;
    ...
    consulta.Close;
    consulta.SQL.Text := 'delete from rpw';
    consulta.ExecSQL;


    consulta.Close;
    consulta.SQL.Text := 'update gri set grisituacao=24';
    consulta.ExecSQL;
    ...
    if consulta.FieldByName('cfgmgouctadelivery').AsString <> '' then
    begin
      vlctadelivery := consulta.FieldByName('cfgmgouctadelivery').AsString;
    end;

    if vlctadelivery <> '' then
    begin
      ...
      if consulta.FieldByName('ccxchave').AsString <> '' then
      begin
        vlccxchave := consulta.FieldByName('ccxchave').AsString;

        consulta.Close;
        consulta.SQL.Text := 'update ccx set ccxdatafecha=now() ,ccxhorafecha=now(), ccxfinal=now() where  ccxchave=' + vlccxchave +
          '  and ctacodigo=' + vlctadelivery + ' and  ccxfinal is null ';
        consulta.ExecSQL;
  ...
  consulta.Close;
  consulta.SQL.Text := 'UPDATE  imp,itm,gri SET imp.itmchave=itm.itmchave where imp.itochave = itm.itochave AND imp.sepcodigo = gri.sepcodigo ';
  consulta.SQL.Add('and imp.itmchave<>itm.itmchave  and gricontrolaproducao=1');
  consulta.ExecSQL;

  consulta.Close;
  consulta.SQL.Text := 'UPDATE mes SET mesregistro=mesemissao   WHERE tdfcodigo= '+QuotedStr('65')+'  AND mesregistro='+QuotedStr('0000-00-00');
  consulta.ExecSQL;
  consulta.Close;
  consulta.SQL.Text := 'UPDATE mes SET mesdatanfe=mesemissao   WHERE tdfcodigo= '+QuotedStr('65')+' AND mesdatanfe='+QuotedStr('0000-00-00');
  consulta.ExecSQL;
    ...
    begin
...

     {
      ...
      consulta.SQL.Text := 'UPDATE pro SET prosaldo = ' + buscatroca(floattostr(vlSaldo), ',', '.') + ', prosaldodisp =  ' +
        buscatroca(floattostr(vlSaldoDisp), ',', '.') + ' where procodigo=' + proprocodigo.AsString;
      consulta.ExecSQL;
