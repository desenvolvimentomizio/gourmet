...
type
  ...
  private
    ...
    function ReGeraXMLNFCe(vMesChave: string; vFlaCodigo: string = '1'): Boolean;
...
begin
  try
    ...
    *)
    ...
    if vFuncao = 'ReGerarXML' then
    begin
      fnfce.consulta.Close;
      fnfce.consulta.SQL.Text := 'select mesinclusao from mes where meschave=' + vMesChave;
      fnfce.consulta.Open;

      vDia := trim(copy(fnfce.consulta.FieldByName('mesinclusao').AsString, 1, 10));
      vHora := trim(copy(fnfce.consulta.FieldByName('mesinclusao').AsString, 11, 10));
    end
    else
    begin
    ...
    case ansiIndexStr(vFuncao, ['EmiteNFCe', 'ImprimeNFCe', 'CancelaNFCe', 'ConsultaServicoSEFAZNFCE', 'VerificaExistencia', 'AjustaSituacaoNFCe',
      ...
      9:
        Result := fnfce.ReGeraXMLNFCe(vMesChave, fnfce.vpFlacodigo);
      10:
...
Begin
  ...
  try
    ...
    if (pos('arqnfces', vArqNFCe) > 0) and (pos('conarqnfces', vArqNFCe) = 0) then
    begin
      vlNomeArquivo := StringReplace(vArqNFCe, 'arqnfces', 'conarqnfces', []);
    end;
    ...
    begin
      ...
      copyfile(pchar(vArqNFCe), pchar(vlNomeArquivo), False);
   ...
   { If not FileExists(vArqNFCe) Then
    Begin
      if (cfgcfgservarqnfes.AsString <> '127.0.0.1') and (pos('\', cfgcfgservarqnfes.AsString) = 0) then
      begin
        vArqNFCe := BaixaXMLServidor(cfgcfgservarqnfes.AsString, vArqNFCe);
      end;
...
{  If not FileExists(vpNomeArquivoNFCe) Then
  ...
  Begin
    ...
    qDtl.Close;
    qDtl.SQL.Clear;
    qDtl.SQL.add('SELECT lte.ltetroco, lte.ltevalortaxa ');
...
begin
  ...
  Begin
    qDtl.Close;
    qDtl.SQL.Clear;
    qDtl.SQL.add('SELECT lte.ltetroco, ltevalortaxa ');
    qDtl.SQL.add('  FROM rfm ');
    qDtl.SQL.add(' INNER JOIN mfi ON rfm.rfichave = mfi.rfichave ');
    qDtl.SQL.add(' INNER JOIN mlt ON mlt.mfichave = mfi.mfichave ');
    qDtl.SQL.add(' INNER JOIN lte ON mlt.ltechave = lte.ltechave ');
      ...
      *)
      ...
      begin
        ...
        begin
          ...
          {  if ValidaConsumidor(vMesChave, vFlaCodigo) then}
    ...
    //  Total.ICMSTot.vFrete := vlTotalFrete;
     ...
     //   vlTotalItens := RoundTo(vlTotBruto - vlTotalOutras, -2);
      ...
      begin
        ...
        begin
          vlTotalitensProdutos:=vlTotalitensProdutos+itmitmtotal.AsCurrency;
        end;
        itm.Next;
     ...
     vlPercentualProdutos := (mesmesprodutos.AsCurrency + mesmesfrete.AsCurrency) / mesmestotal.AsCurrency;
      ...
      begin
        ...
        begin
              ...
              begin
               ...
               // vPag := rdc.FieldByName('rdcvalor').AsCurrency;
                ...
                if (rdc.FieldByName('adccodigo').AsString<>'') and
                   ...
                   (pos('{',rdc.FieldByName('rdcnrauto').AsString)=0)  then
                begin
          ...
          begin
            ...
            if (vVoucher > 0) and (rdc.RecordCount>0)  then
            begin
              with pag.add do
              begin
...
Begin
  ...
  Begin
    ...
    begin
      ...
      consulta.Close;
      consulta.SQL.Text := 'select unisimbolo, proqtdtrib  from pro,uni where pro.prounitrib=uni.unicodigo  and  procodigo=' + itmprocodigo.AsString;
      consulta.Open;
      itm.Edit;
      itmpunqtdtribtotal.AsFloat := consulta.FieldByName('proqtdtrib').AsFloat;
      itmunisimbolotrib.AsString := consulta.FieldByName('unisimbolo').AsString;
      vlQtdTributaria := consulta.FieldByName('proqtdtrib').AsFloat;
      itm.post;
        ...
        Begin
          ...
          begin
            ...
            begin
              ...
              if itmproanpcodigo.AsString = '210203001' then
              begin
                pGLP := 100;
                descANP := 'GLP';
                ...
                descANP := 'OUTROS ÓLEOS LUBRIFICANTES AUTOMOTIVOS';

              { origCombl := TorigCombCollectionItem.Create;

                origCombl.indImport := iiNacional;
                origCombl.cUFOrig := 51;
                origCombl.pOrig := 100.0000;

                origComb.add(origCombl);
              }
            end;
         ...
         // vlTotalFrete := vlTotalFrete + self.itmitmfrete.AsCurrency;
          ...
          Begin
            ...
            begin
              ...
              if vlTemANP then
              begin
                if (itmproanpcodigo.AsString <> '') AND (itmproanpcodigo.AsString = '210203001') then
                begin

                  itm.Edit;
                  itmcstcodigo.AsString := '061';
                  itm.post;
                end;
            ...
            Begin
              ...
              // qBCMonoRet := (vlQtdTributaria * itmitmquantidade.AsFloat); // base do calculo
              adRemICMSRet := 1.2571;
              vICMSMonoRet := 0; // qBCMonoRet * adRemICMSRet;
...
if mesmesretirabalcao.AsInteger=0 then
      ...
      *)
      ...
      if pos(trim(vlMensagemCargaTrib), InfAdic.infCpl) = 0 then
        InfAdic.infCpl := InfAdic.infCpl + trim(vlMensagemCargaTrib) + #13;

      if pos(trim(cfgcfgmensagempdv.AsString), InfAdic.infCpl) = 0 then
        InfAdic.infCpl := InfAdic.infCpl + ' ' + cfgcfgmensagempdv.AsString;

      if pos(trim(vlMensagemLimite), InfAdic.infCpl) = 0 then
        InfAdic.infCpl := InfAdic.infCpl + #13 + #13 + vlMensagemLimite;

      if pos(trim(mesmesobs.AsString), InfAdic.infCpl) = 0 then
        InfAdic.infCpl := InfAdic.infCpl + #13 + mesmesobs.AsString;

      if pos(trim(vlMensagemReducaoBase), InfAdic.infCpl) = 0 then
        InfAdic.infCpl := InfAdic.infCpl + #13 + vlMensagemReducaoBase;
      ...
      *)
      ...
      Total.ICMSTot.vOutro := vlTotalOutras;
   //   Total.ICMSTot.vOutro := vlTotalFrete;

      Total.ICMSTot.vICMS := RoundTo(vlTotICMS, -2);
      Total.ICMSTot.vProd := RoundTo(vlTotBruto, -2);
      Total.ICMSTot.vDesc := RoundTo(vlTotDesc, -2);
      Total.ICMSTot.vNF := RoundTo((vlTotBruto - vlTotDesc + vlTotalOutras+vltotalfrete ), -2);
  ...
  Except
    ...
    Begin
        ...
        if cfgcfgmodonfe.AsInteger = 2 then
        begin
          ShowMessage('Linha 5833 '+E.Message);
        end;
      ...
      begin
        ...
        ShowMessage('A T E N Ç Ã O:' + #13 + #13 + 'O produto :' + #13 + #13 + ACBrNFeNFCe.NotasFiscais.Items[0].NFe.Det.Items[StrToInt(vlItem) - 1]
...
begin
  ...
  Except
    ...
    Begin
     ...
     ShowMessage('Linha 6091 '+E.Message);

      if (pos(lowercase('CPF do destinatario invalido'), lowercase(E.Message)) > 0)
      or (pos(lowercase('CNPJ do destinatario invalido'), lowercase(E.Message)) > 0)  then
      begin
         etddoc.close;
       ...
       {  etddoc.Edit;
         ...
         ShowMessage('A T E N Ç Ã O:' + #13 + #13 +uppercase('Gerador novo CPF.') + #13 + 'Envie novamente a NFCe !');}
...
Begin
  try
    ...
    begin
      ...
      if IncMinute(recrecdthoraentrada.AsDateTime,2)>now() then
      begin
    ...
    begin
      ...
      try
        faguarde:=tfaguarde.Create(self);
        faguarde.Show;
        faguarde.Label1.Caption:='Conectando SEFAZ - Aguarde ... ';
        application.ProcessMessages;
...
Begin
  try
    ...
    if (pos('arqnfces', vpNomeArquivoNFCe) > 0) and (pos('conarqnfces', vpNomeArquivoNFCe) = 0) then
    begin
      vlNomeArquivo := StringReplace(vpNomeArquivoNFCe, 'arqnfces', 'conarqnfces', []);
    end;
...
function Tfnfce.ReGeraXMLNFCe(vMesChave: string; vFlaCodigo: string = '1'): Boolean;
Var
  vErro: String;
  vMensagemErro: String;
...
Begin
  ...
  Begin
    ...
    *)
    ...
    begin
      ide.dEmi := strtodatetime(datetimetostr(mesmesdatanfe.AsDateTime) + ' ' + mesmeshoranfe.AsString);
      ide.hSaiEnt := mesmeshoranfe.AsDateTime;
    end
    else
    begin
      ide.dEmi := strtodatetime(datetimetostr(mesmesregistro.AsDateTime) + ' ' + trim(copy(mesmesinclusao.AsString, 11, 10)));
      ide.hSaiEnt := mesmeshoranfe.AsDateTime;
      vpDataAtual := mesmesregistro.AsDateTime;
    ...
    { end
      ...
      ide.tpEmis := teOffLine;
      ide.dhCont := mesmesemissao.AsDateTime;
      ide.xJust := 'Falha de comunicação com servidores da SEFAZ.';
    ...
    *)
    ...
    Total.ICMSTot.vOutro := vlTotatOutras;
 //   Total.ICMSTot.vFrete := vlTotatFrete ;
    ...
    qDtl.SQL.add('INNER JOIN dtl ON mlt.ltechave = dtl.ltechave ');
    qDtl.SQL.add('WHERE rfm.meschave=' + Self.mesmeschave.AsString + ' ');
    qDtl.SQL.add(' AND if(mdacodigo=4 or mdacodigo=5,tmfcodigo in (2,21),  IF(mdacodigo=7,tmfcodigo=2,tmfcodigo=21)) ');
    qDtl.SQL.add('GROUP BY dtl.mdacodigo, dtl.rdcnrauto ');
    ...
    begin
      ...
      begin
        ...
        if mesmesregistro.AsString = '' then
        begin
          vpNomeArquivoNFCe := vpPastaPrincipal + vpSubPastaDoc + '\' + formatdatetime('yyyymm', mesmesemissao.AsDateTime) + '\' + vpNomeArquivoNFCe +
            '-nfe.xml';
        ...
        begin
          ...
          vpNomeArquivoNFCe := vpPastaPrincipal + vpSubPastaDoc + '\' + formatdatetime('yyyymm', mesmesregistro.AsDateTime) + '\' + vpNomeArquivoNFCe
            + '-nfe.xml';
        end;
...
procedure Tfnfce.AtivaMizioArquivos;
var
  vlHamdle: thandle;
  vlarq: string;
begin

  {
    ...
    begin
    ShellExecute(Handle, 'open', pchar(vlarq), '', '', SW_SHOWNORMAL);
    end;
