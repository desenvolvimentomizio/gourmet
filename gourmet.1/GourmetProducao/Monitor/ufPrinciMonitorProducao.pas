...
type
  ...
  private
    vpMostrar : Boolean;
    vpContadorProducao:Integer;
    vpContadorServidorArquivos:Integer;
    vpContadorAiqFome:Integer;
    vpContadorMeNuvem:Integer;
    vpContadorWhatsApp:Integer;
...
begin
  try
    try
      ...
      LResponse := TRequest.New.BaseURL('http://'+vpServidorArquivos+':'+aPorta+'/test')
      ...
      begin
        ...
        if lowercase(aExecutavel)=lowercase('ProducaoUPServer.exe') then
          vpContadorProducao:=0
        else if lowercase(aExecutavel)=lowercase('GourmetAIQ') then
          vpContadorAiqFome:=0
        else if lowercase(aExecutavel)=lowercase('GourmetMen.exe') then
          vpContadorMeNuvem:=0
        else if lowercase(aExecutavel)=lowercase('GereWhatsGou.exe') then
          vpContadorWhatsApp:=0
        else if lowercase(aExecutavel)=lowercase('MizioServidorArquivo.exe') then
          vpContadorServidorArquivos:=0;
      ...
      begin
        ...
        begin
          ...
          Memo1.lines.Add(DateTimetoStr(now())+ ' Tentativa de conecao '+ LResponse.Content+#13+
                                                ' http://'+vpServidorArquivos+':'+aPorta+'/test');
          aPainel.Color:=clred;

          if lowercase(aExecutavel)=lowercase('ProducaoUPServer.exe') then
            vpContadorProducao:=vpContadorProducao+1
          else if lowercase(aExecutavel)=lowercase('GourmetAIQ') then
            vpContadorAiqFome:=vpContadorAiqFome+1
          else if lowercase(aExecutavel)=lowercase('GourmetMen.exe') then
            vpContadorMeNuvem:=vpContadorMeNuvem+1
          else if lowercase(aExecutavel)=lowercase('GereWhatsGou.exe') then
            vpContadorWhatsApp:=vpContadorWhatsApp+1
          else if lowercase(aExecutavel)=lowercase('MizioServidorArquivo.exe') then
            vpContadorServidorArquivos:=vpContadorServidorArquivos+1;




          if (vpContadorProducao>=5) or
             (vpContadorAiqFome>=5) or
             (vpContadorMeNuvem>=5) or
             (vpContadorWhatsApp>=5) or
             (vpContadorServidorArquivos>=5) then
          begin
            // Memo1.Lines.Clear;
...
begin
  VerificaStatusPrograma('8096','MizioServidorArquivo.exe',relogioServidorArquivos,plStatusServidorArquivos, PnlDataHoraServidorArquivos );
end;
...
Begin
  ...
  With arquini Do
  Begin
    vpServidorArquivos := ReadString('posi', 'servidorarquivos', '');

    vpServidor := ReadString('posi', 'servidor', '127.0.0.1');
    if vpServidorArquivos='' then
      vpServidorArquivos:=vpServidor;
  End;
  arquini.Free;
