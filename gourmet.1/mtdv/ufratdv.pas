...
uses
  ...
  uPegaBase, uFuncoes, System.ImageList, Vcl.ImgList;
...
begin
  ...
  begin
    ...
    begin
      consulta.Close;
      consulta.SQL.Text := 'delete from idt where idtchave=' + idtidtchave.AsString;
      consulta.ExecSQL;
...
begin
  inherited;
  if qitmitmquantidade.AsFloat <= jitmitmquantidade.AsFloat then
  begin
    ShowMessage('Este item não pode mais ser devolvido!');
    exit;
  end;

  if not mdt.Active then
  begin
    mdt.Open;
