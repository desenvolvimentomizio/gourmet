...
type
  Tfintegracaostone = class(TForm)
    ...
    plPedidos: TPanel;
    dtlrec: TUniQuery;
    dtlrecmdaidentificacao: TStringField;
    dtlrecdtlvalor: TFloatField;
    dtlrecltechave: TIntegerField;
    dtlrecmdacodigo: TIntegerField;
    dtlrecrdcnrauto: TStringField;
    dtlrecdtlchave: TIntegerField;
    orcssaioriidentificacao: TStringField;
    mostra: TProgressBar;
    Panel1: TPanel;
    mmlog: TMemo;
    mprocessado: TMemo;
    mesaiq: TUniQuery;
    mesaiqmeschave: TIntegerField;
    mesaiqsdecodigo: TStringField;
    mesaiqtdfcodigo: TStringField;
    dtlaiq: TUniQuery;
    dtlaiqmdaidentificacao: TStringField;
    dtlaiqdtlvalor: TFloatField;
    dtlaiqltechave: TIntegerField;
    dtlaiqmdacodigo: TIntegerField;
    dtlaiqrfichave: TIntegerField;
    ...
    procedure lteBeforePost(DataSet: TDataSet);
  protected
    Acesso: TAcesso;
  ...
  private
    ...
    procedure ProcessaRecebimentosStone;
...
var
  fintegracaostone: Tfintegracaostone;
...
begin
  FintegracaoStone := Tfintegracaostone.Create(AOwner);
end;
...
function  Tfintegracaostone.recebimentoDireto(aOrcChave:String;aPedido:String;aMaquinaPOS:String; aValor:Double; aNome:string):string;
var
 lRes  : IResponse;
 lJson : TJson;
...
procedure  Tfintegracaostone.ProcessaPagamentos(aOrcChave:String;aNumeroPedido:String;aIdPagamento:String);
var
 lRes  : IResponse;
 lJson : TJson;
 valor : String;
  ...
  try
    ...
    try
      ...
      begin
        ...
        on E: Exception do
          begin
            mmlog.Lines.Add(Datetimetostr(now())+' '+e.Message);
            vlRequest:=False;
            vlTentativas:=vlTentativas+1;
            sleep(300);
      ...
      begin
        ...
        begin
          ...
          rpwemaberto.close;
          rpwemaberto.Connection:=Conexao;
         ...
         // begin
            ...
            begin
              ...
              rctorc.Close;
              rctorc.ParamByName('orcchave').AsString := aOrcChave;
              rctorc.Open;
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      mmlog.Lines.Add(Datetimetostr(now())+' '+'   Ajustado valor do Lote: '+lteltetotal.AsString);
...
Function Tfintegracaostone.SalvaRegistroLote:String;
var
  vlTentativas:Integer;
  vlLteChave:Integer;
begin
  morlte.Close;
  morlte.Connection:=Conexao;
  ...
  begin
    ...
    on E: Exception do
      begin
        ...
        mmlog.Lines.Add(Datetimetostr(now())+' '+' Erro ao Salvar Lote ');
...
procedure Tfintegracaostone.DadosCaixa;
var
  qccx: tuniquery;
...
begin
  ...
  try
    ...
    qccx.SQL.Text := 'select ccxchave, ctacodigo,clbcodigo, trmcodigo from ccx where ctacodigo=' + vlctadelivery + ' and ccxfinal is null';
    qccx.Open;
    ...
    begin
      ...
      vpTrmcodigo := qccx.FieldByName('trmcodigo').AsInteger;
      vpclbcodigo:= qccx.FieldByName('clbcodigo').AsInteger;

      acesso.IdAcesso:=1;
      acesso.usuario:=vpclbcodigo;
      acesso.Filial:=1;
      acesso.Terminal:=vpTrmcodigo;

      mmlog.Lines.Add(Datetimetostr(now())+' '+' Caixa: '+vpCcxchave.ToString);
...
procedure Tfintegracaostone.ProcessaRecebimento;
var
  vlOrcchave:Integer;
begin
  ...
  mmlog.Lines.Add(Datetimetostr(now())+' '+' ************** Inicia Recebimento OrcChave: '+vpOrcChave.ToString);
 ...
 { if orc.FieldByName('stocodigo').AsInteger=0 then
  ...
  begin
    mmlog.Lines.Add(Datetimetostr(now())+' '+'Recebimento OrcChave: '+vpOrcChave.ToString);
    ...
    begin
      ...
      begin
        ...
        mmlog.Lines.Add(Datetimetostr(now())+' '+'Dados da entidade OrcChave: '+vlOrcchave.ToString);


        FinalizaRecebimento;

        mmlog.Lines.Add(Datetimetostr(now())+' '+'Finalizado Recebimento OrcChave: '+vlOrcchave.ToString);
      ...
      orc.close;
      orc.sql.Text:='select orcchave, stocodigo,orctotalav  from orc where orcchave='+vlOrcchave.ToString;
      orc.Open;
      ...
      mmlog.Lines.Add(Datetimetostr(now())+' '+' ************** Conclui Processamento OrcChave: '+vlOrcchave.ToString);
      mprocessado.Lines.Add(Datetimetostr(now())+' '+' Processado com sucesso OrcChave: '+vlOrcchave.ToString);


    end
    else
    begin
      mmlog.Lines.Add(Datetimetostr(now())+' '+' ???????  OrcChave: '+vpIdPagamento);
...
procedure Tfintegracaostone.DadosConfiguracao;
var
  vlcfg:TUniquery;
  vltrm:TUniquery;
  vladc:TUniquery;
...
begin
 ...
 { clb.Close;
  ...
  mmlog.Lines.Add(Datetimetostr(now())+' '+'        ColaboradorOrcChave: '+vpOrcChave.ToString);
  mmlog.Lines.Add(Datetimetostr(now())+' '+vpClbIdentificacao);

end;



function Tfintegracaostone.ConfirmaRecebimento(aOrder:String):Boolean;
var
 lRes: IResponse;
 lJson: TJson;
 vlRequest:Boolean;
...
function Tfintegracaostone.ProcessaOrcamento:Boolean;
type
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        vlMesChave := '';
        vlMesChave := Processa(Application, Conexao, vpOrcChave.ToString, Acesso);
...
procedure Tfintegracaostone.DadosEntidadeVenda;
begin
...
function Tfintegracaostone.RegistraCCO(avalor: Double; aCtaCodigo: String;  adoacao: Integer = 0; aTfiCodigo:Integer=0; altechave:Integer=0; aDtlChave:Integer=0): string;
var
  vlCcoChave:Integer;
begin
...
procedure Tfintegracaostone.SalvaRegistroCCO(aLteChave:String; mccoChave:String);
var
  vlCcoChave : String;
  vlTentativas:Integer;
begin
...
procedure Tfintegracaostone.SalvaRegistroCLT(aCcoChave:String;aLteChave:String);
var
  vlTentativas:Integer;
begin
...
procedure Tfintegracaostone.FinalizaRecebimento;
var
...
begin
 ...
 // FinalizaRecebimentoParciais;
  ...
  mrdc.Close;
  mcco.close;
  mmlog.Lines.Add(Datetimetostr(now())+' '+'        Inicia Geração Nota OrcChave: '+vpOrcChave.ToString);

  try
    GeraNFCe(vlOrcChave.ToString);
  except
    on E: Exception do
            begin
              mmlog.Lines.Add(Datetimetostr(now())+' Erro emissão nfce '+e.Message);
            end;

    end;
  mmlog.Lines.Add(Datetimetostr(now())+' '+' Geração de Nota concluida OrcChave: '+vpOrcChave.ToString);
...
procedure Tfintegracaostone.FormCreate(Sender: TObject);
begin
...
procedure Tfintegracaostone.FormShow(Sender: TObject);
begin
  vpcontador:=0;
  Timer1.Enabled:=true;
end;

procedure Tfintegracaostone.GeraNFCe(aOrcChave:String);
var
  vlMesChave: string;
begin
  inherited;
...
procedure Tfintegracaostone.lteBeforePost(DataSet: TDataSet);
begin
 lteflacodigo.AsInteger:=1;
end;

function Tfintegracaostone.ModuloNFCe(vfuncao: string; vTrmCodigo: string; vmeschave: string; vClbCodigo: string): Boolean;
type
...
begin
  try
   mmlog.Lines.Add(Datetimetostr(now())+' Linha 2164 vai gerar nota');
  ...
  begin
    webOnLine:=true;
    vlRetorno := ModuloNFCe(Application, Self.Conexao, vmeschave, vfuncao, Acesso, false, false, '');
    Result := vlRetorno;
  end;
  except
  on E: Exception do
    begin
      mmlog.Lines.Add(Datetimetostr(now())+'  Linha 2182 '+e.Message);
    end;
...
function Tfintegracaostone.RegistraTit(aetdcodigo: Integer; atitvalor:Currency; aVencimento:TDate): Integer;
begin
...
function Tfintegracaostone.RegistraRfi(atitcodigo: Integer; aVencimento: TDate; aParcela: Integer; avalor: Double): Integer;
var
  vlRfiChave:Integer;
  vlRfiValor:Currency;
  vlMfiChave:Integer;
...
function Tfintegracaostone.SalvaRegistroTITVENDA(aLteChave:String):string;
var
 vlQuitado:Boolean;
 vlTentativas:Integer;
begin
...
procedure Tfintegracaostone.Timer1Timer(Sender: TObject);
var
...
begin
  ...
  begin
    ...
    try
     mmlog.Lines.Add(Datetimetostr(now())+' linha 2465  '+'  vai atualizar lista');

     AtualizaListaProcessaRecebimentos;

    except
    on E: Exception do
      begin
        mmlog.Lines.Add(Datetimetostr(now())+e.ClassName+' '+' linha 2472 '+e.Message);
      end;
    end;


    vpcontador:=1;
    plPedidos.Caption:='Pedidos: '+orcssai.RecordCount.ToString;

    try

      mmlog.Lines.Add(Datetimetostr(now())+' '+'Vai analizar vendas online-aiqfome');


      mesaiq.Close;
    //  mesaiq.ParamByName('cznchave').AsInteger:= vpCznChave;
      mesaiq.Open;

      mmlog.Lines.Add(Datetimetostr(now())+' '+'Achou '+mesaiq.RecordCount.ToString+ ' vendas online-aiqfome');

      while not mesaiq.Eof do
      begin
        try
          dtlaiq.Close;
          dtlaiq.ParamByName('meschave').AsInteger:=mesaiqmeschave.AsInteger;
          dtlaiq.Open;
        except
            on E: Exception do
            begin
              mmlog.Lines.Add(Datetimetostr(now())+' linha 2498 Erro emissão nfce '+e.Message);
            end;
        end;
       ...
       //   GeraNFCe(mesaiqorcchave.asstring );
        ...
        if ((dtlaiqmdacodigo.AsInteger=4) or
           (dtlaiqmdacodigo.AsInteger=5)) and
           (trim(dtlaiqrdcnrauto.AsString)<>'') and
           (pos('{',dtlaiqrdcnrauto.AsString)=0) then
        begin
          mmlog.Lines.Add(Datetimetostr(now())+' '+'Vai gerar nota da venda '+dtlaiqmdaidentificacao.AsString +' '+ mesaiqmeschave.AsString);
          GeraNFCe(mesaiqorcchave.asstring );
        end;
...
procedure Tfintegracaostone.AtualizaListaProcessaRecebimentos;
var
  i:Integer;
begin
  try
    ...
    mmlog.Lines.Add(Datetimetostr(now())+' '+'Verificando Pedidos');

    mostra.Max:=orcssai.RecordCount;
    mostra.Position:= 0;
    ...
    begin
      ...
      mmlog.Lines.Add(Datetimetostr(now())+' '+'Verificando Recebimento de Pedidos');

      dtlrec.Close;
      dtlrec.ParamByName('orcchave').AsInteger:=orcssaiorcchave.AsInteger;
      dtlrec.ParamByName('flacodigo').AsInteger:=1;
      dtlrec.Open;

       mmlog.Lines.Add(Datetimetostr(now())+' '+'Código da Modalidade do Pedido '+orcssaiorcchave.AsString+' '+dtlrecmdacodigo.asstring);

      if (dtlrecmdacodigo.AsInteger=4) or
         (dtlrecmdacodigo.AsInteger=5) or
         (dtlrecmdacodigo.AsInteger=6) then
      begin


        rpworc.Close;
        rpworc.ParamByName('orcchave').AsInteger:=orcssaiorcchave.AsInteger;
        rpworc.Open;

        while not rpworc.Eof do
        begin

          rpw.close;
          rpw.ParamByName('orcchave').AsString:=orcssaiorcchave.AsString;
          rpw.ParamByName('rpwstatus').AsString:='EM ABERTO';
          rpw.ParamByName('cznchave').AsInteger:=rpworccznchave.AsInteger;
          rpw.ParamByName('rpwtoken').AsString:=rpworcrpwtoken.AsString;
          rpw.Open;

          if not rpw.IsEmpty then
          begin
            mmlog.Lines.Add(Datetimetostr(now())+' '+'Processando OrcChave: '+orcssaiorcchave.AsString+ ' Token: '+rpworcrpwtoken.AsString);
            ProcessaRecebimentosStone;
          end;
    ...
    mmlog.Lines.Add(Datetimetostr(now())+' '+'Verificando Online');
    ...
    begin
      ...
      mmlog.Lines.Add(Datetimetostr(now())+' '+'Verificando Recebimento  online de Pedidos');
      ...
      if (dtlrecmdacodigo.AsInteger=78) then // online
      begin

        mor.close;
        mor.ParamByName('orcchave').AsString:=orcssaiorcchave.AsString;
        mor.Open;
        ...
        begin
          ...
          mes.Close;
          mes.ParamByName('meschave').AsString:=mor.FieldByName('meschave').AsString;
          mes.FilterSQL:='mes.temcodigo=2';
          mes.Open;

          mmlog.Lines.Add(Datetimetostr(now())+' '+'  Vai emitir NFCe OrcChave: '+orcssaiorcchave.AsString);

          GeraNFCe(orcssaiorcchave.AsString);

          mmlog.Lines.Add(Datetimetostr(now())+' '+'  Emitiu NFCe OrcChave: '+orcssaiorcchave.AsString);
  ...
  on E: Exception do
    begin
      mmlog.Lines.Add(Datetimetostr(now())+e.ClassName+' '+' linha 2654 '+e.Message);
    end;
  end;
...
procedure Tfintegracaostone.TimerPagamentoTimer(Sender: TObject);
begin
  ...
  begin
    ...
    trmstone.Close;
    trmstone.Connection:=Conexao;
    trmstone.parambyname('trmcodigo').asinteger:=vpClbcodigo;
    trmstone.Open;

    if trmstonetrmintegrapagarme.AsInteger=1 then
    begin
      // processa venda //
...
procedure Tfintegracaostone.TimerRPWTimer(Sender: TObject);
begin
  try
...
Procedure Tfintegracaostone.ProcessaRecebimentosStone;
begin
 if not rpw.Active then
   exit;


  while not rpw.eof do
  begin
    vpValorAReceber:=rpworcorctotalav.ascurrency;
    ProcessaPagamentos(rpworcchave.AsString,
                       rpwrpwmesa.AsString,
                       rpwrpwtoken.AsString);
...
procedure Tfintegracaostone.trmstoneBeforeOpen(DataSet: TDataSet);
begin
  (DataSet as tuniquery).Connection:=Conexao;
end;

function Tfintegracaostone.SalvaRegistroCartaoCredito(aDtlChave:String):String;
var
  vlTentativas:Integer;
begin
...
function Tfintegracaostone.EncerraRecebimento(aOrder:String):Boolean;
var
 lRes: IResponse;
 lJson: TJson;
 vlTentativas:Integer;
...
procedure Tfintegracaostone.SalvaRegistroRFIVENDA(aTitCodigo:String;mtitcodigo:String; aLtechave:String; aSituacao:Boolean);
var
  vlRfiChave:String;
  vlRfiValor:Currency;
  vlTentativas:Integer;
...
procedure Tfintegracaostone.SalvaRegistroRFMVENDA(aRfichave:String);
var
  vlTentativas : Integer;
begin
...
procedure Tfintegracaostone.SalvaRegistroMFIVENDA(arfiChave:String; aRfiValor:Currency; atmfCodigo:Integer; aLteChave:String);
var
  vlmfiChave: String;
  vlTentativas : Integer;
begin
...
procedure Tfintegracaostone.SalvaRegistroMLTVENDA(aMfiChave:String; aLteChave:String);
var
  vlTentativas : Integer;
begin
    vlTentativas := 0;
...
Procedure Tfintegracaostone.ConectaBanco;
Var
  Arquini: Tinifile;
  vlNomeBanco: String;
...
procedure  Tfintegracaostone.ajustaconexao;
var
   i:integer;
begin
