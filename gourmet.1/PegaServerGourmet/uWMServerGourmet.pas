...
type
  TWMServerGourmet = class(TWebModule)
...
var
  WebModuleClass: TComponentClass = TWMServerGourmet;
...
uses uSMServerGourmet, uSCServerGourmet, Web.WebReq;

procedure TWMServerGourmet.ServerFunctionInvokerHTMLTag(Sender: TObject; Tag: TTag;
  const TagString: string; TagParams: TStrings; var ReplaceText: string);
begin
  if SameText(TagString, 'urlpath') then
    ...
    ReplaceText := uSCServerGourmet.TSCServerGourmet.ClassName
...
procedure TWMServerGourmet.WebModuleDefaultAction(Sender: TObject;
  Request: TWebRequest; Response: TWebResponse; var Handled: Boolean);
begin
...
procedure TWMServerGourmet.WebModuleBeforeDispatch(Sender: TObject;
  Request: TWebRequest; Response: TWebResponse; var Handled: Boolean);
begin
...
function TWMServerGourmet.AllowServerFunctionInvoker: Boolean;
begin
  Result := (Request.RemoteAddr = '127.0.0.1') or
    (Request.RemoteAddr = '0:0:0:0:0:0:0:1') or (Request.RemoteAddr = '::1');
end;

procedure TWMServerGourmet.WebFileDispatcher1BeforeDispatch(Sender: TObject;
  const AFileName: string; Request: TWebRequest; Response: TWebResponse;
  var Handled: Boolean);
var
  D1, D2: TDateTime;
...
procedure TWMServerGourmet.WebModuleCreate(Sender: TObject);
begin
