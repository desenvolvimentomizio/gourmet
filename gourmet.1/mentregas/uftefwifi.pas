...
type
  ...
  private
    ...
    function recebimentoDiretoSuperTEF(aOrcChave, aPedido, aMaquinaPOS: String;
      aValor: Double; aNome: string): string;
    function ExecutaCancelamentoSuperTEF(aIdPagamento,
      aOrcChave: string): boolean;
    procedure ProcessaPagamentosSuperTEF(aOrcChave, aNumeroPedido,
      aIdPagamento: String);

    function LocalizarIDPOSSuperTEF(aSerialKey:String):String;
  ...
  public
    ...
    procedure ProcessaPagamentos(aOrcChave, aNumeroPedido,
      aIdPagamento: String);
    function ExecutaCancelamento(aIdPagamento, aOrcChave: string): boolean;
    function recebimentoDireto(aOrcChave, aPedido, aMaquinaPOS: String;
      aValor: Double; aNome: string): string;
...
Const
  ...
  TokenSuperTEF = '862e3a8bbbf4b396fc8f249aba7495377bceeaf7e409659a6afcf3f7a627996a';
...
{$R *.dfm}
function Tftefwifi.recebimentoDireto(aOrcChave:String;aPedido:String;aMaquinaPOS:String; aValor:Double; aNome:string):string;
begin
  result:='';
  ...
  if uppercase(adcadcserviconome.AsString)='MIZIO' then
    result:= recebimentoDiretoStone(aOrcChave,aPedido,aMaquinaPOS,aValor,aNome)
  else if uppercase(adcadcserviconome.AsString)='SUPERTEF' then
    result:= recebimentoDiretoSuperTEF(aOrcChave,aPedido,aMaquinaPOS,aValor,aNome);
...
function  Tftefwifi.recebimentoDiretoSuperTEF(aOrcChave:String;aPedido:String;aMaquinaPOS:String; aValor:Double; aNome:string):string;
var
 lRes  : IResponse;
 lJson : TJson;
 vlTentativas:Integer;
...
begin
  ...
  vlIDSuperTef:= aMaquinaPOS;// LocalizarIDPOSSuperTEF(aMaquinaPOS);
  vlValor:=FloatToStr(aValor);
  vlValor:=StringReplace(vlValor, ',', '.', [rfReplaceAll]);
  // Converte para float
  LFloat := StrToFloat(vlValor, TFormatSettings.Invariant);
  ...
  try
    ...
    lJson.Put('cliente_chave', vlIDSuperTef);
    lJson.Put('pos_id', 1);
    lJson.Put('transaction_type', '1');
    lJson.Put('installment_count', 1);
    lJson.put('amount', LFloat);
    lJson.Put('order_id', aPedido);
    lJson.Put('description', aNome);
    lJson.Put('installment_type', 1);
    ...
    begin
      ...
      lRes := TRequest.New.BaseURL('https://api.supertef.com.br/api/pagamentos')
                          .AddHeader('Authorization', 'Bearer ' + TokenSuperTEF)
                          .AddBody(lJson.Stringify)
                          .Post;
        vlRequest:=true;
        break;
    ...
    begin
      ...
      begin
        ...
        lJson.Parse(lRes.Content);
        vlIdPagamento := lJson.Values['payment_uniqueid'].AsString;
...
begin
  ...
  if uppercase(adcadcserviconome.AsString)='MIZIO' then
    ...
    ProcessaPagamentosSuperTEF(aOrcChave,aNumeroPedido,aIdPagamento)
...
procedure  Tftefwifi.ProcessaPagamentosSuperTEF(aOrcChave:String;aNumeroPedido:String;aIdPagamento:String);
var
 lRes  : IResponse;
 lJson : TJson;
 valor : String;
...
function Tftefwifi.LocalizarIDPOSSuperTEF(aSerialKey: String): String;
var
  lRes: IResponse;
  lJson : TJson;
...
begin
  ...
  try
    ...
    begin
      try
        lRes := TRequest.New.BaseURL(format('https://api.supertef.com.br/api/pos', [aSerialKey]))
                         .AddHeader('Authorization', 'Bearer ' + TokenSuperTEF)
                         .ContentType('application/json')
                        .Get;
    ...
    begin
      ...
      begin
        ...
        lJsonArray:= lJson.Values['data'].AsArray;
        for I := 0 to lJsonArray.Count-1 do
        begin
          vlnome:=lJsonArray.Items[i].AsObject.Values['data'].AsObject.Values['nome'].AsString;
          vlid:=lJsonArray.Items[i].AsObject.Values['data'].AsObject.Values['id'].AsInteger;
          vlstatus:=lJsonArray.Items[i].AsObject.Values['data'].AsObject.Values['status'].AsInteger;
          vlchave:=lJsonArray.Items[i].AsObject.Values['data'].AsObject.Values['chave'].AsString;
          vltoken:=lJsonArray.Items[i].AsObject.Values['data'].AsObject.Values['token'].AsString;
        end;
...
function Tftefwifi.ExecutaCancelamento(aIdPagamento:string; aOrcChave:string):boolean;
begin
...
function Tftefwifi.ExecutaCancelamentoSuperTEF(aIdPagamento:string; aOrcChave:string):boolean;
var
  lJson: TJson;
  lRes: IResponse;
  vlTentativas:Integer;
