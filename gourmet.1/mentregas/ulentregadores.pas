...
uses
  ...
  ufuncoes, frxClass, frxDACComponents, frxUniDACComponents, ufrmbase, upegabase,uni,
  MemDS, DBAccess;
...
function Tlentregadores.MostraLista(pModulo: String; pFiltro: string = ''; pChaveMestre: string = ''): string;
var
...
procedure Tlentregadores.btSelecionarMaquinaPOSClick(Sender: TObject);
var
  vlposcodigo:string;
begin
  inherited;
  ...
  vlposcodigo:=MostraLista('mpos','clbcodigo='+Dclb.DataSet.FieldByName('clbcodigo').AsString);
  ...
  begin
    ...
    consulta.SQL.Text:='update pos set  clbcodigo='+Dclb.DataSet.FieldByName('clbcodigo').AsString+
