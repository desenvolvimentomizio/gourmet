...
uses
  ...
  Vcl.Controls, Winapi.Windows, Winapi.Messages, WebView2;
...
type
  ...
  public
    ...
    /// </summary>
    [HPPGEN(
      'System::UnicodeString GetBrowserExecutableFolder();' + sLineBreak +
      'System::UnicodeString GetUserDataFolder();' + sLineBreak +
      'void SetBrowserExecutableFolder(const System::UnicodeString s);' + sLineBreak +
      'void SetUserDataFolder(const System::UnicodeString s);' + sLineBreak +
      '__property System::UnicodeString BrowserExecutableFolder = {read=GetBrowserExecutableFolder, write=SetBrowserExecutableFolder};' + sLineBreak +
      '__property System::UnicodeString UserDataFolder = {read=GetUserDataFolder, write=SetUserDataFolder};' + sLineBreak +
  ...
  /// </remarks>
  TEdgeBrowser1 = class(TCustomEdgeBrowser)
  published
    property Align;
    property Anchors;
    property TabOrder;
...
(*$HPPEMIT END '	inline System::UnicodeString TCustomEdgeBrowser::GetBrowserExecutableFolder() { return EdgeBrowserGetBrowserExecutableFolder(this); }'  *)
(*$HPPEMIT END '	inline System::UnicodeString TCustomEdgeBrowser::GetUserDataFolder(){ return EdgeBrowserGetUserDataFolder(this); }' *)
(*$HPPEMIT END '	inline void TCustomEdgeBrowser::SetBrowserExecutableFolder(const System::UnicodeString s) { EdgeBrowserSetBrowserExecutableFolder(this, s); }'  *)
(*$HPPEMIT END '	inline void TCustomEdgeBrowser::SetUserDataFolder(const System::UnicodeString s){ EdgeBrowserSetUserDataFolder(this, s); }' *)
(*$HPPEMIT END CLOSENAMESPACE*)

  TEdgeBrowserHelper = class helper for TCustomEdgeBrowser
    class var FBrowserExecutableFolders: TDictionary<TCustomEdgeBrowser, string>;
    class var FUserDataFolders: TDictionary<TCustomEdgeBrowser, string>;
  ...
  /// <summary>
  ///   Exception type used to indicate an exceptional circumstance from within the TEdgeBrowser1 component
  /// </summary>
...
function EdgeBrowserGetBrowserExecutableFolder(EdgeBrowser: TCustomEdgeBrowser): string;
function EdgeBrowserGetUserDataFolder(EdgeBrowser: TCustomEdgeBrowser): string;
procedure EdgeBrowserSetBrowserExecutableFolder(EdgeBrowser: TCustomEdgeBrowser; const S: string);
procedure EdgeBrowserSetUserDataFolder(EdgeBrowser: TCustomEdgeBrowser; const S: string);
...
uses
  System.SysUtils, Winapi.ShLwApi, Winapi.ActiveX, Vcl.Forms, Vcl.EdgeConst;
...
begin
  ...
  if FBrowserExecutableFolders <> nil then
    FBrowserExecutableFolders.Remove(Self);
  if FUserDataFolders <> nil then
    FUserDataFolders.Remove(Self);
  inherited;
end;
...
begin
  ...
  if WebviewProcessID <> 0 then begin
...
begin
  if FWebView <> nil then try
    ...
    for var CDPEventName in FDevToolsProtocolEventReceivedTokenMap.Keys do begin
  ...
  if FWebViewController <> nil then try
...
begin
  if ProcessHResult(AResult) then begin
    ...
    if not ProcessHResult(AHResult) then begin
      FBrowserControlState:=TBrowserControlState.Failed;
      FOnCreateWebViewCompleted(Self, AHResult);
    end;
  end else
    if Assigned(FOnCreateWebViewCompleted) then begin
      FBrowserControlState:=TBrowserControlState.Failed;
...
begin
  ...
  if (FWebView = nil) and (FBrowserControlState <> TBrowserControlState.Creating) then begin
...
begin
  ...
  if ProcessHResult(AResult) then begin
    ...
    if ProcessHResult(LHResult) then begin
      ...
      if ProcessHResult(LHResult) then begin
        ...
        if (FWebView <> nil) and (FWebViewController <> nil) then begin
          ...
          FWebView.add_NavigationStarting(
            Callback<ICoreWebView2, ICoreWebView2NavigationStartingEventArgs>.CreateAs<ICoreWebView2NavigationStartingEventHandler>(
              ...
              begin
                Result:=S_OK;
                if Assigned(FOnNavigationStarting) then begin
...
begin
  if FWebViewSettings <> nil then begin
    var ResultInt: Integer;
...
begin
  if FWebView <> nil then begin
    var Title: PChar:=nil;
    if ProcessHResult(FWebView.get_DocumentTitle(Title)) then begin
      Result:=Title;
      CoTaskMemFree(Title);
    end;
  end;
...
begin
  ...
  if not ProcessHResult(hr) and Assigned(FOnCreateWebViewCompleted) then begin
    FBrowserControlState:=TBrowserControlState.Failed;
    FOnCreateWebViewCompleted(Self, hr);
  end;
end;

function TCustomEdgeBrowser.Navigate(const AUrl: string): Boolean;
begin
  Result:=False;
  if FWebView = nil then begin
    if BrowserControlState = TBrowserControlState.None then
      CreateWebView;
    if AUrl.Trim.Length > 0 then
      FLastURL:=AUrl;
  end else
    ProcessHResult(FWebView.Navigate(PChar(AUrl)));
end;
...
begin
  inherited;
  if FWebViewController <> nil then begin
...
begin
  if FWebView = nil then Exit;
  var Receiver: ICoreWebView2DevToolsProtocolEventReceiver;
  if ProcessHResult(FWebView.GetDevToolsProtocolEventReceiver(PChar(CDPEventname), Receiver)) then begin
    // If we are already subscribed to this event, unsubscribe first.
    var PreExistingToken: EventRegistrationToken;
    if FDevToolsProtocolEventReceivedTokenMap.TryGetValue(CDPEventName, PreExistingToken) then begin
    ...
    ProcessHResult(Receiver.add_DevToolsProtocolEventReceived(
      Callback<ICoreWebView2, ICoreWebView2DevToolsProtocolEventReceivedEventArgs>.CreateAs<ICoreWebView2DevToolsProtocolEventReceivedEventHandler>(
        ...
        begin
          Result:=S_OK;
          if Assigned(FOnDevToolsProtocolEventReceived) then begin
...
procedure TCustomEdgeBrowser.CMVisibleChanged(var &Message: TMessage);
begin
  inherited;
  if FWebViewController <> nil then
...
procedure TCustomEdgeBrowser.WMSysCommand(var &Message: TWMSysCommand);
begin
  if FWebViewController <> nil then
    case &Message.CmdType of
      SC_MINIMIZE: ProcessHResult(FWebViewController.Set_IsVisible(Integer(LongBool(False))));
      SC_RESTORE: ProcessHResult(FWebViewController.Set_IsVisible(Integer(LongBool(True))));
    end
end;
...
begin
  if FBrowserExecutableFolders = nil then
    FBrowserExecutableFolders:=TDictionary<TCustomEdgeBrowser, string>.Create;
...
begin
  if FUserDataFolders = nil then
    FUserDataFolders:=TDictionary<TCustomEdgeBrowser, string>.Create;
...
finalization
  ...
  FreeAndNil(TEdgeBrowser1.FBrowserExecutableFolders);
  FreeAndNil(TEdgeBrowser1.FUserDataFolders);
end.
