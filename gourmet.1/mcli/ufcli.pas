...
uses
  ...
  DBAccess, Uni, Vcl.Buttons, Vcl.ComCtrls, ufrabase, ufuncoes, Vcl.Dialogs,
  Vcl.Graphics, System.SysUtils, System.StrUtils, System.ImageList;
...
type
  Tfcli = class(Tfrmbase)
    ...
    cfgcfgctbcliindiv: TIntegerField;
    cfgcfgctbcli: TIntegerField;
    pcg: tuniquery;
    ...
    Label10: TLabel;
    dpflocaltrabalho: TDBEdit;
    Label11: TLabel;
    dpfcontatotrabalho: TDBEdit;
    Label13: TLabel;
    dpfdataadmissaotrabalho: TDBEdit;
    Label14: TLabel;
    dpfrendamensal: TDBEdit;
    Label15: TLabel;
    dpfedrcodigo: TDBEdit;
    Label16: TLabel;
    dpfetfcodigo: TDBEdit;
    dpfedr: tuniquery;
    edpetf: tuniquery;
    dpfedredrcodigo: TIntegerField;
    dpfedredrendereco: TStringField;
    dpfdpfdataadmissaotrabalho: TDateField;
    cfgcfgeminclusao: TIntegerField;
    registroetdnfemodelos: TStringField;
    registrogrtcodigo: TIntegerField;
    grt: tuniquery;
    grtgrtcodigo: TIntegerField;
    grtgrtidentificacao: TStringField;
    ...
    procedure PledrResize(Sender: TObject);
  ...
  TtpFrameDocados = (tfEdr, tfEtf, tfEte);

  TFramesDocados = array [0 .. 2] of string;

const
  FramesDocados: TFramesDocados = ('edr', 'etf', 'ete');
...
Begin
  ...
  Begin
    ...
    begin
      if registrotcbcodigo.AsString = '' then
      begin
  ...
  begin
     consulta.Close;
    consulta.SQL.Text := 'update mfi,rfi set mfisituacao=-1 where mfi.rfichave=rfi.rfichave AND etdcodigo=' + Self.etdcodigo.Field.AsString;
    consulta.ExecSQL;

  consulta.Close;
    consulta.SQL.Text := 'update v_rfi set etdidentificacao=' + QuotedStr(etdidentificacao.Field.AsString) + ' where srfcodigo in (0,1)  and etdcodigo=' +
...
begin
  ...
  begin
    ...
    begin
      ...
      if grtcodigo.Visible then
      begin
        if grtcodigo.Field.AsString='' then
        begin
          grtcodigo.Field.Asinteger:=1;
        end;
      end;
...
Begin
  ...
  begin
    ...
    if (Self.registrotpecodigo.AsInteger = 1) or (Self.registrotpecodigo.AsInteger = 9) then
      registrotcbcodigo.AsInteger := 3
    else
  ...
  Begin
    ...
    Exit;
  End;
  if tpecodigo.Field.AsString = '9' then
  begin
    Bvalidar.Visible := True;
    tpecodigo.Enabled := True;
    etddoc1.Enabled := True;
    ...
    case consulta.Fields[2].AsInteger of
      ...
      tviFornecedor, tviTransportador, tviColaborador:
        If Application.MessageBox(PChar('Esta entidade já está cadastrada como ' + IfThen(consulta.Fields[2].AsInteger = tviFornecedor, 'Fornecedor !',
          IfThen(consulta.Fields[2].AsInteger = tviTransportador, ' Transportador !', ' Colaborador !')) + #13 + #13 + 'Deseja cadastrá-la como Cliente também ?'), 'Atenção',
          MB_YESNO + MB_DEFBUTTON1 + MB_ICONQUESTION) = idyes Then
        Begin
          registro.Close;
          registro.Params[0].AsInteger := consulta.Fields[0].AsInteger;
  ...
  Begin
    ...
    begin
      ...
      MostraFormu('metl', vlEtlChave, registroetdcodigo.AsString);
  ...
  begin
    ...
    if vCarregaFrame then
      ...
      begin
        If psituacao.Caption = 'Cancelando' Then
...
procedure Tfcli.etdapelidoExit(Sender: TObject);
begin
...
procedure Tfcli.etdidentificacaoExit(Sender: TObject);
begin
  if length(trim(etdidentificacao.Field.AsString))<3 then
  begin
      ShowMessage('Atenção: Este campo precisa no mínimo 3 letras !');
...
procedure Tfcli.etvAfterInsert(DataSet: TDataSet);
begin
  inherited;
  if Self.cfg.FieldByName('cfgctbcli').AsString <> '' then
    etvpcgcodigo.AsString := Self.cfg.FieldByName('cfgctbcli').AsString;
...
procedure Tfcli.PledrResize(Sender: TObject);
begin
  inherited;

  Pledr.Height := (Self.Height - 285) div 3;
  Pletf.Height := (Self.Height - 285) div 3;
  Plete.Height := (Self.Height - 285) div 3;
...
begin
 ...
 registroetdnfemodelos.AsInteger:=99;
