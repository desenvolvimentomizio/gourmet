...
type
  TfMonitorMizioServImpressoras = class(TForm)
    ...
    procedure relagioImpressorasTimer(Sender: TObject);
  private
...
begin
  ...
  relagioImpressoras.Enabled := True;
end;
...
begin
  ...
  List := GetAllProcessPathsByName('MizioImpressoras.exe');
  try
...
begin
  if fileexists(extractfilepath(application.ExeName)+'MizioImpressoras.exe') then
    ShellExecute(Handle ,'open', PChar(extractfilepath(application.ExeName)+'MizioImpressoras.exe'), '', nil, SW_SHOWNORMAL);
end;

procedure TfMonitorMizioServImpressoras.relagioImpressorasTimer(Sender: TObject);
begin
...
begin
  ...
  relagioImpressoras.Enabled := false;
