  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if vlNumePedido<=4999 then
          vlPedido:=fImpressora01.GeraTextoPedido(immorcchave.AsString,vlTciCodigo)
        else
          vlPedido:=fImpressora01.GeraTextoPedidoimm(immorcchave.AsString,vlTciCodigo,vlNumePedido.ToString);

        if EnviaTextoPedidoServidor(vlPedido,IfThen(vlNumePedido<=4999,'entrega', 'salao'), formatFloat('00000', vlNumePedido),vlTciPorta,vID)=201 then
        begin
        ...
        begin
          ...
          ConsultaImpressoraServidor(vID.ToString,  vlTciPorta);
...
begin
  try
    ...
    if (LResponse.StatusCode=200) or (LResponse.StatusCode=201) or (LResponse.StatusCode=202) then
    begin
      vlResposta:=LResponse.Content;
      result:=201;
    end
    else
    begin
