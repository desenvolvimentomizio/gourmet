  ...
  object ito: TUniQuery
    SQL.Strings = (
      ...
      'INNER JOIN tci ON imm.tcicodigo = tci.tcicodigo     '
        ...
        'LEFT JOIN sfn ON pro.procodigo = sfn.procodigo AND uni.unicodigo' +
        ' = sfn.unicodigo'
      ''
      '-- Aqui entra o LEFT JOIN da prioridade'
      'LEFT JOIN ('
      '    SELECT p.procodigo, t.tciprioridade'
      '    FROM tci t'
      '    INNER JOIN gri i ON i.tcicodigo = t.tcicodigo'
      '    INNER JOIN grp g ON i.grpcodigo = g.grpcodigo'
      '    INNER JOIN pro p ON g.grpcodigo = p.grpcodigo'
      ') AS tcip ON tcip.procodigo = ito.procodigo'
      ''
      'WHERE orc.orcchave = @orcchave'
      '  AND (@tcicodigo = 0 OR imm.tcicodigo = @tcicodigo)'
      '  AND ito.stocodigo NOT IN (9, 88)'
      '  AND ito.procodigo NOT IN (SELECT procodigo FROM bai)      '
      '  AND imm.immhoraimpresso IS NULL   '
      ''
      'ORDER BY tcip.tciprioridade, gri.gripedidoaux;'
      ''
      ''
      '')
    Left = 32
