...
uses
  ...
  Dialogs, uPegaBase, pcnConversaoNFe, Vcl.StdCtrls, pcnNFe;
...
function impnfe(AOwner: TComponent; conexao: TUniConnection; Acesso: TAcesso; chave: string): string;
Var
  i: Integer;
Begin
  try
...
function impitensnfe(AOwner: TComponent; conexao: TUniConnection; Acesso: TAcesso; chave: string): string;
Var
  i: Integer;
Begin
  try
...
exports impnfe, impitensnfe, impertificadonfe, conscertificadonfe;
...
Begin
  ...
  consulta.Close;
  consulta.SQL.Text := 'select meschave, mesregistro from mes where temcodigo<>90 and meschavenfe=''' + chave + '''' + ' and flacodigo=' + Acesso.Filial.ToString;
  consulta.Open;
  ...
  Begin
    Application.MessageBox(PChar('Atenção esta NF-e não pertence a este CNPJ!' + #13 + 'Não será importada !' + #13 + #13 + 'CNPJ Empresa: ' + vDoc1Emitente + #13 +
      'CNPJ NFe    : ' + ACBrNFeImp.NotasFiscais.Items[0].NFe.Dest.CNPJCPF), 'ATENÇÃO', MB_OK + MB_ICONWARNING);
    Result := False;
    Exit;
  End;
...
Begin
  ...
  *)
  ...
  begin
    if (ACBrNFeImp.NotasFiscais.Items[0].NFe.cobr.Fat.nFat <> '') or (ACBrNFeImp.NotasFiscais.Items[0].NFe.Ide.indPag = ipPrazo) and (cfgcfgimpfatura.AsInteger = 1) then
    begin
