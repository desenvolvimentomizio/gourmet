...
type
  ...
  Tfrabrf = class(Tfrabase)
    ...
    tabelardcnrauto: TStringField;
    vbaixardcnrauto: TStringField;
    uqtabelardcnrauto: TStringField;
    VTBaixaLterdcnrauto: TStringField;
    VTBaixaLterfisaldomora: TFloatField;
  ...
  public
    ...
    procedure SelecionarTodosBaixa;
        procedure somarbaixa;
  end;

  TContabilizacaoBaixaRfi = function(AOwner: TComponent; conexao: tuniconnection; vusuario: string; vLteChave: string; vtdgcodigo: string; vCcoChave: string): Boolean;
...
procedure Tfrabrf.SelecionarTodosBaixa;
var
  i: Integer;
begin
  try
      ...
      case CompareValue(vlValorBaixado, vlValorBxParcial) of
        ...
        GreaterThanValue:
          ...
          begin
            ...
            vbaixarfisaldogeral.AsCurrency := vbaixarfisaldocapital.AsCurrency + vbaixarfimulta.AsCurrency + vbaixarfijuros.AsCurrency - vbaixarfidescontos.AsCurrency;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        vbaixarfibaixado.AsCurrency := vbaixarfisaldocapital.AsCurrency + Self.vbaixarfijuros.AsCurrency + Self.vbaixarfimulta.AsCurrency - Self.vbaixarfidescontos.AsCurrency;
...
begin
  ...
  begin
    ...
    IdentificaBaixa.SQL.Text := 'SELECT mfi.mfivalor,  mlt.ltechave,  rfi.etdcodigo,  etd.etdidentificacao,  mfi.rfichave,  lte.tfdcodigo, rfi.rfinumero FROM mlt ';
    ...
    if IdentificaBaixa.Fields[5].AsInteger = tfdRecebimentos then
      vHistorico := 'Recto Títulos Nº: '
    else
      vHistorico := 'Pagto Títulos Nº: ';
...
type
  TregistraloteGourmnet = Function(AOwner: TComponent; conexao: tuniconnection; vchave: string; vtrmcodigo: string; principal: string; Multa: string; juros: string; Desconto: string;
    ...
    pCtaCaixa: Integer = 0; vPodeTrocaDoacao: Boolean = True; vControleEntrega: boolean = False; vCcxchave: Integer = 0; vetdcodigo:Integer=0; vComplemento:Boolean=false): string;
var
  Registra: TregistraloteGourmnet;
  Pack: Cardinal;
  Vpri: String;
  vmulta: String;
  Vjur: String;
...
Begin
  ...
  If Pack <> 0 Then
    Try
      @Registra := GetProcAddress(Pack, PChar('registralotedatagourmet'));
      ...
      Begin
        ...
        begin
          ...
          consulta.Close;
          consulta.SQL.Text:='select ccxchave from lte where ltechave='+vchave;
          consulta.Open;
          ...
          if not Renegociacao then
            vRetBaixa := Registra(Application, zcone, vchave, '1', Vpri, vmulta, Vjur, Vdes, acesso, vmodo, diabaixa.Date, vPodeConvenio, 0, 0, vPodeCartoes,
              RecebCaixa.Conta, False,false,vlccxchave)
          else
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      consulta.Close;
                      consulta.SQL.Text := 'select mcgchave from mcg where rfichave=' + mcgrfirfichave.AsString + ' and ccgcodigo=' + mcgrficcgcodigo.AsString + ' and mcgtabela=' +
                        QuotedStr('cco') + ' and mcgchaveorigem=' + vpCcoChave;
                      consulta.Open;
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      consulta.Close;
                      consulta.SQL.Text := 'select mcgchave from mcg where ccgcodigo=' + cfgccgmultajuros.AsString + ' and mcgchaveorigem=' + vpCcoChave + ' and mcgtabela=' +
                        QuotedStr('cco');
                      consulta.Open;
          ...
          if not Renegociacao then
            ...
            begin
              if fileexists(ExtractFilePath(Application.ExeName) + 'Relat\ReciboCPA.fr3') then
                mrfrImprimir(Application, zcone, DVTBaixaLte, ExtractFilePath(Application.ExeName) + 'Relat\ReciboCPA.fr3', tiImprimir, acesso.Usuario.ToString);
