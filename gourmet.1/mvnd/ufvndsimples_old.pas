...
type
  Tfvndsimples = class(Tfrmbase)
    ...
    registromespararevenda: TIntegerField;
    sen: tuniquery;
    sensencodigo: TIntegerField;
    labmespararevenda: TLabel;
    mespararevenda: TDBEdit;
  ...
  TajustaDesc = Function(AOwner: TComponent; Conexao: tuniconnection; vDesconto, vPercentual, vvalor, vbruto, vtipo, vusrcodigo, Vpermax: String;
...
begin
  ...
  if pack <> 0 then
    try
      ...
      begin
        vu := vusrcodigo;
        vret := AjusDesc(Application, Self.zcone, vDesconto, vPercentual, vvalor, vbruto, vtipo, vusrcodigo, xpermax, vTdeCodigo, '',
          registromeschave.AsString);
      end;
    finally
...
begin
  ...
  vRetorno := mostradesconto(Self.itmitmdesconto.AsString, Self.itmitmpercdesc.AsString, Self.itmitmvalor.AsString, Self.itmitmtotal.AsString,
    'Valor Líquido do Item R$', IntToStr(Acesso.Usuario), IntToStr(tdeIndividual));
...
Begin
  ...
  VRetAjuDesc := '0';
  VRetAjuDesc := mostradesconto(Self.mesdesconto.Field.AsString, '0', Self.registromesvalor.AsString, Self.registromesvalor.AsString,
    'Valor Líquido da Venda R$', IntToStr(Acesso.Usuario), IntToStr(tdeTotal));
  ...
  Begin
    ...
    begin
      ...
      begin
        ...
        If Application.MessageBox(PChar('Remover registros Financeiros?'), PChar('Excluir'), MB_TASKMODAL + MB_ICONQUESTION + MB_YESNO + MB_DEFBUTTON2)
          = IDYES Then
        Begin
  ...
  begin
    ...
    begin
      if Application.MessageBox(PChar('Cliente possui Bloqueio no cadastro!!' + #13 + 'Liberar venda?'), 'Atenção', MB_YESNO + MB_ICONWARNING) = IDYES then
        ...
        if Application.MessageBox(PChar('Esta atendimento já foi REFATURADO!' + #13 + 'Já tem registro financanceiro deseja importar realmente os itens? '),
...
begin
  ...
  registromespararevenda.AsInteger := 0;
end;
...
begin
  ...
  begin
    ...
    begin
      consulta.Close;
      consulta.SQL.Text := 'select toecfopsaida,ttocodigo from toe where toecodigo=' + Self.toecodigo.Field.AsString;
      consulta.Open;
      ...
      if consulta.FieldByName('ttocodigo').AsInteger = ttoVenda then
      begin
        labmespararevenda.Visible := True;
        mespararevenda.Visible := True;
        mespararevenda.TabStop := True;
        mespararevenda.SetFocus;
      end
      else
      begin
        labmespararevenda.Visible := False;
        mespararevenda.Visible := False;
        mespararevenda.TabStop := False;
