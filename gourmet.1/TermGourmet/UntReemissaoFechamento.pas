...
procedure TFrmReemissaoFechamento.ImprimeResumo(pOrcChave: Integer; pOrigem: Integer = 1);
var
  vlNomeArq: string;
  vlsql: string;
begin

  vlNomeArq := Extractfilepath(Application.ExeName) + 'relat\itensdamesmesa.fr3';
  ...
  begin
    ...
    begin
      mrfrImprimir(dmdados.dmsaimprime, vlNomeArq, tiImprimir);
      vlsql := 'update orc set orcimpressao = (IFNULL(orcimpressao,0) + 1) where orcchave = ' + InttoStr(pOrcChave);
      dmdados.ExecutaSQL(vlsql);
...
procedure TFrmReemissaoFechamento.Button1Click(Sender: TObject);
var
 vlOrcChave:String;
 vlNomeArq:String;
begin
  ...
  vlNomeArq := Extractfilepath(Application.ExeName) + 'relat\itensdamesmesa.fr3';
  vlOrcChave:=DmDados.ditoparc.DataSet.FieldByName('orcchave').AsString;

  if FileExists(vlNomeArq) then
  begin

    dmdados.ConsultaSQL('SELECT * from orc where orcchave=' +vlOrcChave);

    DmDados.dadosrelatorio.close;
    DmDados.dadosrelatorio.sql.text:='SELECT * FROM orc WHERE orcchave='+vlOrcChave;
    DmDados.dadosrelatorio.open;

    if not DmDados.dadosrelatorio.isempty then
    begin

      mrfrImprimir(DmDados.dsdadosrelatorio, vlNomeArq,  tiImprimir);
