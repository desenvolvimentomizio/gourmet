...
begin
  ...
  if not DirectoryExists( extractfilepath(application.ExeName)+'logsstone\'+vpCozinha.ToString+'\') then
    ForceDirectories( extractfilepath(application.ExeName)+'logsstone\'+vpCozinha.ToString+'\');

  NomeArquivo := extractfilepath(application.ExeName)+'logsstone\'+vpCozinha.ToString+'\'+ Format('Logos_%s.txt', [FormatDateTime('yyyy-mm-dd_hh-nn-ss', Now)]);
