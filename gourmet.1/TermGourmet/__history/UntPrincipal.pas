            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  if plmesasemfechamento.ComponentCount<=8 then
                  begin

                {   if plmesasemfechamento.Visible then
...
begin
  ...
  begin
    ...
    { with dmdados do
      begin
      if Autorizado(ActCancelaTodos, 'Cacelamento de todos itens da comanda') then
      begin
      if not ito.IsEmpty then
      begin
      if MessageDlg('Tem certeza que deseja cancelar o pedido ?' + #13 + #13 +
      'Após o cancelamento não será mais possível recuperar os itens do pedido !', mtConfirmation, [mbYes, mbNo], 0, mbNo) = mrYes then
      begin
      if ExecutaSQL('SET @orcchave = ' + orcorcchave.AsString + '; ' + 'UPDATE orc SET stocodigo = 88 WHERE orcchave = @orcchave; ' +
