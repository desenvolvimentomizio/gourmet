object FrmIngredienteFracionado: TFrmIngredienteFracionado
  Left = 0
  Top = 0
  BorderIcons = []
  BorderStyle = bsToolWindow
  ClientHeight = 719
  ClientWidth = 998
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 998
    Height = 719
    Align = alClient
    BevelKind = bkFlat
    BevelOuter = bvNone
    TabOrder = 0
    object Panel11: TPanel
      Left = 0
      Top = 0
      Width = 994
      Height = 50
      Align = alTop
      BevelKind = bkFlat
      BevelOuter = bvNone
      Color = 12615680
      ParentBackground = False
      TabOrder = 0
      object LbItem: TLabel
        AlignWithMargins = True
        Left = 12
        Top = 3
        Width = 975
        Height = 43
        Align = alClient
        Caption = 'Item'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -32
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        ExplicitWidth = 78
        ExplicitHeight = 39
      end
      object Panel2: TPanel
        Left = 0
        Top = 0
        Width = 9
        Height = 46
        Align = alLeft
        BevelOuter = bvNone
        Color = 12615680
        ParentBackground = False
        TabOrder = 0
      end
    end
    object Panel8: TPanel
      Left = 0
      Top = 50
      Width = 994
      Height = 5
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 1
    end
    object PCGeral: TPageControl
      Left = 0
      Top = 55
      Width = 994
      Height = 660
      ActivePage = TabGerSabores
      Align = alClient
      TabOrder = 2
      object TabGerTamanho: TTabSheet
        Caption = 'Tamanhos'
        object Panel22: TPanel
          Left = 0
          Top = 335
          Width = 7
          Height = 252
          Align = alLeft
          BevelOuter = bvNone
          ParentBackground = False
          TabOrder = 0
        end
        object Panel25: TPanel
          Left = 0
          Top = 594
          Width = 986
          Height = 38
          Align = alBottom
          BevelOuter = bvNone
          ParentBackground = False
          TabOrder = 1
          object Panel26: TPanel
            Left = 735
            Top = 0
            Width = 251
            Height = 38
            Align = alRight
            BevelOuter = bvNone
            TabOrder = 0
            object Button7: TButton
              Left = 2
              Top = 4
              Width = 120
              Height = 31
              Caption = 'Sabores'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -15
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
              TabOrder = 0
              OnClick = Button7Click
            end
            object Button8: TButton
              Left = 124
              Top = 4
              Width = 120
              Height = 31
              Caption = 'Cancelar'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -15
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
              TabOrder = 1
              OnClick = Button8Click
            end
          end
        end
        object Panel27: TPanel
          Left = 7
          Top = 335
          Width = 972
          Height = 252
          Align = alClient
          BevelOuter = bvNone
          TabOrder = 2
        end
        object Panel38: TPanel
          Left = 0
          Top = 0
          Width = 986
          Height = 335
          Align = alTop
          BevelOuter = bvNone
          TabOrder = 3
          object Panel35: TPanel
            Left = 0
            Top = 0
            Width = 986
            Height = 32
            Align = alTop
            BevelKind = bkFlat
            BevelOuter = bvNone
            Color = 12615680
            ParentBackground = False
            TabOrder = 0
            object Label12: TLabel
              AlignWithMargins = True
              Left = 3
              Top = 3
              Width = 976
              Height = 25
              Align = alClient
              Alignment = taCenter
              Caption = 'Tamanhos'
              Color = 12615680
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWhite
              Font.Height = -17
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentColor = False
              ParentFont = False
              ExplicitWidth = 88
              ExplicitHeight = 21
            end
          end
          object DBGrid7: TDBGrid
            Left = 7
            Top = 32
            Width = 972
            Height = 303
            Align = alClient
            Color = 13303807
            DataSource = DmDados.dsfn
            DrawingStyle = gdsGradient
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -16
            Font.Name = 'Tahoma'
            Font.Style = []
            Options = [dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgTitleHotTrack]
            ParentFont = False
            TabOrder = 1
            TitleFont.Charset = DEFAULT_CHARSET
            TitleFont.Color = clWindowText
            TitleFont.Height = -11
            TitleFont.Name = 'Tahoma'
            TitleFont.Style = []
            OnDblClick = DBGrid7DblClick
            OnKeyDown = DBGrid7KeyDown
            Columns = <
              item
                Expanded = False
                FieldName = 'pronomereduzido'
                Width = 700
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'punprecoav'
                Width = 120
                Visible = True
              end>
          end
          object Panel28: TPanel
            Left = 0
            Top = 32
            Width = 7
            Height = 303
            Align = alLeft
            BevelOuter = bvNone
            ParentBackground = False
            TabOrder = 2
          end
          object Panel29: TPanel
            Left = 979
            Top = 32
            Width = 7
            Height = 303
            Align = alRight
            BevelOuter = bvNone
            ParentBackground = False
            TabOrder = 3
          end
        end
        object PnBorda: TPanel
          Left = 7
          Top = 335
          Width = 972
          Height = 252
          Align = alClient
          BevelOuter = bvNone
          TabOrder = 4
          object Panel32: TPanel
            Left = 0
            Top = 0
            Width = 972
            Height = 252
            Align = alClient
            BevelOuter = bvNone
            TabOrder = 0
            object DBGrid6: TDBGrid
              Left = 496
              Top = 312
              Width = 450
              Height = 288
              DataSource = DmDados.dsbr
              DrawingStyle = gdsGradient
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -16
              Font.Name = 'Tahoma'
              Font.Style = []
              Options = [dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgTitleHotTrack]
              ParentFont = False
              TabOrder = 0
              TitleFont.Charset = DEFAULT_CHARSET
              TitleFont.Color = clWindowText
              TitleFont.Height = -11
              TitleFont.Name = 'Tahoma'
              TitleFont.Style = []
              Columns = <
                item
                  Expanded = False
                  FieldName = 'pronome'
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clWindowText
                  Font.Height = -15
                  Font.Name = 'Tahoma'
                  Font.Style = [fsBold]
                  ReadOnly = True
                  Title.Font.Charset = DEFAULT_CHARSET
                  Title.Font.Color = clWindowText
                  Title.Font.Height = -13
                  Title.Font.Name = 'Tahoma'
                  Title.Font.Style = []
                  Width = 270
                  Visible = True
                end
                item
                  Expanded = False
                  FieldName = 'tsiopcao'
                  Title.Caption = 'Quatidade'
                  Width = 100
                  Visible = True
                end>
            end
            object Panel33: TPanel
              Left = 0
              Top = 0
              Width = 972
              Height = 32
              Align = alTop
              BevelKind = bkFlat
              BevelOuter = bvNone
              Color = 12615680
              ParentBackground = False
              TabOrder = 1
              object Label11: TLabel
                AlignWithMargins = True
                Left = 3
                Top = 3
                Width = 962
                Height = 25
                Align = alClient
                Alignment = taCenter
                Caption = 'Bordas'
                Color = 12615680
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clWhite
                Font.Height = -17
                Font.Name = 'Tahoma'
                Font.Style = [fsBold]
                ParentColor = False
                ParentFont = False
                ExplicitWidth = 60
                ExplicitHeight = 21
              end
            end
            object DBGridBordas: TDBGrid
              Left = 0
              Top = 32
              Width = 972
              Height = 220
              Align = alClient
              Color = 13303807
              DataSource = DmDados.dbrd
              DrawingStyle = gdsGradient
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -16
              Font.Name = 'Tahoma'
              Font.Style = []
              Options = [dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgTitleHotTrack]
              ParentFont = False
              TabOrder = 2
              TitleFont.Charset = DEFAULT_CHARSET
              TitleFont.Color = clWindowText
              TitleFont.Height = -11
              TitleFont.Name = 'Tahoma'
              TitleFont.Style = []
              OnCellClick = DBGridBordasCellClick
              OnDrawColumnCell = DBGridBordasDrawColumnCell
              OnKeyDown = DBGridBordasKeyDown
              Columns = <
                item
                  Alignment = taCenter
                  Expanded = False
                  FieldName = 'sel'
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clWindowText
                  Font.Height = -9
                  Font.Name = 'Tahoma'
                  Font.Style = []
                  Title.Caption = 'Quatidade'
                  Width = 25
                  Visible = True
                end
                item
                  Expanded = False
                  FieldName = 'brdidentificacao'
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clWindowText
                  Font.Height = -15
                  Font.Name = 'Tahoma'
                  Font.Style = [fsBold]
                  ReadOnly = True
                  Title.Font.Charset = DEFAULT_CHARSET
                  Title.Font.Color = clWindowText
                  Title.Font.Height = -13
                  Title.Font.Name = 'Tahoma'
                  Title.Font.Style = []
                  Width = 270
                  Visible = True
                end>
            end
          end
        end
        object Panel31: TPanel
          Left = 0
          Top = 587
          Width = 986
          Height = 7
          Align = alBottom
          BevelOuter = bvNone
          ParentBackground = False
          TabOrder = 5
        end
        object Panel34: TPanel
          Left = 979
          Top = 335
          Width = 7
          Height = 252
          Align = alRight
          BevelOuter = bvNone
          ParentBackground = False
          TabOrder = 6
        end
      end
      object TabGerSabores: TTabSheet
        Caption = 'Sabores'
        ImageIndex = 1
        object Panel3: TPanel
          Left = 0
          Top = 0
          Width = 444
          Height = 632
          Align = alLeft
          BevelOuter = bvNone
          Color = 13816530
          ParentBackground = False
          TabOrder = 0
          object Panel6: TPanel
            Left = 0
            Top = 0
            Width = 444
            Height = 32
            Align = alTop
            BevelKind = bkFlat
            BevelOuter = bvNone
            Color = 12615680
            ParentBackground = False
            TabOrder = 0
            object Label1: TLabel
              AlignWithMargins = True
              Left = 3
              Top = 3
              Width = 434
              Height = 25
              Align = alClient
              Alignment = taCenter
              Caption = 'Dispon'#237'veis'
              Color = 12615680
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWhite
              Font.Height = -17
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentColor = False
              ParentFont = False
              ExplicitWidth = 99
              ExplicitHeight = 21
            end
          end
          object DBGrid4: TDBGrid
            Left = 2
            Top = 86
            Width = 442
            Height = 506
            Align = alClient
            Color = 13303807
            DataSource = DmDados.dfsbr
            DrawingStyle = gdsGradient
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -16
            Font.Name = 'Tahoma'
            Font.Style = []
            Options = [dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgTitleHotTrack]
            ParentFont = False
            TabOrder = 1
            TitleFont.Charset = DEFAULT_CHARSET
            TitleFont.Color = clWindowText
            TitleFont.Height = -11
            TitleFont.Name = 'Tahoma'
            TitleFont.Style = []
            OnDblClick = DBGrid4DblClick
            OnKeyDown = DBGrid4KeyDown
            OnKeyUp = DBGrid4KeyUp
            Columns = <
              item
                Expanded = False
                FieldName = 'sbridentificacao'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clWindowText
                Font.Height = -15
                Font.Name = 'Tahoma'
                Font.Style = [fsBold]
                ReadOnly = True
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clWindowText
                Title.Font.Height = -13
                Title.Font.Name = 'Tahoma'
                Title.Font.Style = []
                Width = 400
                Visible = True
              end>
          end
          object Panel16: TPanel
            Left = 0
            Top = 86
            Width = 2
            Height = 506
            Align = alLeft
            BevelOuter = bvNone
            TabOrder = 2
          end
          object Panel21: TPanel
            Left = 0
            Top = 32
            Width = 444
            Height = 54
            Align = alTop
            BevelOuter = bvNone
            ParentBackground = False
            TabOrder = 3
            object Label2: TLabel
              Left = 17
              Top = -2
              Width = 50
              Height = 21
              Caption = 'Buscar'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -17
              Font.Name = 'Tahoma'
              Font.Style = []
              ParentFont = False
            end
            object EdtBuscaSabores: TEdit
              Left = 16
              Top = 18
              Width = 393
              Height = 27
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -16
              Font.Name = 'Tahoma'
              Font.Style = []
              ParentFont = False
              TabOrder = 0
              OnChange = EdtBuscaSaboresChange
              OnEnter = EdtBuscaSaboresEnter
              OnKeyUp = EdtBuscaSaboresKeyUp
            end
          end
          object Panel30: TPanel
            Left = 0
            Top = 592
            Width = 444
            Height = 40
            Align = alBottom
            BevelOuter = bvNone
            ParentBackground = False
            TabOrder = 4
            object Button12: TButton
              Left = 4
              Top = 6
              Width = 120
              Height = 31
              Caption = 'Tamanhos'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -15
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
              TabOrder = 0
              OnClick = Button12Click
            end
          end
        end
        object Panel37: TPanel
          Left = 982
          Top = 0
          Width = 4
          Height = 632
          Align = alRight
          BevelOuter = bvNone
          TabOrder = 1
        end
        object Panel36: TPanel
          Left = 499
          Top = 0
          Width = 483
          Height = 632
          Align = alClient
          BevelOuter = bvNone
          TabOrder = 2
          object Panel7: TPanel
            Left = 0
            Top = 7
            Width = 483
            Height = 326
            Align = alClient
            BevelOuter = bvNone
            TabOrder = 0
            ExplicitHeight = 414
            object Panel10: TPanel
              Left = 0
              Top = 0
              Width = 483
              Height = 326
              Align = alClient
              BevelOuter = bvNone
              TabOrder = 0
              ExplicitHeight = 414
              object PcAdicionais: TPageControl
                Left = 0
                Top = 0
                Width = 483
                Height = 326
                ActivePage = TabAdicionais
                Align = alClient
                TabOrder = 0
                ExplicitHeight = 414
                object TabSabores: TTabSheet
                  Caption = 'Sabores'
                  ImageIndex = 2
                  ExplicitHeight = 386
                  object Panel40: TPanel
                    Left = 0
                    Top = 0
                    Width = 475
                    Height = 28
                    Align = alTop
                    BevelKind = bkFlat
                    BevelOuter = bvNone
                    Color = 12615680
                    ParentBackground = False
                    TabOrder = 0
                    object LbFracao: TLabel
                      Left = 0
                      Top = 0
                      Width = 471
                      Height = 24
                      Align = alClient
                      Alignment = taCenter
                      Caption = 'Sabores (0/0)'
                      Color = 12615680
                      Font.Charset = DEFAULT_CHARSET
                      Font.Color = clWhite
                      Font.Height = -17
                      Font.Name = 'Tahoma'
                      Font.Style = [fsBold]
                      ParentColor = False
                      ParentFont = False
                      ExplicitWidth = 122
                      ExplicitHeight = 21
                    end
                  end
                  object DBGrid1: TDBGrid
                    Left = 0
                    Top = 28
                    Width = 475
                    Height = 214
                    Align = alClient
                    Color = 13303807
                    DataSource = dvtpro
                    DrawingStyle = gdsGradient
                    Font.Charset = DEFAULT_CHARSET
                    Font.Color = clWindowText
                    Font.Height = -16
                    Font.Name = 'Tahoma'
                    Font.Style = []
                    Options = [dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgCancelOnExit, dgTitleHotTrack]
                    ParentFont = False
                    TabOrder = 1
                    TitleFont.Charset = DEFAULT_CHARSET
                    TitleFont.Color = clWindowText
                    TitleFont.Height = -11
                    TitleFont.Name = 'Tahoma'
                    TitleFont.Style = []
                    OnDblClick = DBGrid1DblClick
                    OnKeyDown = DBGrid1KeyDown
                    Columns = <
                      item
                        Expanded = False
                        FieldName = 'descricao'
                        Font.Charset = DEFAULT_CHARSET
                        Font.Color = clWindowText
                        Font.Height = -15
                        Font.Name = 'Tahoma'
                        Font.Style = [fsBold]
                        ReadOnly = True
                        Title.Font.Charset = DEFAULT_CHARSET
                        Title.Font.Color = clWindowText
                        Title.Font.Height = -13
                        Title.Font.Name = 'Tahoma'
                        Title.Font.Style = []
                        Width = 440
                        Visible = True
                      end>
                  end
                  object Panel17: TPanel
                    Left = 0
                    Top = 242
                    Width = 475
                    Height = 56
                    Align = alBottom
                    BevelOuter = bvNone
                    ParentBackground = False
                    TabOrder = 2
                    ExplicitTop = 330
                    object Label14: TLabel
                      Left = 5
                      Top = 4
                      Width = 250
                      Height = 18
                      Caption = 'Observa'#231#227'o do sabor selecionado'
                      Font.Charset = DEFAULT_CHARSET
                      Font.Color = clBlack
                      Font.Height = -15
                      Font.Name = 'Tahoma'
                      Font.Style = [fsBold]
                      ParentFont = False
                    end
                    object Panel39: TPanel
                      Left = 335
                      Top = 0
                      Width = 140
                      Height = 56
                      Align = alRight
                      BevelOuter = bvNone
                      TabOrder = 0
                      object Button9: TButton
                        Left = 18
                        Top = 14
                        Width = 120
                        Height = 31
                        Caption = '&Ingredientes'
                        Font.Charset = DEFAULT_CHARSET
                        Font.Color = clWindowText
                        Font.Height = -15
                        Font.Name = 'Tahoma'
                        Font.Style = [fsBold]
                        ParentFont = False
                        TabOrder = 0
                        OnClick = Button9Click
                      end
                    end
                    object DBMemo2: TDBMemo
                      Left = 6
                      Top = 25
                      Width = 331
                      Height = 23
                      Ctl3D = True
                      DataField = 'obs'
                      DataSource = dvtpro
                      Font.Charset = DEFAULT_CHARSET
                      Font.Color = clWindowText
                      Font.Height = -13
                      Font.Name = 'Tahoma'
                      Font.Style = []
                      ParentCtl3D = False
                      ParentFont = False
                      TabOrder = 1
                    end
                  end
                end
                object TabIngredientes: TTabSheet
                  Caption = 'Ingredientes'
                  ExplicitHeight = 386
                  object Panel13: TPanel
                    Left = 0
                    Top = 262
                    Width = 475
                    Height = 36
                    Align = alBottom
                    BevelOuter = bvNone
                    ParentBackground = False
                    TabOrder = 0
                    ExplicitTop = 350
                    object Panel14: TPanel
                      Left = 349
                      Top = 0
                      Width = 126
                      Height = 36
                      Align = alRight
                      BevelOuter = bvNone
                      TabOrder = 0
                      object Button1: TButton
                        Left = 4
                        Top = 4
                        Width = 120
                        Height = 31
                        Caption = '&Adcionais'
                        Font.Charset = DEFAULT_CHARSET
                        Font.Color = clWindowText
                        Font.Height = -15
                        Font.Name = 'Tahoma'
                        Font.Style = [fsBold]
                        ParentFont = False
                        TabOrder = 0
                        OnClick = Button1Click
                      end
                    end
                    object Button11: TButton
                      Left = 2
                      Top = 4
                      Width = 120
                      Height = 31
                      Caption = 'Sabores'
                      Font.Charset = DEFAULT_CHARSET
                      Font.Color = clWindowText
                      Font.Height = -15
                      Font.Name = 'Tahoma'
                      Font.Style = [fsBold]
                      ParentFont = False
                      TabOrder = 1
                      OnClick = Button11Click
                    end
                  end
                  object Panel23: TPanel
                    Left = 0
                    Top = 0
                    Width = 475
                    Height = 28
                    Align = alTop
                    BevelKind = bkFlat
                    BevelOuter = bvNone
                    Color = 12615680
                    ParentBackground = False
                    TabOrder = 1
                    object LbIngrediente: TLabel
                      Left = 0
                      Top = 0
                      Width = 471
                      Height = 24
                      Align = alClient
                      Alignment = taCenter
                      Caption = 'Ingredientes'
                      Color = 12615680
                      Font.Charset = DEFAULT_CHARSET
                      Font.Color = clWhite
                      Font.Height = -17
                      Font.Name = 'Tahoma'
                      Font.Style = [fsBold]
                      ParentColor = False
                      ParentFont = False
                      ExplicitWidth = 110
                      ExplicitHeight = 21
                    end
                  end
                  object DBGrid3: TDBGrid
                    Left = 0
                    Top = 28
                    Width = 475
                    Height = 234
                    Align = alClient
                    Color = 13303807
                    DataSource = DmDados.dfIngrediente
                    DrawingStyle = gdsGradient
                    Font.Charset = DEFAULT_CHARSET
                    Font.Color = clWindowText
                    Font.Height = -16
                    Font.Name = 'Tahoma'
                    Font.Style = []
                    Options = [dgEditing, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgTitleHotTrack]
                    ParentFont = False
                    TabOrder = 2
                    TitleFont.Charset = DEFAULT_CHARSET
                    TitleFont.Color = clWindowText
                    TitleFont.Height = -11
                    TitleFont.Name = 'Tahoma'
                    TitleFont.Style = []
                    OnDblClick = DBGrid3DblClick
                    OnExit = DBGrid3Exit
                    OnKeyDown = DBGrid3KeyDown
                    OnKeyUp = DBGrid3KeyUp
                    Columns = <
                      item
                        Expanded = False
                        FieldName = 'pronome'
                        Font.Charset = DEFAULT_CHARSET
                        Font.Color = clWindowText
                        Font.Height = -15
                        Font.Name = 'Tahoma'
                        Font.Style = [fsBold]
                        ReadOnly = True
                        Title.Font.Charset = DEFAULT_CHARSET
                        Title.Font.Color = clWindowText
                        Title.Font.Height = -13
                        Title.Font.Name = 'Tahoma'
                        Title.Font.Style = []
                        Width = 335
                        Visible = True
                      end
                      item
                        Expanded = False
                        FieldName = 'tsiopcao'
                        Title.Caption = 'Quatidade'
                        Width = 100
                        Visible = True
                      end>
                  end
                end
                object TabAdicionais: TTabSheet
                  Caption = 'Adicionais'
                  ImageIndex = 1
                  OnShow = TabAdicionaisShow
                  ExplicitHeight = 386
                  object DBGridAdicionais: TDBGrid
                    Left = 0
                    Top = 82
                    Width = 475
                    Height = 180
                    Align = alClient
                    Color = 13303807
                    DataSource = DmDados.dAdc
                    DrawingStyle = gdsGradient
                    Font.Charset = DEFAULT_CHARSET
                    Font.Color = clWindowText
                    Font.Height = -16
                    Font.Name = 'Tahoma'
                    Font.Style = []
                    Options = [dgEditing, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgConfirmDelete, dgCancelOnExit, dgTitleHotTrack]
                    ParentFont = False
                    TabOrder = 0
                    TitleFont.Charset = DEFAULT_CHARSET
                    TitleFont.Color = clWindowText
                    TitleFont.Height = -11
                    TitleFont.Name = 'Tahoma'
                    TitleFont.Style = []
                    OnDblClick = DBGridAdicionaisDblClick
                    OnKeyDown = DBGridAdicionaisKeyDown
                    Columns = <
                      item
                        Expanded = False
                        FieldName = 'pronome'
                        Font.Charset = DEFAULT_CHARSET
                        Font.Color = clWindowText
                        Font.Height = -15
                        Font.Name = 'Tahoma'
                        Font.Style = [fsBold]
                        Title.Font.Charset = DEFAULT_CHARSET
                        Title.Font.Color = clWindowText
                        Title.Font.Height = -13
                        Title.Font.Name = 'Tahoma'
                        Title.Font.Style = []
                        Width = 300
                        Visible = True
                      end
                      item
                        Expanded = False
                        FieldName = 'tsiopcao'
                        Width = 120
                        Visible = True
                      end>
                  end
                  object Panel18: TPanel
                    Left = 0
                    Top = 262
                    Width = 475
                    Height = 36
                    Align = alBottom
                    BevelOuter = bvNone
                    Font.Charset = DEFAULT_CHARSET
                    Font.Color = clWindowText
                    Font.Height = -15
                    Font.Name = 'Tahoma'
                    Font.Style = [fsBold]
                    ParentBackground = False
                    ParentFont = False
                    TabOrder = 1
                    ExplicitTop = 350
                    object Panel19: TPanel
                      Left = 214
                      Top = 0
                      Width = 261
                      Height = 36
                      Align = alRight
                      BevelOuter = bvNone
                      TabOrder = 0
                      object Button3: TButton
                        Left = 12
                        Top = 4
                        Width = 120
                        Height = 31
                        Caption = 'Selecionar'
                        TabOrder = 0
                        OnClick = Button3Click
                      end
                      object Button4: TButton
                        Left = 134
                        Top = 4
                        Width = 120
                        Height = 31
                        Caption = 'Cancelar'
                        TabOrder = 1
                        OnClick = Button4Click
                      end
                    end
                  end
                  object Panel20: TPanel
                    Left = 0
                    Top = 28
                    Width = 475
                    Height = 54
                    Align = alTop
                    BevelOuter = bvNone
                    ParentBackground = False
                    TabOrder = 2
                    object Label7: TLabel
                      Left = 17
                      Top = -2
                      Width = 50
                      Height = 21
                      Caption = 'Buscar'
                      Font.Charset = DEFAULT_CHARSET
                      Font.Color = clWindowText
                      Font.Height = -17
                      Font.Name = 'Tahoma'
                      Font.Style = []
                      ParentFont = False
                    end
                    object EdtBuscaItem: TEdit
                      Left = 16
                      Top = 18
                      Width = 401
                      Height = 27
                      Font.Charset = DEFAULT_CHARSET
                      Font.Color = clWindowText
                      Font.Height = -16
                      Font.Name = 'Tahoma'
                      Font.Style = []
                      ParentFont = False
                      TabOrder = 0
                      OnChange = EdtBuscaItemChange
                      OnEnter = EdtBuscaItemEnter
                      OnKeyDown = EdtBuscaItemKeyDown
                    end
                  end
                  object Panel24: TPanel
                    Left = 0
                    Top = 0
                    Width = 475
                    Height = 28
                    Align = alTop
                    BevelKind = bkFlat
                    BevelOuter = bvNone
                    Color = 12615680
                    ParentBackground = False
                    TabOrder = 3
                    object LbAdicionais: TLabel
                      Left = 0
                      Top = 0
                      Width = 471
                      Height = 24
                      Align = alClient
                      Alignment = taCenter
                      Caption = 'Ingredientes Adicionais'
                      Color = 12615680
                      Font.Charset = DEFAULT_CHARSET
                      Font.Color = clWhite
                      Font.Height = -17
                      Font.Name = 'Tahoma'
                      Font.Style = [fsBold]
                      ParentColor = False
                      ParentFont = False
                      ExplicitWidth = 203
                      ExplicitHeight = 21
                    end
                  end
                end
              end
            end
          end
          object Panel4: TPanel
            Left = 0
            Top = 506
            Width = 483
            Height = 126
            Align = alBottom
            BevelOuter = bvNone
            ParentBackground = False
            TabOrder = 1
            object Panel9: TPanel
              Left = 234
              Top = 0
              Width = 249
              Height = 56
              Align = alRight
              BevelOuter = bvNone
              TabOrder = 0
              ExplicitHeight = 38
              object Button2: TButton
                Left = 0
                Top = 6
                Width = 120
                Height = 31
                Caption = 'Incluir'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clWindowText
                Font.Height = -15
                Font.Name = 'Tahoma'
                Font.Style = [fsBold]
                ParentFont = False
                TabOrder = 0
                OnClick = Button2Click
              end
              object Button10: TButton
                Left = 126
                Top = 4
                Width = 120
                Height = 31
                Caption = 'Cancelar'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clWindowText
                Font.Height = -15
                Font.Name = 'Tahoma'
                Font.Style = [fsBold]
                ParentFont = False
                TabOrder = 1
                OnClick = Button10Click
              end
            end
            object DBGrid2: TDBGrid
              Left = 0
              Top = 56
              Width = 483
              Height = 70
              Align = alBottom
              DataSource = DataSource1
              TabOrder = 1
              TitleFont.Charset = DEFAULT_CHARSET
              TitleFont.Color = clWindowText
              TitleFont.Height = -11
              TitleFont.Name = 'Tahoma'
              TitleFont.Style = []
            end
          end
          object Panel15: TPanel
            Left = 0
            Top = 0
            Width = 483
            Height = 7
            Align = alTop
            BevelOuter = bvNone
            ParentBackground = False
            TabOrder = 2
          end
          object PnSabores: TPanel
            Left = 0
            Top = 393
            Width = 483
            Height = 113
            Align = alBottom
            BevelKind = bkFlat
            BevelOuter = bvNone
            Color = 12615680
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -15
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentBackground = False
            ParentFont = False
            TabOrder = 3
            ExplicitTop = 481
            object Label5: TLabel
              Left = 144
              Top = 55
              Width = 29
              Height = 18
              Caption = 'Und'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWhite
              Font.Height = -15
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object Label6: TLabel
              Left = 237
              Top = 55
              Width = 40
              Height = 18
              Caption = 'Valor'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWhite
              Font.Height = -15
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object Label4: TLabel
              Left = 379
              Top = 55
              Width = 36
              Height = 18
              Caption = 'Qtde'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWhite
              Font.Height = -15
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object Label3: TLabel
              Left = 4
              Top = 0
              Width = 161
              Height = 23
              Caption = 'Dados adicion'#225'is'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWhite
              Font.Height = -19
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object Label9: TLabel
              Left = 231
              Top = 2
              Width = 45
              Height = 18
              Caption = 'Copos'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWhite
              Font.Height = -15
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object Label10: TLabel
              Left = 354
              Top = 2
              Width = 49
              Height = 18
              Caption = 'Pratos'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWhite
              Font.Height = -15
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object Label8: TLabel
              Left = -2
              Top = 32
              Width = 480
              Height = 23
              Caption = '________________________________________'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWhite
              Font.Height = -19
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object DBLookupComboBox1: TDBLookupComboBox
              Left = 142
              Top = 75
              Width = 64
              Height = 26
              Ctl3D = True
              DataField = 'puncodigo'
              DataSource = DmDados.dstito
              KeyField = 'puncodigo'
              ListField = 'unisimbolo'
              ListSource = DmDados.dfpun
              ParentCtl3D = False
              ReadOnly = True
              TabOrder = 0
            end
            object DBEdit2: TDBEdit
              Left = 220
              Top = 75
              Width = 107
              Height = 26
              Ctl3D = True
              DataField = 'punprecoav'
              DataSource = DmDados.dfpun
              ParentCtl3D = False
              ReadOnly = True
              TabOrder = 1
            end
            object Button6: TButton
              Left = 337
              Top = 73
              Width = 40
              Height = 30
              Caption = '-'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -19
              Font.Name = 'Tahoma'
              Font.Style = []
              ParentFont = False
              TabOrder = 2
              OnClick = Button6Click
            end
            object EdtQtde: TDBEdit
              Left = 379
              Top = 75
              Width = 50
              Height = 26
              Ctl3D = True
              DataField = 'qtde'
              DataSource = DmDados.dstito
              MaxLength = 3
              ParentCtl3D = False
              TabOrder = 3
              OnExit = EdtQtdeExit
              OnKeyPress = EdtQtdeKeyPress
            end
            object Button5: TButton
              Left = 431
              Top = 73
              Width = 40
              Height = 30
              Caption = '+'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -19
              Font.Name = 'Tahoma'
              Font.Style = []
              ParentFont = False
              TabOrder = 4
              OnClick = Button5Click
            end
            object EdtCopos: TDBEdit
              Left = 229
              Top = 22
              Width = 107
              Height = 26
              Ctl3D = True
              DataField = 'copos'
              DataSource = DmDados.dstito
              MaxLength = 2
              ParentCtl3D = False
              TabOrder = 5
              OnClick = EdtCoposEnter
              OnEnter = EdtCoposEnter
              OnExit = EdtCoposExit
            end
            object EdtPratos: TDBEdit
              Left = 352
              Top = 21
              Width = 107
              Height = 26
              Ctl3D = True
              DataField = 'pratos'
              DataSource = DmDados.dstito
              MaxLength = 2
              ParentCtl3D = False
              TabOrder = 6
              OnClick = EdtPratosEnter
              OnEnter = EdtPratosEnter
              OnExit = EdtPratosExit
            end
          end
          object Panel12: TPanel
            Left = 0
            Top = 333
            Width = 483
            Height = 60
            Align = alBottom
            BevelKind = bkFlat
            BevelOuter = bvNone
            Color = 12615680
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -15
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentBackground = False
            ParentFont = False
            TabOrder = 4
            ExplicitTop = 421
            object Label13: TLabel
              Left = 5
              Top = 4
              Width = 134
              Height = 18
              Caption = 'Observa'#231#227'o Geral'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWhite
              Font.Height = -15
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object DBMemo1: TDBMemo
              Left = 6
              Top = 24
              Width = 469
              Height = 23
              Ctl3D = True
              DataField = 'obs'
              DataSource = DmDados.dstito
              ParentCtl3D = False
              TabOrder = 0
            end
          end
        end
        object Panel5: TPanel
          Left = 444
          Top = 0
          Width = 55
          Height = 632
          Align = alLeft
          BevelOuter = bvNone
          ParentBackground = False
          TabOrder = 3
          object BtnIncSabores: TBitBtn
            Left = 4
            Top = 124
            Width = 48
            Height = 42
            Glyph.Data = {
              
              4D2B984D2B994E2B9B4F2C9C4F2C9C4F2C9E512D9F512D9F512D}
            TabOrder = 0
            OnClick = BtnIncSaboresClick
          end
          object BtnExcSabores: TBitBtn
            Left = 4
            Top = 169
            Width = 48
            Height = 42
            Glyph.Data = {
              
              43258142257F41247F41247D3F237C3F237B3F237B3F23000000}
            TabOrder = 1
            OnClick = BtnExcSaboresClick
          end
        end
      end
    end
  end
  object dvtpro: TUniDataSource
    DataSet = DmDados.vtpro
    OnDataChange = dvtproDataChange
    Left = 600
    Top = 504
  end
  object DataSource1: TDataSource
    DataSet = DmDados.vIngrediente
    Left = 835
    Top = 390
  end
end
