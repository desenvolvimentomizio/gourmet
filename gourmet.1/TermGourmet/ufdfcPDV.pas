...
procedure TfdfcPDV.bcancelaClick(Sender: TObject);
begin
...
procedure TfdfcPDV.AoConfirmar;
begin
...
procedure TfdfcPDV.bconfirmaClick(Sender: TObject);
var
  i: Integer;
begin
    Drfi.DataSet.First;
...
procedure TfdfcPDV.GerarParcelas(vQtdParcelas: Integer; vTotalParcelas: Double);
Var
  i, o: Integer;
  VParcela: Double;
  SParcela: string;
...
procedure TfdfcPDV.bvalidarClick(Sender: TObject);
Var
  i, o: Integer;
  VParcela: Double;
  SParcela: string;
  ...
  Begin
    ...
    begin
      ...
      begin
        ...
        try
          rfirfivencimento.AsFloat := strtodate(FormatFloat('00', vlInterDias) + '/' + FormatFloat('00', myMonth) + '/' + FormatFloat('0000',
            myYear));
        except
          try
...
procedure TfdfcPDV.DBGListaColEnter(Sender: TObject);
begin
...
procedure TfdfcPDV.DBGListaDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
...
procedure TfdfcPDV.DBGListaKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TfdfcPDV.DBGListaKeyPress(Sender: TObject; var Key: Char);
Var
...
procedure TfdfcPDV.DBGListaKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  If Key = VK_DOWN Then
  Begin
    If rfi.State = dsInsert Then
...
procedure TfdfcPDV.DrfiStateChange(Sender: TObject);
begin
  if Drfi.DataSet.State = dsInsert then
    Drfi.DataSet.Cancel;
end;

procedure TfdfcPDV.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  { bloquer o ctrl+f4 }
...
procedure TfdfcPDV.FormKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if (((Shift = [ssAlt]) And (Key = VK_F4))) then
    Abort;
...
procedure TfdfcPDV.FormShow(Sender: TObject);
begin

  fdfcPDV.qtd.Text := vpParcelas;

  if fdfcPDV.qtd.Text <> '' then
  begin
    bvalidar.Click;
...
function TfdfcPDV.RecalculaTotais: Boolean;
var
...
function TfdfcPDV.ValidaTotais: Boolean;
var
  VPodeValor: Boolean;
  vTotalParcelas: Double;
  VDiferenca: Double;
...
function TfdfcPDV.ValidaVencimento: Boolean;
Var
