...
type
  TipoAutorizacao = (taDesconto, taAjusteBaixa, taAction, taActionAten, taLiberaCred, taVoltaColaborador, taTrocaFinalizador);
  ...
  Tfliberacao = class(TForm)
    ...
    LHistorico: TLabel;
    MHistorico: TMemo;
    aacaachistorico: TStringField;
    aacaacnometabela: TStringField;
    aacaacchaveregistro: TStringField;
  ...
  private
    ...
    procedure RegistraAutorizacao(Solicitante, Autorizado: String; vtabela: string = ''; vregistro: string = '');
...
begin
  try
    ...
    fliberacao.Height := 365;
    fliberacao.Width := 305;
    fliberacao.MHistorico.TabStop := false;
    ...
    if not(fliberacao.vpSolicitante = '1') or (vForcaLogin) then
      ...
      else if Pos('TrocaFinalizador', vmotivo) > 0 then
      begin
        fliberacao.vpTipoAutorizacao := taTrocaFinalizador;
...
begin
  try
    ...
    fliberacao.vpTabela := vtabela;
    fliberacao.vpRegistro := vregistro;
    ...
    if not(fliberacao.vpSolicitante = '1') or (vForcaLogin) then
      ...
      else if (Pos('Registro', vmotivo) > 0) and (pos('entregas.exe',lowercase(application.ExeName))=0) then
      begin
        fliberacao.LHistorico.Visible := true;
        fliberacao.MHistorico.Visible := true;
        fliberacao.Height := 365;
        fliberacao.MHistorico.TabStop := true;
        fliberacao.Repaint;
...
Var
  ...
  vAutorizado: Boolean;
Begin
  if fliberacao.MHistorico.Visible then
  begin
    if fliberacao.MHistorico.TabStop = true then
    begin
      if fliberacao.MHistorico.Visible then
      begin
        if fliberacao.MHistorico.text = '' then
        begin
          ShowMessage('Por favor informar o histórico!');
          fliberacao.MHistorico.SetFocus;
          exit;
        end;
      end;
    end;
...
procedure Tfliberacao.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if key=vk_f4 then
    abort;
...
procedure Tfliberacao.RegistraAutorizacao(Solicitante, Autorizado: String; vtabela: string = ''; vregistro: string = '');
begin
  cau.Open;
  ...
  case vpTipoAutorizacao of
    ...
    taAction:
      begin
        ...
        aacaachistorico.AsString := MHistorico.text;
        aacaacnometabela.AsString := vpTabela;
        aacaacchaveregistro.AsString := vpRegistro;
        aac.Post;
      end;
    taActionAten:
      begin
        ...
        aacactcodigo.AsString := vpActCodigo;
        aacaachistorico.AsString := MHistorico.text;
        aacaacnometabela.AsString := 'ORC';
        aacaacchaveregistro.AsString := vpOrcChave;
        aac.Post;
