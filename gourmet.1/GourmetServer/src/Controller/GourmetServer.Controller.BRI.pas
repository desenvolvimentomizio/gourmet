...
begin
  ...
  vlBrdCodigo := INTTOSTR(BuscaCodigoPROpunCodigo(vlBrdCodigo));

  vlBrdCodigo := BuscaCodigoBRDproCodigo(vlBrdCodigo);

  if (vlBrdCodigo = '') and (vItemBorda.getvalue('brdcodigo', '') <> '') then
  begin
    vlBrdCodigo := vItemBorda.getvalue('brdcodigo', '');
