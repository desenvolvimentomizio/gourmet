...
Function ManutencaoUNINome(vNome: string; vOrigem:Integer): Integer;

function BuscaNomeUNICodigo(vGrpCodigo: Integer): String;
function BuscaSimboloUNICodigo(vGrpCodigo: Integer): String;

function BuscaUNINome(vNome: string; vOrigem:Integer): Integer;
...
begin
  FDAO := TDAOGeneric<TUNI>.New;
  FDAO.dao.sql.Fields('unicodigo, unisimbolo,  uninome,  tuncodigo,  stgcodigo,  sipcodigo, uninomeaiq, uninomenuc, uninomeifood').&End.Find;
...
begin
  FDAO := TDAOGeneric<TUNI>.New;
  FDAO.dao.sql.Fields('unicodigo, unisimbolo,  uninome,  tuncodigo,  stgcodigo,  sipcodigo, uninomeaiq, uninomenuc, uninomeifood')
...
begin
  ...
  if vOrigem=8 then // aiqfome
  begin
  FDAO.dao.sql.Where('lower(uninomeaiq)=' + QuotedStr(lowercase(vNome))).&End.Find;
  end
  else
  begin
...
begin
  ...
  vlUNIcodigo := BuscaUNINome(vNome,vOrigem);
  ...
  begin
    ...
    vUnidade.AddPair('uninomeaiq', vNome);
    vUnidade.AddPair('uninomenuc', vNome);
    vUnidade.AddPair('uninomeifood', vNome);
...
begin
  ...
  FUNI.uninomeaiq := vGrupo.getvalue('uninomeaiq', '');
  FUNI.uninomenuc := vGrupo.getvalue('uninomenuc', '');
  FUNI.uninomeifood := vGrupo.getvalue('uninomeifood', '');
