...
begin
  ...
  begin
    ...
    vImm.AddPair('immnumepedido', vItensPedido[i].getvalue('immnumepedido', ''));
...
Function RegistraTaxaEntrega(vBaiIdentificacao: String; vValorTaxa: String; vOrcChave: Integer; vInclusao: string; vNumeroPedido: String;
...
begin
  ...
  begin
    ...
    vItemTaxa.AddPair('immnumepedido', vNumeroPedido);
...
Function RegistraTaxaAIQFome(vBaiIdentificacao: String; vValorTaxa: String; vOrcChave: Integer; vInclusao: string; vNumeroPedido: String;
  vItemTaxa: TJsonObject): TJsonObject;
var
...
begin
  ...
  begin
    ...
    vlItoChave := v1ManutencaoITO(vItemTaxa);
    if IsConsole then
    WRITELN('Rwegistro taxa serviço: '+vlItoChave.ToString);
    vItemTaxa.RemovePair('itochave');
    vItemTaxa.AddPair('itochave', vlItoChave.ToString);
    vlItensTaxa := TJsonarray.Create;
    vlItensTaxa.Add(vItemTaxa);
...
begin
  ...
  try
      ...
      vlEtdOrigem := BuscaCodigoETD(vletftelefone, vletfcelular, vleteemail, vletdidentificacao);
...
begin
  ...
  // if trim(uppercase(SituacaoCozinha)) <> 'OPEN' then
  // begin
  // exit;
  // end;
  ...
  vlNumeroPedido := '0';
  vlNumeroPedido := vPedido.getvalue('id', '');
  vlNumeroPedido := v1BuscaPedidoNumeroOrigem(vlNumeroPedido, vOrigem);
  ...
  vlOrcchave := 0;
  vlOrcchave := v1BuscaPedidoNumeroOrigem(vlNumeroPedido, 8).ToInteger;

  vlMeschave := 0;
  vlMeschave := v1BuscaVendaNumeroPedidoAIQ(vlNumeroPedido, 8);
 ...
 // ManutencaoETFFIXO(vletdcodigo, vlCliente);
  ...
  begin
    ...
    vlPedido.AddPair('orcdescontoav', StringReplace(vlVlrDescontoTotal, '.', ',', [rfReplaceAll, rfIgnoreCase]));
    vlPedido.AddPair('orcdescontoap', StringReplace(vlVlrDescontoTotal, '.', ',', [rfReplaceAll, rfIgnoreCase]));
  end
  else
  begin
  ...
  {vlorcgeral := floattostr(strtofloat(vlservice_feetotal) + (strtofloat(vlorcgeral) - strtofloat(vlVlrDescontoTotal)) +
    strtofloat(StringReplace(vlpayment_method.getvalue('delivery_tax', ''), '.', ',', [])));}
  ...
  vlPedido.AddPair('orcnumeropedido', vPedido.getvalue('id', ''));

  vlPedido.AddPair('immnumepedido', NovoNumeroPedidoCFG(vlCznChave).ToString);
  ...
  begin
    ...
    if ((vlSbrCodigo <> 0) and (pos('PIZZA', uppercase(vlGrpIdentificacao)) > 0) and (vlorder_item_subitems.Count = 0)) or
      (((vlorder_item_subitems = nil)) and (vlSbrCodigo <> 0) or (vlSbrCodigo <> 0)) then
    begin
    ...
    begin
      ...
      begin
        ...
        if vladiconarnapizzatoda then
        begin
          if vlItemAdicionais <> nil then
          begin
            vlItemAdicionais.RemovePair('sbichave');
            vlItemAdicionais.AddPair('sbichave', vlSbiChave.ToString);
            ManutencaoISI(vlItemAdicionais);
  ...
  begin
    ...
    begin
      ...
      vlItem := RegistraTaxaEntrega(uppercase(vlAdress.getvalue('neighborhood_name', '')), vlpayment_method.getvalue('delivery_tax', ''), vlOrcchave,
        vlDataAbert + ' ' + vlHoraAbert, vlNumeroPedido, vlItemTaxa);
      if vlItem <> nil then
      begin
        vlItens.Add(vlItem);
      end;
    ...
    begin
      ...
      vlItem := RegistraTaxaAIQFome('TAXA DE SERVIÇO AIQFOME', vlservice_feetotal, vlOrcchave, vlDataAbert + ' ' + vlHoraAbert, vlNumeroPedido,
        vlItemTaxaAIQFome);
      if vlItem <> nil then
      begin
        vlItens.Add(vlItem);
   ...
   // vlNumeroPedido:=BuscaNumeroUltimoPedido(vlCznChave).ToString;

   // AjustaNumeroPedido(vlNumeroPedido, vlOrcchave.ToString);
    ...
    AjustaSatusPedido(vlNumeroPedido, '1',vlOrcchave.ToString);
...
begin
  ...
  vlNumeroPedido := v1BuscaPedidoNumeroOrigem(vlJsonPedido, 8);
  ...
  begin
    ...
    begin
      Res.Send<TJsonObject>(FDAO.Update(vJSONObject));
    end;
