...
begin
  ...
  begin
    ...
    End
    Else If Application.MessageBox(PChar('Confirma a exclusão do registro selecionado?'), PChar('Excluir'), MB_TASKMODAL + MB_ICONQUESTION + MB_YESNO + MB_DEFBUTTON2) = IDYES Then
    Begin
      Self.itm.Delete;
...
procedure Tfcprsimples.mesemissaoExit(Sender: TObject);
begin
  inherited;
  ...
  if Self.mesemissao.Field.AsString = '' then
  begin
    {
      Tarefa: 2067
      Campo emissao nao pode ficar em branco.
    }
    ShowMessage('ATENÇÃO, Data de emissão deve ser informada.');
    mesemissao.SetFocus;
    Exit;
  end;
...
begin
  ...
  consulta.Close;
  consulta.SQL.Text := 'select meschave, mesregistro from mes where sdecodigo=' + QuotedStr('00') + ' and etdcodigo=' + Self.etdcodigo.Field.AsString + ' and ';
  ...
  begin
    Application.MessageBox(PChar('Já existe registro deste documento para este fornecedor.' + #13 + 'Dia: ' + datetostr(consulta.Fields[1].AsFloat) + ' Nr. Registro: ' +
      consulta.Fields[0].AsString), PChar('Duplicidade'), MB_ICONERROR + MB_OK);
...
begin
  ...
  If (Self.mesvalor.Field.AsFloat <= 0) Then
  Begin
...
begin
  ...
  inherited;
  Self.txtFiltro := 'tdfcodigo=' + QuotedStr('00') + ' or tdfcodigo=' + QuotedStr('01') + ' or tdfcodigo=' + QuotedStr('PE');
...
begin
  ...
  Begin
    if (Self.etdcodigo.Field.AsInteger = Self.cfgcfgetdempresa.AsInteger) or (tdfcodigo.Field.AsString = '00') then
      ...
      Self.txtFiltro := ' toeorigem = ''1'' and ttmcodigo = 1 and ttecodigo = 0';
  End
  Else if (Self.etdcodigo.Field.AsInteger = Self.cfgcfgetdempresa.AsInteger) or (tdfcodigo.Field.AsString = '00') then
    ...
    Self.txtFiltro := ' toeorigem = ''2'' and ttmcodigo = 1 and ttecodigo = 0';
...
begin
  ...
  begin
    ...
    { consulta.Close;
      ...
      if (consulta.Fields[0].AsFloat >= mestotal.Field.AsFloat) and (mestotal.Field.AsFloat > 0) and (mesvalor.Color <> $00FFD8B0) then
      begin
      ShowMessage('Atenção: O total dos itens já é igual ao valor informado para o Total da Compra!');
      end
      else
      begin }

    CriaFormulario(Tfitmcpr, '', Self.registromeschave.AsString);
...
begin
  ...
  { if mestotal.Field.AsFloat < vlTot then
    ShowMessage('Atenção: Valor de registro de itens maior que valor informado para o Total da Nota!'); }
...
begin
  ...
  if registro.State <> dsEdit then
    registro.Edit;
  { *
    (* Se NÃO for de Emissão Própria abandona rotina *)
    if (not vpIsEmissaoPropria) then
    Exit;
    * }
  qTotaisItens.Close;
...
begin
  if (registro.State = dsInsert) or (registro.State = dsEdit) then
    try
      ...
      Self.mestotal.Field.AsCurrency := (Self.mesvalor.Field.AsCurrency + Self.mesfrete.Field.AsCurrency + Self.mesicms.Field.AsCurrency + Self.messeguro.Field.AsCurrency +
        Self.mesipi.Field.AsCurrency + Self.mesoutras.Field.AsCurrency) - Self.mesdesconto.Field.AsCurrency;
    except
    end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (vpIsEmissaoPropria) { and (Self.tdfcodigo.Field.AsString <> 'PE') } then
      begin
      ...
      begin
        ...
        mesdesconto.Color := PEG_COR_VALORREQUERIDO;
...
Begin
  ...
  Try
    ...
    While Not itm.Eof Do
    Begin
      vtotit := vtotit + itmitmtotal.AsFloat { + itmitmicm.AsFloat } + itmitmicms.AsFloat + itmitmipi.AsFloat + itmitmfrete.AsFloat + itmitmoutras.AsFloat + itmitmseguro.AsFloat -
        itmitmdesconto.AsFloat;
      itm.Next;
    End;
  ...
  vdif := SimpleRoundTo(vtotit - Self.mestotal.Field.AsFloat, -2);

  if vdif <> 0 then
  Begin
    ShowMessage('Atenção: Total dos Produtos não está de acordo com o Total da Nota!');
  ...
  { * DANIEL 18/09
    No caso de pedido, se nao tiver o numero do documento vai colocar a chave do mes
    no local do numero.
    * }
  ...
  begin
    ...
    else if cfgcfgproinativsaldozero.AsInteger = 2 then
    begin
  ...
  Try
    ...
    Begin
      ...
      Begin
        ...
        itmitmdesconto.AsFloat := (Self.mesdesconto.Field.AsFloat * (Self.itmitmtotal.AsFloat / Self.mesvalor.Field.AsFloat)) / Self.itmitmquantidade.AsFloat;
...
begin
  ...
  begin
    ...
    consulta.Close;
    consulta.SQL.Text := 'select edrinscest, edritem from edr where etdcodigo=' + Self.etdcodigo.Field.AsString + ' and edritem=' + Self.registroedritem.AsString;
    consulta.Open;
...
begin
  try
    ...
    Begin
      vtoeatu := Self.toecodigo.Field.AsString;
      Self.bvalidar.Click;
...
begin
  ...
  Application.MessageBox(PChar('Data de entrada fora do período ativo do Sistema.' + #13 + #13 + 'Período ativo: ' + Self.spdspddatainicial.AsString + ' até ' +
    Self.spdspddatafinal.AsString + '.'), 'Período Inválido', MB_ICONWARNING + MB_OK);
