...
type
  Tfitmcpr = class(Tfrmbase)
    ...
    procedure cfocfopEnter(Sender: TObject);
...
procedure Tfitmcpr.bconfirmaEnter(Sender: TObject);
Var
  voutros: double;
  vtotal: double;
  vdivi: double;
...
procedure Tfitmcpr.cfocfopEnter(Sender: TObject);
...
begin
  ...
  begin
    cfocfop.Field.AsString := consulta.fields[0].AsString;
    cfocfop.ReadOnly := True;
    cfocfop.TabStop := False;
    cfocfop.Enabled := False;
  end
  else
  begin
    cfocfop.ReadOnly := False;
    cfocfop.TabStop := True;
    cfocfop.Enabled := True;
  end;
...
begin
  ...
  begin
    ...
    Begin
      ...
      self.registrocfocfopdestinacao.AsString := consulta.fields[0].AsString;
      cfocfop.Field.AsString := self.registrocfocfopdestinacao.AsString;
      cfocfop.ReadOnly := True;
      cfocfop.TabStop := False;
      // unicodigo.SetFocus;
    End;
  ...
  begin
    consulta.Close;
    consulta.SQL.Text := 'select tdfcodigo from mes where meschave=' + vChaveMestre;
    consulta.Open;
    if consulta.fields[0].AsString = 'PE' then
    begin
...
begin
  ...
  consulta.SQL.Text := 'Select ufs.ufssigla, edr.tedcodigo, etd.etdcodigo ' + ' From cdd Inner Join ufs On ufs.ufscodigo = cdd.ufscodigo Inner Join ' +
    ' edr On cdd.cddcodigo = edr.cddcodigo Inner Join  etd On etd.etdcodigo = edr.etdcodigo ' + ' Where edr.tedcodigo = 1 and etd.etdcodigo=' + vetdcodigo;
  consulta.Open;

  ufsetd := consulta.fields[0].AsString;
  ...
  If ufsetd = self.cfgufssigla.AsString Then
  Begin
    // dentro do estado
