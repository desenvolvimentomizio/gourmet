...
type
  Tfopm = class(TForm)
    ...
    mesccxchave: TIntegerField;
    mesmesnumeropedido: TStringField;
    orcorcnumeropedido: TStringField;
    itopcocodigo: TIntegerField;
    itoitopercentualcombo: TFloatField;
    itmpcocodigo: TIntegerField;
    itmitmpercentualcombo: TFloatField;
    itmclbatendente: TIntegerField;
    itmclbentregador: TIntegerField;
    itmoricodigo: TIntegerField;
    itoclbatendente: TIntegerField;
    mesmoccodigo: TIntegerField;
    mescznchave: TIntegerField;
    orccznchave: TIntegerField;
    itmitmvalofcpicm: TFloatField;
    itmitmbasefcpicm: TFloatField;
    itmitmpercfcpicm: TFloatField;
    itmitmmva2: TFloatField;
    itmitmacrescimoav: TFloatField;
    cfgcfgtributacaoimendes: TIntegerField;
    cfgcfgtoeinteproducaopropria: TIntegerField;
    taxa: tuniquery;
    mesbaicodigo: TIntegerField;
    orcbaicodigo: TIntegerField;
    mesmdacodigo: TIntegerField;
    orcmdacodigo: TIntegerField;
    itooricodigo: TIntegerField;
    mesmesretirabalcao: TIntegerField;
    orcorcpedidointegracao: TStringField;
    pro: TUniQuery;
    mesmespedidointegracao: TStringField;
...
Begin
  ...
  try
    ...
    fopm.zcone := Conexao;
    fopm.vpOrcchave := pChave;
    fopm.Acesso := pAcesso;
    fopm.VAFaturar := AFaturar;

    fopm.vmeschave := fopm.ProcessaVenda(fopm.vpOrcchave);
...
Begin
  ...
  try
    ...
    fopm.vmeschave := fopm.FinalizaVenda(fopm.vpOrcchave);
...
function CarregaOrc(AOwner: TComponent; Conexao: TUniConnection; pOrcChave: string; pOrcDelChave: string; pAcesso: TAcesso;
...
Begin
  ...
  try
    fopm.zcone := Conexao;
    fopm.vpOrcchave := pOrcChave;
    fopm.Acesso := pAcesso;
    fopm.VAFaturar := AFaturar;

    fopm.vpOrcchave := fopm.CarregaVenda(pOrcChave, pOrcDelChave);

    fopm.vretorno := fopm.vpOrcchave;
    Result := fopm.vretorno;
  finally
    fopm.Free;
  end;
...
begin
  ...
  Self.vmeschave := ProcessaVenda(Self.vpOrcchave);
  ...
  try
    ...
    try
      ...
      begin
        ...
        SalvarTextoEmArquivo('Mensagem: ' + e.Message + #13 + #13 + e.StackTrace, ExtractFilePath(Application.ExeName) + 'Erros\Atendimento\' +
          FormatDateTime('ddmmyyhhnnsszzz', Now()) + '.txt');
...
function Tfopm.AjustaTit(vtitcodigo: string; vetdcodigo: Integer; vtitnumero: String; vtitvalor: Double; vtitemissao: TDate; vtitvctoinicial: TDate;
...
begin
  ...
  try
    ...
    try
      ...
      orc.Close;
      orc.Params[0].AsString := Self.vpOrcchave;
      orc.Params[1].AsInteger := acesso.Filial;
      orc.Open;

      consulta.Close;
      consulta.Connection := zcone;
      consulta.SQL.Text := 'select cznchave, cznabertura, cznchave from czn where cznfechamento is null order by cznchave desc limit 1';
      consulta.Open;

      vpCznData := strtodate(copy(consulta.FieldByName('cznabertura').AsString, 1, 10));
      ...
      *)
      if (mormeschave.AsString <> '0') and (mormeschave.AsString <> '') then
      begin
        mes.Close;
        mes.FilterSQL := 'meschave=' + mormeschave.AsString;
        mes.Open;
        ...
        begin
          mes.Close;
          mes.FilterSQL := 'oricodigo<>3 and (oricodigo=' + orcoricodigo.AsString + ' and mespedidointegracao=' + orcorcpedidointegracao.AsString + ')';
          mes.Open;
      ...
      begin
        ...
        if (orcorcnumeropedido.AsString <> '0') and (orcorcnumeropedido.AsString <> '') and (orcorcpedidointegracao.asstring<>'') then
        begin
          mes.Close;
          mes.FilterSQL := 'oricodigo<>3 and (oricodigo=' + orcoricodigo.AsString + ' and mespedidointegracao=' + orcorcpedidointegracao.AsString + ')';
          mes.Open;
      ...
      mesmesemissao.AsFloat := vpCznData;
      mesmesregistro.AsFloat := vpCznData;
      ...
      ito.Close;
      ito.Params[0].AsString := Self.vpOrcchave;
...
//      mesmesprodutos.AsFloat := VTotProdutos + vlacrescimoav;
      ...
      mesmestotal.AsFloat := Self.orcorctotalav.AsFloat+mesmesdesconto.AsFloat;
      mesmesdesconto.AsFloat := Self.orcorcdescontoav.AsFloat;
      mesmesvalor.AsFloat := Self.orcorctotalav.AsFloat+mesmesdesconto.AsFloat;

      mestfpcodigo.AsInteger := tfpAVista;

      if (Orcmoccodigo.AsInteger = mocRetiraBalcao) or (Orcmoccodigo.AsInteger = mocMesa)  then
      begin
        mesmesretirabalcao.AsInteger:=1;
        mesrefcodigo.AsInteger:=0;
      end
      else
      begin
        mesmesretirabalcao.AsInteger:=0;
      ...
      taxa.Close;
      taxa.SQL.Text :=
        'SELECT IFNULL(SUM(itototalav),0) taxaentrega FROM ito,pro WHERE ito.procodigo=pro.procodigo and pro.tpocodigo=9 AND ito.orcchave=' +
        orcorcchave.AsString;
      taxa.Open;

      mesmesfrete.AsFloat := 0;
      mesmesfrete.AsFloat := taxa.FieldByName('taxaentrega').AsCurrency;




      // mesmesservicos.AsFloat := VTotServicos;
      mesmesservicos.AsFloat :=0;// taxa.FieldByName('taxaentrega').AsCurrency;
      ...
      mesccxchave.AsInteger := orcccxchave.AsInteger;
      mesoricodigo.AsString := orcoricodigo.AsString;
      mesmoccodigo.AsInteger := orcmoccodigo.AsInteger;
      mescznchave.AsInteger := vpCznChave;
      mesbaicodigo.AsString := orcbaicodigo.AsString;
      mesmdacodigo.AsString := orcmdacodigo.AsString;
      mesmesnumeropedido.AsString := orcorcnumeropedido.AsString;

    {  if mesoricodigo.AsInteger=OriDelivery then
        mesmesvalor.AsFloat := Self.mesmesvalor.AsFloat
      else}
      mesmestotal.AsFloat := mesmesvalor.AsFloat-mesmesdesconto.AsCurrency;
      mesmesvalor.AsFloat := Self.mesmesvalor.AsFloat-mesmesfrete.AsFloat;
      ...
      mesmesoutras.AsCurrency:=0;
      mesmespedidointegracao.AsString:=orcorcpedidointegracao.AsString;
      mes.post;
      ...
      mor.Open;
      mor.Append;
      mororcchave.AsString := Self.vpOrcchave;
      mormeschave.AsString := vmeschave;
      morflacodigo.AsInteger := Acesso.Filial;
      mor.post;
      mor.Close;

      if (orcorcnumeropedido.AsString = '') or (orcorcnumeropedido.AsString = '0') then
      begin
      ...
      *)
      ...
      itm.Close;
      itm.ParamByName('meschave').AsString := mesmeschave.AsString;
      ...
      begin
        ...
        taxa.Close;
        taxa.SQL.Text := 'select procodigo, tpocodigo from pro where procodigo=' + itoprocodigo.AsString;
        taxa.Open;

        if taxa.FieldByName('tpocodigo').AsInteger <> tpoServicos then
        begin

          itm.Close;
          itm.ParamByName('meschave').AsString := mesmeschave.AsString;
          itm.FilterSQL := 'itochave=' + itoitochave.AsString;
          itm.Open;
          ...
          // daniel 20241022 1041
          itmitmacrescimoav.AsCurrency := 0;
         ...
         {end;}
          ...
          begin
            ...
            consulta.Close;
            consulta.Connection := zcone;
            consulta.SQL.Text := 'select  cfocfop, tpocodigo  from pro where procodigo=' + Self.itoprocodigo.AsString;
            consulta.Open;

            vlcfocfop := consulta.FieldByName('cfocfop').AsString;
            vlTpocodigo := consulta.FieldByName('tpocodigo').AsString;
            ...
            consulta.Connection := zcone;
            consulta.SQL.Text :=
              'select  icmcodigo,  propercreducaobaseicm, cstcodigo,csicodigo,cspcodigo,csfcodigo,   propisaliquota,  procofinsaliquota, cfocfop, icmcodigofora,  proipialiquota, promva,propercreducaobaseicm  from pro where procodigo='
          ...
          Begin
            ...
            begin
              ...
              itmitmbicm.AsCurrency := itmitmbicm.AsCurrency -
                (itmitmbicm.AsCurrency * (consulta.FieldByName('propercreducaobaseicm').AsFloat / 100));
            end;
          ...
          begin
            if itotempoemproducao.AsString <> '' then
            begin
          ...
          begin
            ...
            if itotempoementrega.AsString <> '' then
            begin
          ...
          itmitmitem.AsInteger := itoitoitem.AsInteger;
          itmpcocodigo.AsInteger := itopcocodigo.AsInteger;
          itmitmpercentualcombo.AsFloat := itoitopercentualcombo.AsFloat;
          itmoricodigo.AsString := orcoricodigo.AsString;
          itmclbentregador.AsString := orcclbentregador.AsString;
          itmclbatendente.AsString := itoclbatendente.AsString;

          consulta.Close;
          consulta.SQL.Text := 'select toecodigo from toe where toecfopsaida=' + QuotedStr('5.933');
          consulta.Open;
          if consulta.IsEmpty then
          begin

            consulta.Close;
            consulta.SQL.Text := 'insert into toe value ( @chave, ' + QuotedStr('PRESTAÇÃO DE SERVIÇO') + ', ' + QuotedStr('PRESTAÇÃO DE SERVIÇO') +
              ',7,  ' + QuotedStr('5.933') + ', ';
            consulta.SQL.Add('  5,  1,  1,  0,  1,  0,  NULL,  NULL,  ' + QuotedStr('900') + ',  ' + QuotedStr('99') + ', ' + QuotedStr('99') +
            ...
            vlToeCodigoServico := consulta.FieldByName('toecodigo').AsInteger;
          end;
          ...
          if consulta.FieldByName('tpocodigo').AsInteger = tpoServicos then
          begin
            itmtoecodigo.AsInteger := vlToeCodigoServico;
            itmcfocfop.AsString := '5.933';
            itmcfocfopdestinacao.AsString := '5.933';
          end;

          vlPercentual := 0;
          consulta.Close;
          consulta.Connection := zcone;
          consulta.SQL.Text := 'SELECT propercfcp FROM pro WHERE procodigo=' + itmprocodigo.AsString;
          consulta.Open;

          if (consulta.FieldByName('propercfcp').AsString <> '') then
          begin
            if (consulta.FieldByName('propercfcp').AsString <> '0') then
            begin

              if (consulta.FieldByName('propercfcp').AsString <> '0.00') and (consulta.FieldByName('propercfcp').AsString <> '0,00') then
              begin

                vlPercentual := strtofloat(stringreplace(trim(consulta.FieldByName('propercfcp').AsString), '.', ',', []));

                itmitmbasefcpicm.AsCurrency := itmitmtotal.AsCurrency - itmitmdesconto.AsCurrency;
                itmitmpercfcpicm.AsFloat := vlPercentual;
                itmitmvalofcpicm.AsCurrency := (itmitmtotal.AsCurrency - itmitmdesconto.AsCurrency) * (vlPercentual / 100);
              end;
...
//          itmitmfrete.AsFloat := mesmesfrete.AsCurrency * (itmitmtotal.AsCurrency / (mesmestotal.AsCurrency + mesmesfrete.AsCurrency));
      ...
      mesmoccodigo.AsInteger := orcmoccodigo.AsInteger;
      mescznchave.AsInteger := vpCznChave;
      mesmestotal.AsCurrency := orcorctotalav.AsCurrency;
      ...
      // zcone.Commit;
      {
        ...
        saldo.Close;
        saldo.SQL.Text := 'CALL AjustaSitacaoProduto(' + itoprocodigo.AsString + ', IF((select prosaldo from pro where procodigo=' +
        itoprocodigo.AsString + ')=0,2,1) )';
        saldo.ExecSQL;

        saldo.Close;
        saldo.SQL.Text := 'CALL AjustaSitacaoProduto(' + itoprocodigo.AsString + ', IF((select prosaldodisp from pro where procodigo=' +
        itoprocodigo.AsString + ')=0,2,1) )';
        saldo.ExecSQL;
        ...
        saldo.Close;
        saldo.SQL.Text := 'select procodigo, prosaldo, prosaldodisp,proestoque from pro where procodigo=' + itoprocodigo.AsString +
        ' and unicodigo=' + itounicodigo.AsString;
        saldo.Open;
      ...
      begin
        VTotFrete := VTotFrete + itmitmfrete.AsCurrency;
        itm.Next;
      end;

      if VTotFrete <> 0 then
      begin
        if VTotFrete > mesmesfrete.AsCurrency then
        begin
          itm.edit;
          itmmeschave.AsString := Self.vmeschave;

          itmitmfrete.AsCurrency := itmitmfrete.AsCurrency - (VTotfrete - mesmesfrete.AsCurrency);
          itm.post;
        end
        else if VTotFrete < mesmesfrete.AsCurrency then
        begin
          itm.edit;
          itmmeschave.AsString := Self.vmeschave;
          itmitmfrete.AsCurrency := itmitmfrete.AsCurrency + (mesmesfrete.AsCurrency - VTotfrete);
          itm.post;
        end;
