...
begin
  ...
  if not((sbCartaoDebito.Down or sbCartaoCredito.Down or sbDinheiro.Down or sbConvenio.Down or sbAFaturar.Down or sbPIX.Down or sbOnLine.Down)  IN [True]) then
  begin
...
begin
  RemoveMarcacoes;
   if FprinciEnt.Autorizado(FprinciEnt.ActDescontoGeral,'LIBERAÇÃO DE PAGAMENTO ON-LINE') then
   begin
    AjustaSpeedButton(Sender);
   end;
end;
...
begin
   ShowMessage('voucher');
  if FprinciEnt.Autorizado(FprinciEnt.ActDescontoGeral,'LIBERAÇÃO DE VOUCHER') then
  begin
