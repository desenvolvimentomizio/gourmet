...
begin
  ...
  consulta.Close;
  consulta.SQL.Text := 'SELECT orcnumeropedido FROM orc WHERE cznchave=' + vpCznChave + ' AND orcnumeropedido<=4999  order by  CAST(orcnumeropedido AS UNSIGNED)   desc limit 1';
  consulta.Open;
  ...
  if consulta.fieldbyname('orcnumeropedido').AsString='' then
    vlNumeroPedido :=1
  else
    vlNumeroPedido :=consulta.fieldbyname('orcnumeropedido').asinteger;

  consulta.First;

  while not consulta.eof do
  begin
    if consulta.fieldbyname('orcnumeropedido').asinteger=vlNumeroPedido then
    begin
      consulta.next;
      vlNumeroPedido:=vlNumeroPedido+1;
    end
    else
    begin
      break;
  ...
  vlNumePedidoAtual := vlNumeroPedido.tostring;
  vpImmNumePedido :=  vlNumeroPedido.tostring;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        consulta.Close;
        consulta.SQL.Text :=
          ' insert into imm (immchave, itochave, immmodo, immhorapedido, cznchave, clbcodigo,tcicodigo ) values (@chave,  ' +
          inttostr(vlItoChave) + ',9,now(),' + vpCznChave + ',' + ACESSO.Usuario.ToString + ',' + vlTcicodigo +  ')';
        consulta.ExecSQL;
  ...
  try
   ...
   // CriaPedidoGourmet;
    ...
    begin
      ...
      consulta.Close;
      consulta.SQL.Text := 'update orc set orcnumeropedido=' + vpImmNumePedido + ' where orcchave=' + vpOrcChave;
      consulta.ExecSQL;
