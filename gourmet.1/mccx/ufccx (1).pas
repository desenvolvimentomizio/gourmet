    ...
    try
      ...
      begin
        ...
        begin
          ...
          Result := Registra(Application, zcone, Vchave, vpTrmCodigo, Vpri, vMulta, Vjur, Vdes, Acesso, IntToStr(tfdTesouraria), vPodeConvenio, vlTeclaFinalizador,
            vlValorFinalizador, False, StrToInt(vpCtaCodigo));
        end
        else
          Result := Registra(Application, zcone, Vchave, vpTrmCodigo, Vpri, vMulta, Vjur, Vdes, Acesso, IntToStr(tfdTesouraria), vPodeConvenio, 0, 0, False, StrToInt(vpCtaCodigo));
      End;
...
begin
  ...
  begin
    ...
    if vpLteChave <> '' then
    begin
      lte.SQL.Text := 'SELECT ltechave, tfdcodigo, ltedata, lteprincipal, ltemulta, ltejuros, ltedesconto, ltetotal, lteextenso, clbcodigo, ctacodigo, lteregistro ';
        ...
        begin
          ...
          begin
            ...
            { consulta.Close;
              ...
              begin
              if ccxccxinicio.AsDateTime > consulta.FieldByName('lteregistro').AsDateTime then
              begin
              ...
              ajulte.Edit;
              ajulte.FieldByName('lteregistro').AsDateTime := consulta.FieldByName('ccoregistro').AsDateTime;
 ...
 { if cfgcfgusagou.AsInteger = 1 then
  begin
    if Operacao = tocFechamentoCaixa then
    begin
      ajustacaixas.Close;
      ajustacaixas.Open;
      while not ajustacaixas.Eof do
      begin
        linhas.Close;
        linhas.SQL.Text := ajustacaixas.Fields[0].AsString;
        linhas.ExecSQL;
        ajustacaixas.Next;
      end;
    end;
...
procedure Tfccx.valorExit(Sender: TObject);
begin
  if valor.Field.IsNull then
