...
type
  Tfrapro = class(Tfrabase)
    ...
    uqtabelaproenvioimendes: TDateField;
    uqtabelaproretornoimendes: TDateField;
    ...
    procedure ActIMEStatusExecute(Sender: TObject);
  ...
  try
    ...
    begin
      ...
      try
        ...
        fProcessaSaneamento.uqtabela.IndexFieldNames := uqtabela.IndexFieldNames;
        if fProcessaSaneamento.ShowModal = mrok then
        begin
          actatualizar.Execute;
          uqtabela.Locate('procodigo', vlprocodigo, []);

          if (uqtabelaimecodigo.AsString <> '') and (uqtabelaimecodigo.AsString <> 'PENDENTE') then
          begin
...
begin
  ...
  a:=vlJsonProduto.ToString;

  if vlJsonProduto.ToString<>'[]' then
  begin
...
procedure Tfrapro.ActIMEStatusExecute(Sender: TObject);
var
  vlJsonResult: tJsonObject;
