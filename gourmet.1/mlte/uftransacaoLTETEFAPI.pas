...
type
  TftransacaoTEFAPI = class(TForm)
    ...
    procedure CompACBrTEFQuandoDetectarTransacaoPendente(
      RespostaTEF: TACBrTEFResp; const MsgErro: string);
    procedure CompACBrTEFQuandoEsperarOperacao(
      OperacaoAPI: TACBrTEFAPIOperacaoAPI; var Cancelar: Boolean);
    procedure CompACBrTEFQuandoExibirMensagem(const Mensagem: string;
    ...
    procedure CompACBrTEFQuandoFinalizarTransacao(RespostaTEF: TACBrTEFResp;
      AStatus: TACBrTEFStatusTransacao);
    procedure CompACBrTEFQuandoGravarLog(const ALogLine: string;
      var Tratado: Boolean);
    procedure CompACBrTEFQuandoPerguntarCampo(
      DefinicaoCampo: TACBrTEFAPIDefinicaoCampo; var Resposta: string;
      var Validado, Cancelado: Boolean);
    procedure CompACBrTEFQuandoPerguntarMenu(const Titulo: string;
...
begin
  ...
  begin
    RemoverPagamento(fOperacaoTEF.Rede,fOperacaoTEF.AutorizacaoRetorno);
  end;
...
begin
  ...
  begin
   ...
   { habilitar para homologação sitef e comentar trexo abaixo
  ...
  *)
  ...
  begin
    ...
    begin
      ...
      fOperacaoTEF.Bandeira := copy(fOperacaoTEF.Bandeira,1,pos(' ',fOperacaoTEF.Bandeira)-1);
      fOperacaoTEF.Bandeira := RetornaBdcCodigo(fOperacaoTEF.Bandeira );
