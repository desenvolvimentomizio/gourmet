...
begin
  ...
  try
    ...
    q.sql.Text := 'SELECT etd.etdapelido, etd.etddoc1  FROM   cfgmcfg,   etd ';
    q.sql.Add('WHERE etd.etdcodigo = cfgmcfg.cfgetdempresa AND cfgmcfg.cfgcodigo = 1 ');
    q.Open;

    vlCnpj := q.FieldByName('etddoc1').AsString;
    q.close;

    q.sql.Text:='SELECT COUNT(*) AS existe '+
                'FROM information_schema.tables '+
                'WHERE table_schema = DATABASE() '+
                'AND table_name   = '+QuotedStr('nrt');
    q.Open;
    ...
    begin
      ...
      q.sql.text:='CREATE TABLE IF NOT EXISTS nrt ( '+
                  'nrtcodigo varchar(100) NOT NULL, '+
                  'nrtcstibscbs varchar(5) NOT NULL,   '+
                  'nrtcfop_interno varchar(5) NOT NULL, '+
                  'crtcodigo int(11) NOT NULL, '+
                  'nrtidentificacao varchar(100) NOT NULL, '+
                  'nrticm_interno decimal(10, 3) NOT NULL, '+
                  'nrtcfop_externo varchar(5) NOT NULL, '+
                  'nrticm_externo decimal(10, 3) NOT NULL, '+
                  'nrtcombate_pobreza_aliquota decimal(10, 3) NOT NULL, '+
                  'nrtreducao_base_aliquota decimal(10, 3) NOT NULL, '+
                  'cstcodigo varchar(5) NOT NULL, '+
                  'csicodigo varchar(5) NOT NULL, '+
                  'cspcodigo varchar(5) NOT NULL, '+
                  'pis_aliquota decimal(10, 3) NOT NULL, '+
                  'csfcodigo varchar(5) NOT NULL, '+
                  'cofins_aliquota decimal(10, 3) NOT NULL, '+
                  'nrtclassificacao_is varchar(50) NOT NULL, '+
                  'nrtclassificacao_cpresumido varchar(50) NOT NULL, '+
                  'nrtibs_aliquota_estadual decimal(10, 3) NOT NULL, '+
                  'nrtibs_aliquota_municipal decimal(10, 3) NOT NULL DEFAULT 0.000, '+
                  'nrtdata_inicio date NOT NULL, '+
                  'nrtdata_fim date DEFAULT NULL, '+
                  'nrtcbs_aliquota decimal(10, 3) DEFAULT NULL, '+
                  'nrtreducao_cbs decimal(10, 3) DEFAULT NULL, '+
                  'nrtreducao_ibs decimal(10, 3) DEFAULT NULL, '+
                  'PRIMARY KEY (nrtcodigo)) ENGINE = INNODB '+
