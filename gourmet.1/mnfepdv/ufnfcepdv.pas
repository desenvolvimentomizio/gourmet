...
uses
  ...
  ACBrDANFCeFortesFr, ACBrDFeUtil, midaslib, ACBrDFeSSL, blcksock,
  System.JSON,
  REST.Types, ufaguardepdv;
...
uses
  ACBrUtil,   System.Variants,
  ACBrUtil.DateTime, uCloudComunicacaopdv, ufnfcejustificativapdv;
...
function Tfnfcepdv.qqCPF:string;
var
  a:string;
begin
...
function modulonfcepdv(AOwner: TComponent; conexao: TUniConnection; vMesChave: string; vFuncao: string; Acesso: TAcesso; vImprimir: Boolean;
...
begin
  try
    ...
    For i := 0 To fnfcepdv.ComponentCount - 1 Do
    begin
      If fnfcepdv.Components[i] Is TUniQuery Then
        (fnfcepdv.Components[i] As TUniQuery).Connection := conexao;
    ...
    vlHamdle := 0;
    vlHamdle := FindWindow(nil, pchar('MizioArquivos:' + SoNumeros(fnfcepdv.cfgetddoc1.AsString)));

    // verifica se houve erro na criação

    if vlHamdle = 0 then
    begin
      fnfcepdv.AtivaMizioArquivos;
    end;
    ...
    fnfcepdv.vPastaPrincipal := ExtractFilePath(application.ExeName);
    ...
    vpWSNormal := fnfcepdv.ConsultaServicoSEFAZNFCESilent;

    fnfcepdv.vpConsultouSEFAZ := vpWSNormal;
    ...
    begin
      fnfcepdv.rec.Connection := fnfcepdv.zcone;
      fnfcepdv.rec.Open;
    end;
    ...
    begin
      ...
      fnfcepdv.mes.Params[1].AsString := fnfcepdv.vpFlacodigo;
      fnfcepdv.mes.Open;

      if fnfcepdv.mes.RecordCount>0 then
      begin
        if fnfcepdv.mesetdcodigo.AsInteger=0 then
        begin

          fnfcepdv.consulta.Close;
          fnfcepdv.consulta.SQL.Text:='update mes set edritem=1 where mescahve='+fnfcepdv.vpMesChave;
          fnfcepdv.consulta.ExecSQL;
    ...
    case ansiIndexStr(vFuncao, ['EmiteNFCe', 'ImprimeNFCe', 'CancelaNFCe'  ]) of
      0:
      ...
      2:
        fnfcepdv.CancelaNFCe(vMesChave, fnfcepdv.vpFlacodigo);
    end;
...
function Tfnfcepdv.Geraxml(vChaveNFE: string; vFlaCodigo: string = '1'): Boolean;
begin
  try
    Result := False;
    ...
    { if not ValidaConsumidor(vChaveNFE, fnfcepdv.vpFlacodigo) then
...
function Tfnfcepdv.ValidaProdutos(vMesChave: string; vFlaCodigo: string): Boolean;
var
...
begin
  try
    try
      ...
      consulta.Connection := fnfcepdv.zcone;
      rec.Connection := fnfcepdv.zcone;
      mes.Connection := fnfcepdv.zcone;
      itm.Connection := fnfcepdv.zcone;
      etd.Connection := fnfcepdv.zcone;
      toe.Connection := fnfcepdv.zcone;
      enf.Connection := fnfcepdv.zcone;
      mev.Connection := fnfcepdv.zcone;
      cfg.Connection := fnfcepdv.zcone;
      trm.Connection := fnfcepdv.zcone;
      mic.Connection := fnfcepdv.zcone;
      oic.Connection := fnfcepdv.zcone;
      qDtl.Connection := fnfcepdv.zcone;

      NumeroNFCe.Connection := fnfcepdv.zcone;

      disponivel.Connection := fnfcepdv.zcone;
      limite.Connection := fnfcepdv.zcone;
      ncm.Connection := fnfcepdv.zcone;
      itmncm.Connection := fnfcepdv.zcone;
...
function Tfnfcepdv.ConsultaNFCe(vMesChave, vArqNFCe: String; vFlaCodigo: string = '1'; ASilent: Boolean = True): Boolean;
Var
  nProt: String;
  vnrnfe: String;
  vchNFe: String;
...
function Tfnfcepdv.ValidaNota(vACBrNFe: TACBrNFe; vMesChave: string; vFlaCodigo: string = '1'): Boolean;
var
  vlNumeroNFe: string;
  vErro: string;
  vMensagemErro: String;
...
function Tfnfcepdv.ConsultaServicoSEFAZNFCESilent: Boolean;
var
...
Begin
  try
    ...
    begin
      ...
      try
        faguardepdv:=tfaguardepdv.Create(self);
        faguardepdv.Show;
        faguardepdv.Label1.Caption:='Conectando SEFAZ - Aguarde ... ';
        application.ProcessMessages;
...
Function Tfnfcepdv.SalvarArquivoCloud(vTipo: string; vArquivo: string): Boolean;
var
  vlAno: string;
  vlMes: string;
...
procedure Tfnfcepdv.AtivaMizioArquivos;
var
  vlHamdle: thandle;
  vlarq: string;
begin
...
procedure Tfnfcepdv.ImprimirComprovantesTEF(Owner: TApplication; vConexao: TUniConnection; vTrmCodigo, vDirRelat, vImpressora: String; vTipoImpressao: Integer);
var
  vtTrm: TUniQuery;
...
function Tfnfcepdv.ImprimirViaTEF: Boolean;
var
  vDirRelat: String;
begin
  // try
...
Procedure Tfnfcepdv.CancelaNFCe(vMesChave: string; vFlaCodigo: string = '1');
Var
...
Begin
  ...
  try
    ...
    fnfcejustificativapdv := Tfnfcejustificativapdv.Create(Self);
    try
      fnfcejustificativapdv.textocorrecao.Lines.Text := vJustificativaCanc;

      if fnfcejustificativapdv.ShowModal = mrCancel then
        Exit;

      vJustificativaCanc := fnfcejustificativapdv.textocorrecao.Lines.Text;

    finally
      fnfcejustificativapdv.Free;
    end;
