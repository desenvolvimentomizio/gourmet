...
type
  TFormComunicacaopdv = class(TForm)
    IdWebDAVC: TIdWebDAV;
  ...
  private
    ...
    function EnviaFile(FilePath, DestPath: String): Boolean;
...
var
  FormComunicacaopdv: TFormComunicacaopdv;
...
procedure TFormComunicacaopdv.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;

procedure TFormComunicacaopdv.FormCreate(Sender: TObject);
begin
  try
    ...
    Showmessage('Falha de conexão com ' + vpServer)
  end;
end;

procedure TFormComunicacaopdv.FormDestroy(Sender: TObject);
...
function TFormComunicacaopdv.createFolder(FileName: String): Boolean;
begin
  try
...
function TFormComunicacaopdv.deleteFile(FileName: String): Boolean;
var
  vlserver: string;
begin
  try
    vlserver := vpServer + FileName;
...
function TFormComunicacaopdv.downloadFile(FilePath, DestPath: String): Boolean;
var
...
Function TFormComunicacaopdv.getFileListXML(Folder: String): TStringStream;
var
...
function TFormComunicacaopdv.getFileList(Folder: String): TStringStream;
var
  ResponseXML: TStringStream;
...
function TFormComunicacaopdv.EnviaFile(FilePath, DestPath: String): Boolean;
var
  aFile: TMemoryStream;
  ALockToken:String;
begin
 ...
 ALockToken:='';
    ...
    aFile.LoadFromFile(DestPath);

  //  IdWebDAVC.Get(vpServer + FilePath, aFile);
    IdWebDAVC.DAVPut(vpServer + DestPath + ExtractFileName(DestPath), aFile, ALockToken);
...
function TFormComunicacaopdv.uploadFile(SourcePath, DestPath: String; ALockToken: string): Boolean;
var
