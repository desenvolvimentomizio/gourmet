...
uses
  ...
  Spin, db, ACBrNFe, MaskUtils, ACBrBase, ACBrDFe, pcnConversao, Registry,
  pcnConversaoNFe, ACBrUtil, dateutils, Vcl.Mask, Vcl.Menus, IdBaseComponent,
...
type
  ...
  Tfrmxml = class(TForm)
    ...
    Inicia_some: TTimer;
    icon_barra: TTrayIcon;
    menu_icon: TPopupMenu;
    MostraDownloadXML1: TMenuItem;
    OcultarDownloadXML1: TMenuItem;
    N1: TMenuItem;
    FecharDownloadXML1: TMenuItem;
    N2: TMenuItem;
    controle_manifesto: TTimer;
    ...
    procedure Inicia_someTimer(Sender: TObject);
    procedure MostraDownloadXML1Click(Sender: TObject);
    procedure OcultarDownloadXML1Click(Sender: TObject);
    procedure FecharDownloadXML1Click(Sender: TObject);
    procedure N1Click(Sender: TObject);
    procedure controle_manifestoTimer(Sender: TObject);
  private
    FFileLog: String;
    procedure AbrirComWindows(Value: boolean);
    procedure SetCertificadoAtivo;
    procedure SetErrorList(AMsn: String);
    procedure ImprimeDanfe;
    procedure AtualizaPastaXMLRecebidos;
  public
...
begin
  ...
  begin
    sXmlPath := dtmxml.GetPathXMLNFe(dtmxml.xmlnfe.FieldByName('dhEmi').AsDateTime, dtmxml.xmlnfe.FieldByName('chNFe').AsString);

    if FileExists(sXmlPath) then
    begin
      // Gera Danfe em PDF
      sPdfPath := dtmxml.GetPathPDFNFe(dtmxml.xmlnfe.FieldByName('dhEmi').AsDateTime, dtmxml.xmlnfe.FieldByName('chNFe').AsString);
        ...
        begin
          ...
          try
            ...
            begin
              ...
              dtmxml.ACbrNFe1.DANFE.PathPDF := PathWithDelim(sPathPdf) + PathWithDelim(FormatDateTime('YYYYMM', dtmxml.xmlnfe.FieldByName('dhEmi').AsDateTime));
...
Procedure Tfrmxml.AtualizaPastaXMLRecebidos;
var
  sXmlPath: String;
  sNfePath: String;
begin
  ...
  dtmxml.xmlnfe.CommandText := ' select * from xmlnfe ' + ' where NFe = ''T'' ' + ' and dhEmi >= datetime(' + QuotedStr('2016-01-01') + ')' + ' and dhEmi <= datetime(' +
    QuotedStr('2030-12-31') + ')';
  ...
  begin
    ...
    sXmlPath := dtmxml.GetPathXMLNFe(dtmxml.xmlnfe.FieldByName('dhEmi').AsDateTime, dtmxml.xmlnfe.FieldByName('chNFe').AsString);
    sNfePath := extractfilepath(application.ExeName) + 'xml-recebidos\' + dtmxml.xmlnfe.FieldByName('chNFe').AsString + '-nfe.xml';
    copyfile(pchar(sXmlPath), pchar(sNfePath), False);
...
begin
  ...
  begin
    dtmxml.xmlnfe.CommandText := ' select * from xmlnfe ' + ' where NFe = ''T'' ' + ' and xNumeroNFe =  ' + QuotedStr(FormatFloat('000000000', edtNumeroNFe.Value));
  end
  else
  begin
    dtmxml.xmlnfe.CommandText := ' select * from xmlnfe ' + ' where NFe = ''T'' ' + ' and dhEmi >= datetime(' + QuotedStr(FormatDateTime('YYYY-MM-DD', edtDataInicial.DateTime)) +
...
procedure Tfrmxml.FecharDownloadXML1Click(Sender: TObject);
begin
  Close;
end;
...
begin
  try
    AbrirComWindows(False);
    AbrirComWindows(True);
  except

  end;
  FFileLog :=extractfilepath(application.ExeName)+ 'Log\log-' + FormatDateTime('DDMMYYYYHHMM', Date) + '.txt';
...
begin
  try
    if (dtmxml.ACbrNFe1.Configuracoes.Certificados.NumeroSerie <> '') or (dtmxml.ACbrNFe1.Configuracoes.Certificados.ArquivoPFX <> '') then
    begin
...
procedure Tfrmxml.controle_manifestoTimer(Sender: TObject);
begin
  // Consultar SEFAZ
  ...
  try
    ...
    icon_barra.BalloonTitle := 'DownloadXML Notifica';
    icon_barra.BalloonHint := 'Erro ao efetuar a Consulta ao SEFAZ';
    icon_barra.ShowBalloonHint;
    exit
  end;
  // Atualizar Lista
  try
    ...
    icon_barra.BalloonTitle := 'DownloadXML Notifica';
    icon_barra.BalloonHint := 'Erro ao efetuar a Atualização da  Lista';
    icon_barra.ShowBalloonHint;
    exit
  end;
  ...
  try
    ...
    icon_barra.BalloonTitle := 'DownloadXML Notifica';
    icon_barra.BalloonHint := 'Erro ao efetuar a Execução da  Distribuição';
    icon_barra.ShowBalloonHint;
    exit
  end;

  // Executar Envia para Pasta XML-RECEBIDOS
  try
    AtualizaPastaXMLRecebidos;
    application.ProcessMessages;
  except
    icon_barra.BalloonTitle := 'DownloadXML Notifica';
    icon_barra.BalloonHint := 'Erro ao cópias de XML-RECEBIDOS';
    icon_barra.ShowBalloonHint;
    exit
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        if (dtmxml.xmldfe.FieldByName('DFe').AsString = 'F') and (dtmxml.xmldfe.FieldByName('NFe').AsString = 'F') then
        begin
          ...
          try
            ...
            begin
              SetErrorList(Format('Erro ao manifestar a nota: %s, %d - %s', [dtmxml.xmldfe.FieldByName('chNFe').AsString, dtmxml.ACbrNFe1.WebServices.EnvEvento.cStat,
...
var
  ...
  sPathPdf: String;
  sFileNew: string;
  vlArquivoLocal: string;
  vlArquivoRecebido: string;
...
begin
  ...
  try
    ...
    begin
      ...
      sXmlPath := dtmxml.GetPathXMLNFe(dtmxml.xmldfe.FieldByName('dhEmi').AsDateTime, dtmxml.xmldfe.FieldByName('chNFe').AsString);
      ...
      // Gera Danfe em PDF
      sPdfPath := dtmxml.GetPathPDFNFe(dtmxml.xmldfe.FieldByName('dhEmi').AsDateTime, dtmxml.xmldfe.FieldByName('chNFe').AsString);
      ...
      begin
        ...
        begin
          ...
          try
            if dtmxml.ACbrNFe1.NotasFiscais.LoadFromFile(sXmlPath) then
            begin
              dtmxml.ACbrNFe1.DANFE.PathPDF := PathWithDelim(sPathPdf) + PathWithDelim(FormatDateTime('YYYYMM', dtmxml.xmldfe.FieldByName('dhEmi').AsDateTime));
...
begin
  try
    ...
    dtmxml.xmlnfe.Close;
    try
      dtmxml.ACbrNFe1.DistribuicaoDFe(dtmxml.ACbrNFe1.Configuracoes.WebServices.UFCodigo, dtmxml.ACbrNFe1.SSL.CertCNPJ, dtmxml.XmlCfg.FieldByName('ultNSU').AsString, '');
      ListBoxError.Items.Add(Datetimetostr(now())+ ' -  Retorno da consulta no servidor do Ambiente Nacional: ' + Inttostr(dtmxml.ACbrNFe1.WebServices.DistribuicaoDFe.retDistDFeInt.cStat));
      application.ProcessMessages;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                dtmxml.Conn.ExecSQL('select idDFe from xmlnfe where chNFe IN(' + QuotedStr(DocZipItem.resNFe.chNFe) + ')', TmpDataset);
                try
                  // Se não foi, adiciona.
      ...
      else if dtmxml.ACbrNFe1.WebServices.DistribuicaoDFe.retDistDFeInt.cStat = 137 then
      begin
...
begin
  if application.MessageBox('Deseja realmente apagar todas as notas fiscais?', 'Apagar', MB_ICONQUESTION + MB_YESNO + MB_DEFBUTTON2) = ID_YES then
  begin
    dtmxml.Conn.ExecSQL('delete from xmlnfe');
  end;
end;
...
procedure Tfrmxml.Inicia_someTimer(Sender: TObject);
begin
  frmxml.Visible := False;
  Inicia_some.Enabled := False;
  controle_manifesto.Enabled := True;
end;

procedure Tfrmxml.MostraDownloadXML1Click(Sender: TObject);
begin
  frmxml.Visible := True;
end;

procedure Tfrmxml.N1Click(Sender: TObject);
begin
  // Consultar SEFAZ
...
procedure Tfrmxml.OcultarDownloadXML1Click(Sender: TObject);
begin
  frmxml.Visible := False;
end;

procedure Tfrmxml.AbrirComWindows(Value: boolean);
var
  Reg: TRegistry;
begin
  ...
  if (Value = True) then
    Reg.WriteString('DownloadXML', application.ExeName + ' s')
  else
    Reg.DeleteValue('DownloadXML');
  Reg.closekey;
  Reg.Free;
end;
