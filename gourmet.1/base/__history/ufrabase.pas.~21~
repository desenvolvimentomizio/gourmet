...
const
  AcoesSemPermissao: TAcoesSemPermissao = ('ActAtualizar', 'ActConfig', 'ActConfiguracoes', 'ActFiltrar', 'ActPesquisar', 'ActRelatorios',
    'ActInstrucoes', 'ActSair', 'ActVerFiltros');
...
const
  ...
  PEG_COR_TITULO: Integer = $00C08000;
  PEG_COR_TOTAIS: Integer = $00C08000;
...
type
  ...
  TAutorizacao = function(AOwner: TComponent; Conexao: TUniConnection; vusuario: string; vactcodigo: string; vmotivo: string = '';
    vtdecodigo: String = ''; vorcchave: String = ''; vmeschave: String = ''; vltecodigo: String = ''; vddfcodigo: String = '';
    vForcaLogin: Boolean = False; vtabela: string = ''; vregistro: string = ''): string;
...
begin
  ...
  begin
    ...
    if (uqtabela.FieldByName(DBGLista.Columns[i].FieldName).DataType = ftDate) or
      (uqtabela.FieldByName(DBGLista.Columns[i].FieldName).DataType = ftDateTime) then
    begin
    ...
    if vlCheckBox <> nil then
      ...
      begin
        ...
        begin
          ...
          if tabe.Fields[0].DataType = ftString then
            vlFiltro := vlFiltro + Copy(tabe.Fields[0].FieldName, 1, 3) + '.' + tabe.Fields[0].FieldName + '=' +
              QuotedStr(vlCheckBox.HelpKeyword) + ' or ';
...
begin
  ...
  begin
    ...
    if (vlNomeAcao = 'ActIncluirSelecionado') or (vlNomeAcao = 'ActIncluirTodos') or (vlNomeAcao = 'ActRemoverTodos') or
      (vlNomeAcao = 'ActRemoverSelecionado') then
    begin
    end
    else
    begin
            ...
            try
              ...
              begin
                ...
                self.uqtabela.FilterSQL := Copy(fmdfmdcampo.AsString, 1, 3) + '.' + 'mesinclusao' + ' BETWEEN (' +
                  ...
                  QuotedStr(FormatDateTime('yyyy-mm-dd', IncDay(fmdfmddtfinal.AsFloat, 1)) + ' ' +
                  Timetostr(IncSecond(cfgmgoucfgmgouhoravirada.AsDateTime, -1))) + ')';
            ...
            try
              if not fmd.IsEmpty then
                self.uqtabela.FilterSQL := Copy(fmdfmdcampo.AsString, 1, 3) + '.' + fmdfmdcampo.AsString + ' BETWEEN (' +
                  QuotedStr(FormatDateTime('yyyy-mm-dd', fmdfmddtinicial.AsFloat)) + ') and (' +
                  QuotedStr(FormatDateTime('yyyy-mm-dd', fmdfmddtfinal.AsFloat)) + ')';
              AtualizaFiltroUQTabela;
              // Self.uqtabela.Prepare;
              self.uqtabela.Open;
            except
              self.uqtabela.FilterSQL := fmdfmdcampo.AsString + ' BETWEEN (' + QuotedStr(FormatDateTime('yyyy-mm-dd', fmdfmddtinicial.AsFloat)) +
                ') and (' + QuotedStr(FormatDateTime('yyyy-mm-dd', fmdfmddtfinal.AsFloat)) + ')';
              AtualizaFiltroUQTabela;
              // Self.uqtabela.Prepare;
              self.uqtabela.Open;
            end;
...
var
  vltitucol: String;
  vlnomecol: String;
  vltipocol: TFieldType;

begin
  // RegistraEvento(Column);


  if pos('.', vlnomecol) > 0 then
    vlnomecol := Trim(Copy(vlnomecol, pos('.', vlnomecol) + 1, 200));

  if (vltipocol = ftString) then
  begin
    try
      vpalavra := self.DBGLista.DataSource.DataSet.FieldByName(vlnomecol).AsString;
    except
    end;
  end
  else
...
begin
  ...
  vltitucol := DBGLista.Columns[DBGLista.SelectedIndex].Title.caption;
  vlnomecol := DBGLista.Columns[DBGLista.SelectedIndex].FieldName;
  vltipocol := DBGLista.Columns[DBGLista.SelectedIndex].Field.DataType;
  if pos('.', vlnomecol) > 0 then
    vlnomecol := Trim(Copy(vlnomecol, pos('.', vlnomecol) + 1, 200));
...
begin
  ...
  sqlbase := 'select 1 as tmpselecionar, ' + vlNomeColuna + '  from ' + Copy(sqlbase, pos('from', lowercase(sqlbase)) + 4, 10000) + ' group by ' +
    vlNomeColuna + ' order by ' + vlNomeColuna;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            qrySugestao.SQL.text := StringReplace(qrySugestao.SQL.text, 'from etd order by',
              'from etd,etv where etd.etdcodigo=etv.etdcodigo and etv.tvicodigo=2 order by', [rfReplaceAll, rfIgnoreCase]);
          end
          else if (pos('cre', self.Name) > 0) or (pos('brr', self.Name) > 0) then
          begin
            qrySugestao.SQL.text := StringReplace(qrySugestao.SQL.text, 'from etd order by',
              'from etd,etv where etd.etdcodigo=etv.etdcodigo and etv.tvicodigo=1 order by', [rfReplaceAll, rfIgnoreCase]);
          end;
...
begin
  ...
  if Acesso.Usuario = 0 then
  begin
    consulta.Close;
    cpg.Connection := self.ZCone;
    consulta.SQL.text := 'select clbcodigo from acs where acschave=' + Acesso.IdAcesso.ToString;
    consulta.Open;

    if not consulta.IsEmpty then
    begin
      Acesso.Usuario := consulta.FieldByName('clbcodigo').AsInteger;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if ((((((self.Parent as TPanel).Parent as TTabSheet).Parent as TPageControl).Parent) as TFrmBase).FindComponent('psituacao') as TPanel)
          .caption = 'Incluindo' then
        begin
          if ((((((self.Parent as TPanel).Parent as TTabSheet).Parent as TPageControl).Parent) as TFrmBase).FindComponent('cfg') <> nil) then
          begin
            if ((((((self.Parent as TPanel).Parent as TTabSheet).Parent as TPageControl).Parent) as TFrmBase).FindComponent('cfg') as TUniQuery).Active
            then
            begin
              if ((((((self.Parent as TPanel).Parent as TTabSheet).Parent as TPageControl).Parent) as TFrmBase).FindComponent('cfg') as TUniQuery)
                .FindField('cfgeminclusao') <> nil then
              begin
                if ((((((self.Parent as TPanel).Parent as TTabSheet).Parent as TPageControl).Parent) as TFrmBase).FindComponent('cfg') as TUniQuery)
                  .FindField('cfgeminclusao').AsInteger = 1 then
                begin
...
begin
  ...
  begin
    ...
    begin
      if ((self.acoes[p] as TAction).Name <> 'ActPesquisar') And ((self.acoes[p] as TAction).Name <> 'ActFiltrar') And
        ((self.acoes[p] as TAction).Name <> 'ActAtualizar') And ((self.acoes[p] as TAction).Name <> 'ActSair') And
        ((self.acoes[p] as TAction).Name <> 'ActVerFiltros') And ((self.acoes[p] as TAction).Name <> 'ActConfig') And
    ...
    *)
    ...
    begin
      ...
      if (self.acoes[i] as TAction).Enabled then
        if ((self.acoes[i] as TAction).Name <> 'ActPesquisar') And ((self.acoes[i] as TAction).Name <> 'ActFiltrar') And
          ((self.acoes[i] as TAction).Name <> 'ActAtualizar') And ((self.acoes[i] as TAction).Name <> 'ActSair') then
        begin
...
function TFraBase.MostraFormu(pModulo: string; pChave: string; pChaveMestre: string; pFormulario: string = 'formulario';
  pFiltro: String = ''): String;
var
  ExecForm: function(CargaFrame: TCargaFrame): String;
  vlCargaFrame: TCargaFrame;
begin
