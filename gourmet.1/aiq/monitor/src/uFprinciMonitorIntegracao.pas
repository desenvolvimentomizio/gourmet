...
begin
  ...
  begin
     ...
     Memo1.lines.add(vlPastaPedido+'\'+vlNomeArquivo);
     if fileexists(vlPastaPedido+'\'+vlNomeArquivo) then
     begin
        consulta.Close;
        consulta.sql.Text:='update orc set orcstatusintegracao=1, orchoraintegracao=now() where orcchave='+orcsorcchave.AsString;
        consulta.ExecSQL;
     end;
    orcs.Next;
  end;
