...
type
  Tfprincigereimpgoufast02 = class(TForm)
    plCozinha: TPanel;
    plTopo: TPanel;
...
function Tfprincigereimpgoufast02.AjustaSituacaoCozinha: Boolean;
begin
  ...
  if czn.IsEmpty then
  begin
    plCozinha.Caption := 'Aten��o: A Cozinha n�o esta aberta, solicite a abertura da Cozinha!';
...
function Tfprincigereimpgoufast02.enviaBMPImpressora(vlNumePedido: Integer; vporta: string; vlRetorno: Integer; vlimms: string;
...
procedure Tfprincigereimpgoufast02.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;

procedure Tfprincigereimpgoufast02.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  CanClose := True;
end;

procedure Tfprincigereimpgoufast02.FormCreate(Sender: TObject);
begin
  Screen.Cursors[crSQLWait] := Screen.Cursors[crDefault];

end;

procedure Tfprincigereimpgoufast02.inicializarTimer(Sender: TObject);
var
  i: Integer;
begin
...
function Tfprincigereimpgoufast02.ConsultaEstadoImpressora(videntificacao: string; vporta: string): string;
begin
  result := '24'; // ConsultaImpressora(videntificacao, vporta);
end;

function Tfprincigereimpgoufast02.BaixaArqRel(vFoaCodigo: string; vrelarquivo: TBlobField): String;
var
  vlNomeArq: String;
...
Function Tfprincigereimpgoufast02.ConsultaImpressora(videntificacao: string; vporta: string): String;
Var
  i_retorno: Integer;
  sRetorno: String;
Begin
...
procedure Tfprincigereimpgoufast02.VerificaPedidosImpressora(vImpressora: string);
var
  vlTentativas: Integer;
  vlCznCodigo: Integer;
...
procedure Tfprincigereimpgoufast02.VerificaPedidos01(vImpresso: string);
var
  vlTentativas: Integer;
  vlCznCodigo: Integer;
...
procedure Tfprincigereimpgoufast02.VerificaPedidos;
var
...
function Tfprincigereimpgoufast02.ImprimePedido(vOrcChave: string; vFoaCodigo: string; vrelarquivo: TBlobField; vimmorigem: TUniQuery;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          sqlimprime.ParamByName('tcicodigo').AsString := vltcicodigo;
          sqlimprime.Open;
          ...
          begin
            ...
            consulta.close;
            consulta.SQL.Text := 'update imm set  tcicodigo=' + vlTciCodigo + ' ,immhoraimpresso=null where imm.tcicodigo=' + vlTciCodigo +
              '  and  immchave in (' + sqlimprimeimmchave.AsString + ')';
            consulta.ExecSQL;
          ...
          begin
            ...
            begin
              deletefile(vpPastaDia + '\ped' + formatfloat('0000', vlNumePedido) + 'a.fp3');
              frxBMPExport.filename := 'ped' + formatfloat('0000', vlNumePedido) + 'a.bmp';
              relatorio.PreviewPages.SaveToFile(vpPastaDia + '\ped' + formatfloat('0000', vlNumePedido) + 'a.fp3');
          ...
          begin
            ...
            begin
              ...
              consulta.close;
              consulta.SQL.Text := 'update imm set tcicodigo=' + vlTciCodigo + ' ,immhoraimpresso=NOW() where imm.tcicodigo=' + vlTciCodigo +
                '   and immchave in (select immchave from tped where orcchave=' + vOrcChave + ' and immnumepedido=' + inttostr(vlNumePedido) +
            ...
            begin
              ...
              consulta.SQL.Text := 'update imm set tcicodigo=' + vlTciCodigo +
                ' , immhoraimpresso=NOW() where imm.tcicodigo='+vlTciCodigo+'  and  immchave in (select immchave from tped where orcchave=' + vOrcChave + ' and immnumepedido=' +
...
function Tfprincigereimpgoufast02.EstadoImpressora(vporta: string): Integer;
Var
  vu: string;
...
procedure Tfprincigereimpgoufast02.SetZCone(const Value: TUniConnection);
var
  i: Integer;
begin
  FZCone := Value;
...
function Tfprincigereimpgoufast02.StatusEstendido: Integer;
var
  St: TACBrPosPrinterStatus;
begin
  { Traduz o status do ACBrPosPrinter para os codigos legados (24=ok, 32=sem
    papel, 5=pouco papel, 9=tampa aberta, 0=offline). }
