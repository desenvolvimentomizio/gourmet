...
uses
  ...
  Vcl.DBCtrls, Vcl.Mask, Vcl.ImgList, Vcl.Controls, PngImageList, System.Classes,
...
function Tfresgte.selecionachequesterceiro(vchave: string; vschcodigo: Integer): string;
var
...
procedure Tfresgte.bconfirmaClick(Sender: TObject);
var
...
procedure Tfresgte.ccovalorEnter(Sender: TObject);
var
  vccoctadestino: Integer;
  vccochaveatu: Integer;
  vtctorigem: Integer;
...
begin
  ...
  ccd.Params[1].AsString := registroccochave.AsString;
  ccd.Open;
  if ccd.IsEmpty then
  begin
    ccd.append;
    ccdccochave.AsString := registroccochave.AsString;
...
procedure Tfresgte.ctacodigoEnter(Sender: TObject);
begin
  // inherited;

end;

procedure Tfresgte.ctacodigoExit(Sender: TObject);
begin
  inherited;
...
function Tfresgte.finaliza(vchave: string): string;
type
...
procedure Tfresgte.FormShow(Sender: TObject);
begin
...
procedure Tfresgte.registroAfterInsert(DataSet: TDataSet);
begin
  inherited;
