...
type
  ...
  private
    ...
    function CartaoCreditoAvista: Boolean;
...
uses
  ...
  Data.DB, pingsend, uflteFormaCredito;
...
procedure TftransacaoTEFAPI.ProcessaPagamentosPendentes;
var
 vlrct:TUniQuery;
begin
    ...
    while not vlrct.eof do
    begin


      if ProcessaTransacaoPendente(vlrct.FieldByName('rctrede').asstring,
                                vlrct.FieldByName('rctnrauto').asstring) then
                                begin
                                  vlrct.Edit;
                                  vlrct.FieldByName('rctstatus').AsString:='PROCESSADA';
                                  vlrct.Post;
                                end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if IndicePagto=4 then
          begin

            if CartaoCreditoAvista then
              qtParc:='1'
            else
              qtParc:='2';
          ...
          if (OperacaoTEF.QuantidadeParcela='1') then
          begin
          ...
          begin
            ...
            Ok := CompACBrTEF.EfetuarPagamento(IntToStr(OperacaoTEF.numero), AValor, Modalidade, CartoesAceitos, tefmfParceladoEstabelecimento,OperacaoTEF.QuantidadeParcela.ToInteger);
            if ok then
...
function TftransacaoTEFAPI.CartaoCreditoAvista:Boolean;
var
  flteFormaCredito: TflteFormaCredito;
begin
  try
    flteFormaCredito := TflteFormaCredito.Create(application);
    if flteFormaCredito.ShowModal=mrok then
      Result:=true
    else
      Result:=false;

    flteFormaCredito.Free;
