...
begin
  ...
  try
    ...
    lRes := TRequest.New.BaseURL(Format('https://api.pagar.me/core/v5/orders/%s/closed', [lblidPagamento.Caption])).BasicAuthentication(Admin, '').ContentType('application/json').AddBody(lJson.Stringify).Patch;
    ...
    begin
      ...
      lRes := TRequest.New.BaseURL(BaseURL + 'cancel').ContentType('application/json').AddBody(lJson.Stringify).Post;
      if (lRes.StatusCode = 200) and ((lRes.Content) <> '') then
      begin
        try
          orc.close;
  ...
  try
   ...
   lRes := TRequest.New.BaseURL(Format('https://api.pagar.me/core/v5/orders/%s', [lblIdPagamento.Caption]))
    ...
    begin
      ...
      if  vpValorRecebido>=(valorAReceber/100) then
      begin
        ProcessaRecebimento;
      end;
...
procedure  Tfstonehide.recebimentoDireto;
var
 lRes  : IResponse;
 lJson : TJson;
begin
...
function Tfstonehide.SalvaRegistroTITVENDA(aLteChave:String; aQuitado:Boolean):string;
var
 vlQuitado:Boolean;
 vlTentativas:Integer;
begin
  ...
  begin
    ...
    begin
      ...
      try
        ...
        if titetdcodigo.AsInteger<>0 then
          tittfdcodigo.AsInteger :=2;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        vlTitCodigo:=RegistraTit(fEtdCodigo,dtldtlvalor.AsCurrency, DataAtual);
        ...
        RegistraRfi(vlTitCodigo, DataAtual,1,dtldtlvalor.AsCurrency );

        SalvaRegistroTIT(olt.FieldByName('ltechave').AsString);

      end;

    //  RegistraCCO(dtldtlvalor.AsCurrency, ContaCaixa.ToString,0,tfiOutros);
...
begin
  ...
  begin
    ...
    begin
      ...
      // registo financeio de cada lote de venda


      vlTitCodigo:=RegistraTit(fEtdCodigo, rctorcrctvalor.AsCurrency, DataAtual);
      ...
      dfcrfitfdcodigo.AsInteger:=tfdReceber;
...
begin
  ...
  if (lblMesa.Color=$00408000) or (lblMesa.Color=clgreen) or (lblMesa.Color=clred) or (Panel1.Color= $004080FF) then
  begin
    if lblMesa.Color=clgreen then
    begin
        rpw.Edit;
        rpwrpwstatus.AsString:='RECEBIDO';
        rpw.Post;
        ...
        if not mor.IsEmpty then
        begin
          EmiteDocumento(mor.FieldByName('meschave').AsInteger);
        end;
    ...
    begin
      ...
      if MessageDlg('Encerrar recebimento na mesa ?', mtConfirmation, [mbYes, mbNo], 0, mbNo) = IdYes then
      begin
...
procedure Tfstonehide.EmiteDocumento(pMesChave: Integer);
var
  vRetorno: String;
...
begin
   ...
   cfgmcfg.Close;
   cfgmcfg.ParamByName('flacodigo').AsInteger:=facesso.Filial;
   cfgmcfg.Open;
  ...
  begin
    ...
    begin
      ...
      vlEtdCodigo := mostralista('mcli', facesso.usuario.ToString, '');
...
function Tfstonehide.SalvaRegistroTIT(aLteChave:String):string;
var
 vlQuitado:Boolean;
 vlTentativas:Integer;
begin
...
procedure Tfstonehide.SalvaRegistroRFI(aTitCodigo:String;mTitCodigo:String; aLtechave:String; aSituacao:Boolean);
var
  vlRfiChave:String;
  vlRfiValor:Currency;
  vlTentativas:Integer;
...
function Tfstonehide.RegistraCCO(avalor: Double; aCtaCodigo: String;  adoacao: Integer = 0; aTfiCodigo:Integer=0): string;
var
  vlCcoChave:Integer;
begin
...
procedure Tfstonehide.SalvaRegistroCLT(aCcoChave:String;aLteChave:String);
var
  vlTentativas:Integer;
begin
    ...
    try
      ...
      if Assigned(vlImprimirOrc) then
        result := vlImprimirOrc(Application,
                                ZCone, pedido,
                                facesso.Terminal.ToString,
                                facesso.usuario.ToString,
