...
uses midaslib, ufclb, ufdireitos, PASSWORD;
...
begin
  try
    consulta.close;
    consulta.SQL.Text := 'select clbsuper from clb where clbcodigo=' + Acesso.usuario.ToString;
    consulta.Open;
    vlPode := false;

    if (Acesso.usuario = 1) or (uqtabelaclbcodigo.AsInteger = 1) then
    begin
