...
function FormuFlh(AOwner: TComponent; conexao: tuniconnection; acesso: Tacesso; vtipolista: Integer; vmodulo: string; vfiltro: string; vmodo: string;
  vchave: string): string;
begin
  try
...
begin
  ...
  begin
    ...
    If Application.MessageBox(PChar('Já existe um lançamento deste evento nesta folha para este Colaborador!' + #13 + 'Deseja continuar assim mesmo ?'),
      'Atenção', MB_YESNO { + MB_DEFBUTTON1 } + MB_ICONQUESTION) = IDNO Then
    Begin
      result := False;
    End
    else
