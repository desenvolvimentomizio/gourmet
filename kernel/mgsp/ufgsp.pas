...
type
  Tfgsp = class(TForm)
    ...
    btnB_K: TButton;
    prok: TUniQuery;
    prokprocodigo: TIntegerField;
    prokpronome: TStringField;
    prokpunbarra: TStringField;
    proktpocodigo: TIntegerField;
    proktpoidentificacao: TStringField;
    prokproncm: TStringField;
    prokicmcodigo: TStringField;
    prokunicodigo: TIntegerField;
    prokicmaliquotas: TStringField;
    prokunisimbolo: TStringField;
    prokprosaldo: TFloatField;
    prokuninome: TStringField;
    Label1: TLabel;
    prosaldok: TUniQuery;
    prosaldokprocodigo: TIntegerField;
    prosaldokprosaldo: TFloatField;
    rfi: TUniQuery;
    ori: TUniQuery;
    orietdcodigo: TIntegerField;
    rfirfivalor: TCurrencyField;
    dtl: TUniQuery;
    dtldtlvalor: TCurrencyField;
    cta: TUniQuery;
    ctacfgctacodigopix: TIntegerField;
    etdpix: TUniQuery;
    etdpixetdcodigo: TIntegerField;
    etdpixetdidentificacao: TStringField;
    etdpixetddoc1: TStringField;
    etdpixtpecodigo: TIntegerField;
    etdpixedrcodigo: TIntegerField;
    etdpixedrinscest: TStringField;
    etdpixcddcodigo: TStringField;
    etdpixedrrua: TStringField;
    etdpixedrnumero: TStringField;
    etdpixedrcomple: TStringField;
    etdpixedrbairro: TStringField;
    etdpixedritem: TIntegerField;
    ctacfgetdcodigopix: TIntegerField;
    dtlmad: TUniQuery;
    dtlmadmdacodigo: TIntegerField;
    dtlmaddtlvalor: TCurrencyField;
    adc: TUniQuery;
    adcetdcodigo: TIntegerField;
    etdrfi: TUniQuery;
    etdrfietdcodigo: TIntegerField;
    etdrfietdidentificacao: TStringField;
    etdrfietddoc1: TStringField;
    etdrfitpecodigo: TIntegerField;
    etdrfiedrcodigo: TIntegerField;
    etdrfiedrinscest: TStringField;
    etdrficddcodigo: TStringField;
    etdrfiedrrua: TStringField;
    etdrfiedrnumero: TStringField;
    etdrfiedrcomple: TStringField;
    etdrfiedrbairro: TStringField;
    etdrfiedritem: TIntegerField;
...
begin
  try
    ...
    consulta.close;
    consulta.sql.text := 'UPDATE etd SET etdidentificacao=' + QuotedStr('CONSUMIDOR') + ', etdapelido=' + QuotedStr('CONSUMIDOR') +
      ' where etdcodigo=0';
    consulta.execsql;
...
Begin
  ...
  dtlmad.close;
  dtlmad.ParamByName('datainicial').AsDate := Self.spdspddatainicial.AsFloat;
  dtlmad.ParamByName('datafinal').AsDate := Self.spdspddatafinal.AsFloat;
  dtlmad.Open;

  adc.close;
  adc.Open;

  if not dtlmad.IsEmpty then
  begin
    ...
    madetdcodigo.AsInteger := adcetdcodigo.AsInteger;;
    madspdchave.AsInteger := Self.spdspdchave.AsInteger;

    dtlmad.First;
    madmadvalorccredito.AsCurrency := dtlmaddtlvalor.AsCurrency;

    dtlmad.Next;
    madmadvalorcdebito.AsCurrency := dtlmaddtlvalor.AsCurrency;
    ...
    Begin
      ...
      Begin
        ...
        Begin
          ...
          if Trim(Self.mEtdetdidentificacao.AsString) <> '' then
          begin
            if Self.mEtdetdidentificacao.AsString <> 'CONSUMIDOR' then
            begin
      ...
      Begin
        ...
        if mPro.Locate('unisimbolo', Trim(Self.mUniunisimbolo.AsString), []) then
        begin
          With Registro0190New Do
          Begin
            UNID := Trim(Self.mUniunisimbolo.AsString);
            DESCR := Self.mUniuninome.AsString;
          End;
        end
        else
        begin
          if pro.Locate('unisimbolo', Trim(Self.mUniunisimbolo.AsString), []) then
          begin
            With Registro0190New Do
            Begin
              UNID := Trim(Self.mUniunisimbolo.AsString);
      ...
      unimulti.sql.Add('AND  mes.sdecodigo<>' + QuotedStr('02') + ' and mes.tdfcodigo<>' + QuotedStr('CI'));

      unimulti.sql.Add('AND mes.meschave=itm.meschave ');
      unimulti.sql.Add('order by pro.procodigo');
      ...
      Begin
        ...
        Begin
          ...
          Begin
            ...
            Begin
              if { (unimulti.FieldByName('unisimbolo').AsString <> UNID_INV) and }
                (unimulti.FieldByName('procodigo').AsInteger = Self.mProprocodigo.AsInteger) then
              begin

                if pos(Trim(uppercase(unimulti.Fields[0].AsString) + unimulti.Fields[2].AsString), vlListaUnidades.text) = 0 then
                begin
                  ...
                  Begin
                    ...
                    vlListaUnidades.Add(Trim(uppercase(unimulti.Fields[0].AsString + unimulti.Fields[2].AsString)));
...
Begin
  ...
  Begin
    ...
    If Not mUni.Locate('unisimbolo', Trim(uniunisimbolo.AsString), []) Then
    Begin
      mUni.Append;
      mUniunicodigo.AsInteger := uniunicodigo.AsInteger;
      mUniunisimbolo.AsString := Trim(uniunisimbolo.AsString);
  ...
  prok.close;
  prok.ParamByName('datainicial').AsDate := Self.spdspddatafinal.AsFloat;
  prok.ParamByName('datafinal').AsDate := Self.spdspddatafinal.AsFloat;
  prok.Open;

  if not prok.IsEmpty then
  begin

    prok.First;
    While Not prok.Eof Do
    Begin

      If Not mPro.Locate('procodigo', prokprocodigo.AsInteger, []) Then
      Begin
        mPro.Append;
        For i := 0 To mPro.FieldCount - 1 Do
        Begin
          mPro.Fields[i] := prok.Fields[i];
        End;
        mPro.Post;

      End;

      If Not mUni.Locate('unisimbolo', prokunisimbolo.AsString, []) Then
      Begin
        mUni.Append;
        mUniunicodigo.AsInteger := prokunicodigo.AsInteger;
        mUniunisimbolo.AsString := Trim(prokunisimbolo.AsString);
        mUniuninome.AsString := prokuninome.AsString;
        mUni.Post;
...
begin
  ...
  Begin
...

    {
      ...
      if ori.RecordCount > 0 then
      begin
      ori.First;
      while not ori.Eof do
      begin
      rfi.Close;
      rfi.ParamByName('etdcodigo').AsInteger := orietdcodigo.AsInteger;
      rfi.ParamByName('datainicial').AsDate := spdspddatainicial.AsDateTime;
      rfi.ParamByName('datafinal').AsDate := spdspddatafinal.AsDateTime;
      rfi.Open;

      if rfirfivalor.AsCurrency > 0 then
      begin

      With Registro1601New Do
      Begin

      COD_PART_IP := orietdcodigo.AsString + '01';
      COD_PART_IT := orietdcodigo.AsString + '01';
      TOT_VS := rfirfivalor.AsCurrency;
      TOT_ISS := 0;
      TOT_OUTROS := 0;
    ...
    {
      dtl.Close;
      dtl.ParamByName('datainicial').AsDate := spdspddatainicial.AsDateTime;
      dtl.ParamByName('datafinal').AsDate := spdspddatafinal.AsDateTime;
      dtl.Open;

      if dtl.RecordCount > 0 then
      begin

      if dtldtlvalor.AsCurrency > 0 then
      begin

      With Registro1601New Do
      Begin

      COD_PART_IP := ctacfgetdcodigopix.AsString + '01';
      COD_PART_IT := ctacfgetdcodigopix.AsString + '01';
      TOT_VS := dtldtlvalor.AsCurrency;
      TOT_ISS := 0;
      TOT_OUTROS := 0;
    ...
    begin
      ...
      rfi.ParamByName('datainicial').AsDate := spdspddatainicial.AsDateTime;
      rfi.ParamByName('datafinal').AsDate := spdspddatafinal.AsDateTime;
      rfi.ParamByName('etdcodigo').AsInteger := adcetdcodigo.AsInteger;
      rfi.Open;
      ...
      begin
        ...
        begin
          ...
          Begin
            ...
            COD_PART_IP := adcetdcodigo.AsString + '01';
            COD_PART_IT := adcetdcodigo.AsString + '01';
            TOT_VS := rfirfivalor.AsCurrency;
            TOT_ISS := 0;
            TOT_OUTROS := 0;
...
Begin
  ...
  Begin
    If (mes.RecordCount > 0) or (mesnfce.RecordCount > 0) Then
    Begin

      With RegistroC001New Do
      Begin
        if (mesnfce.IsEmpty) and (mes.IsEmpty) then
        begin
          IND_MOV := imSemDados;
        end
        else
        begin
          ...
          Begin // Fran  - 21/12/2011 as 17hs30min
            Label1.caption := mostra1.Position.ToString;
            mostra1.Position := mostra1.Position + 1;
            application.ProcessMessages;



            // if mesmesnumero.Asinteger=21624 then
            // showmessage('') ;
            ...
            begin
              ...
              If (mes.RecordCount > 0) and (itm.RecordCount > 0) Then
              Begin
                ...
                Begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      Begin
                        mostra.Position := mostra.Position + 1;
                        application.ProcessMessages;
                        if itmcfocfop.AsString <> '5.405' then
                        begin
                  ...
                  Begin
                    ...
                    begin
                      ...
                      // if mesmesprodutos.AsFloat = 0 then
                      VL_DOC := mesmestotal.AsFloat - mesmesoutras.AsCurrency;
                      // VL_DOC := mesmestotal.AsFloat - mesmesoutras.AsCurrency;
                      // else
                    ...
                    begin
                      ...
                      begin
                        if (Self.mesmesipi.AsCurrency > 0) and (Self.mesmesoutras.AsCurrency = 0) then
                          VL_MERC := Self.mesmestotal.AsFloat - Self.mesmesipi.AsCurrency + Self.mesmesdesconto.AsFloat
                    ...
                    begin
                      if Self.mesttecodigo.AsInteger = 1 then
                      begin
                        VL_MERC := Self.mesmestotal.AsFloat;
                        // VL_MERC := Self.mesmesprodutos.AsFloat;
                      end
                      else
                      begin
                    ...
                    Begin
                      ...
                      begin
                        ...
                        // daniel 30 03      VL_BC_ICMS := 0;
                        // daniel 30 03      VL_ICMS := 0;
                  ...
                  If ((mesttmcodigo.AsInteger = tmcInclusao) Or (Self.mestdfcodigo.AsString = tdfNotaFiscal)) and
                    (Self.messdecodigo.AsString <> sdeDocCancelado) and (Self.messdecodigo.AsString <> sdeNFeCTeDenegada) Then
                  Begin
                    ...
                    Begin
                      ...
                      Begin
                        ...
                        begin
                          CFOP := SoNumeros(Self.itmcfocfop.AsString);
                        end;
    ...
    With ACBrSPEDFiscal1.Bloco_H Do
      ...
      begin
        ...
        Begin
          ...
          // self.spdspdmotivoinv.AsFloat; -  Motivo do Inventário - ainda não está informado no componente, está só no manual.
...
procedure Tfgsp.btnB_KClick(Sender: TObject);
begin
  ...
  begin
    ...
    prosaldok.close;
    prosaldok.ParamByName('datainicial').AsDate := Self.spdspddatafinal.AsFloat;
    prosaldok.ParamByName('datafinal').AsDate := Self.spdspddatafinal.AsFloat;
    prosaldok.Open;

    if not prosaldok.IsEmpty then
    begin
      ...
      begin
        ...
        begin
          ...
          while not prosaldok.Eof do
          begin

            with RegistroK200New do
            begin

              COD_ITEM := prosaldokprocodigo.AsString;
              QTD := prosaldokprosaldo.AsFloat;
...
Begin
  ...
  Begin
    ...
    if etdetdidentificacao.AsString <> 'CONSUMIDOR' then
    BEGIN
  ...
  Begin
    ...
    if etdcteetdidentificacao.AsString <> 'CONSUMIDOR' then
    BEGIN
      If Not mEtd.Locate('etdcodigo', CodEntidade, []) Then
      Begin
        mEtd.Append;
  ...
  Begin
    ...
    if etdcsvetdidentificacao.AsString <> 'CONSUMIDOR' then
    BEGIN
  ...
  Begin
    ...
    if etdadcetdidentificacao.AsString <> 'CONSUMIDOR' then
    BEGIN
  ...
  // dados de pix e recebimentos online
  {
    ori.Close;
    ori.Open;
    while not ori.Eof do
    ...
    begin
    etdpix.Close;
    etdpix.ParamByName('etdcodigo').AsInteger := orietdcodigo.AsInteger;
    etdpix.Open;

    CodEntidade := StrToInt(etdpixetdcodigo.AsString + FormatFloat('00', etdpixedritem.AsInteger));
  ...
  begin
    ...
    begin
      ...
      if ctacfgctacodigopix.AsString <> '' then
      begin

        etdpix.close;
        etdpix.ParamByName('etdcodigo').AsInteger := ctacfgetdcodigopix.AsInteger;
        etdpix.Open;

        CodEntidade := StrToInt(ctacfgetdcodigopix.AsString + FormatFloat('00', etdpixedritem.AsInteger));
  ...
  begin
    ...
    etdrfi.close;
    etdrfi.ParamByName('etdcodigo').AsInteger := adcetdcodigo.AsInteger;
    etdrfi.Open;

    if not etdrfi.IsEmpty then
    begin
      CodEntidade := StrToInt(etdrfietdcodigo.AsString + FormatFloat('00', etdrfiedritem.AsInteger));
