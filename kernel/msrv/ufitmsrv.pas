...
procedure Tfitmsrv.bcancelaClick(Sender: TObject);
var
  smsg: string;
begin
...
procedure Tfitmsrv.bconfirmaClick(Sender: TObject);
...
Begin
  ...
  begin
    ...
    begin
//      Self.registroitmbicm.AsFloat := Self.itmtotalgeral.Field.AsFloat - (Self.itmtotalgeral.Field.AsFloat * ((100-itmpercreducaobaseicm.Field.AsFloat) / 100));
      Self.registroitmbicm.AsFloat := (Self.itmtotalgeral.Field.AsFloat * ((itmpercreducaobaseicm.Field.AsFloat) / 100));
    end;
  end;
...
procedure Tfitmsrv.FormShow(Sender: TObject);
var
  i: Integer;
  vlTpoCodigo: Integer;
...
procedure Tfitmsrv.icmcodigoExit(Sender: TObject);
begin
  inherited;
  If (itmicm.Field.AsFloat > 0) and (itmbicm.Field.AsFloat = 0) Then
  begin
...
procedure Tfitmsrv.itmaliqcofinsExit(Sender: TObject);
begin
  inherited;
...
procedure Tfitmsrv.itmaliqpisExit(Sender: TObject);
begin
  inherited;
...
procedure Tfitmsrv.itmdescontoChange(Sender: TObject);
begin
  inherited;
  ajustaicm;
end;

procedure Tfitmsrv.itmdescontoExit(Sender: TObject);
begin
  inherited;
  ...
  begin
    if (cstcodigo.Field.AsString = '000') or (cstcodigo.Field.AsString = '010') or (cstcodigo.Field.AsString = '020') then
    begin
    //  registroitmbicm.AsFloat := registroitmtotalgeral.AsFloat - (registroitmtotalgeral.AsFloat * ((100-itmpercreducaobaseicm.Field.AsFloat) / 100));
          Self.registroitmbicm.AsFloat := (Self.itmtotalgeral.Field.AsFloat * ((itmpercreducaobaseicm.Field.AsFloat) / 100));
    end;
  end;
...
procedure Tfitmsrv.itmicmEnter(Sender: TObject);
begin
  inherited;
  ajustaicm;
end;

procedure Tfitmsrv.itmquantidadeChange(Sender: TObject);
begin
  inherited;
  ajustaicm;
end;

procedure Tfitmsrv.itmquantidadeExit(Sender: TObject);
begin
  Self.ValidaSaida(Sender);
...
procedure Tfitmsrv.itmvalorChange(Sender: TObject);
begin
  inherited;
  ajustaicm;
end;

procedure Tfitmsrv.itmvalorEnter(Sender: TObject);
var
  vvalor: Double;
begin
  inherited;
...
function Tfitmsrv.MontaFiltroPun: Integer;
var
  vTfpCodigo: Integer;
begin
...
procedure Tfitmsrv.procodigoExit(Sender: TObject);
var
  Handle: THandle;
begin
  Self.ValidaSaida(Sender);
  ...
  consulta.Close;
  consulta.SQL.Text := 'SELECT prosaldodisp, tpocodigo,cspcodigo, csfcodigo, propisaliquota, procofinsaliquota FROM pro WHERE ';
  ...
  begin
    ...
    begin
      ...
      Handle := FindWindow('Tfsrvsimples', nil);

      if Handle <> 0 then
      begin
        if Application.FindComponent('fsrvsimples') <> nil then
        begin
          (Application.FindComponent('fsrvsimples') as Tfsrvsimples).Retorno := Self.procodigo.Field.AsString;;
...
procedure Tfitmsrv.puncodigoEnter(Sender: TObject);
begin
  inherited;
...
procedure Tfitmsrv.puncodigoExit(Sender: TObject);
var
  vvalor: Double;
  vTfpCodigo: Integer;
begin
...
procedure Tfitmsrv.registroAfterInsert(DataSet: TDataSet);
var
  vlCstCodigo: string;
begin
  inherited;
...
procedure Tfitmsrv.registroBeforePost(DataSet: TDataSet);
begin
  inherited;
...
procedure Tfitmsrv.registroCalcFields(DataSet: TDataSet);
begin
  inherited;
...
procedure Tfitmsrv.DefineDadosEmbalagem;
var
  vTfpCodigo: Integer;
begin
