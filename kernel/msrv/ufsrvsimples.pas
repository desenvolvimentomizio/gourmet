...
uses ufitmsrv, uftomsrv, ufdtmsrv;

function Tfsrvsimples.mostradesconto(vDesconto, vPercentual, vvalor, vbruto, vtipo, vusrcodigo, vTdeCodigo: string): string;
var
...
procedure Tfsrvsimples.acoesExecute(Action: TBasicAction; var Handled: Boolean);
begin
  inherited;
...
procedure Tfsrvsimples.ActAlterarExecute(Sender: TObject);
var
  vRegistro: Integer;
begin
  inherited;
  ...
  CriaFormulario(Tfitmsrv, Self.itmitmchave.AsString, Self.meschave.Field.AsString);
  itm.Refresh;
  recalculatotais;
...
procedure Tfsrvsimples.ActDescontoNoItemExecute(Sender: TObject);
Var
  vRetorno: String;
...
procedure Tfsrvsimples.ActDescontoNoTotalExecute(Sender: TObject);
Var
...
procedure Tfsrvsimples.ActDtmAlterarExecute(Sender: TObject);
var
  vch: string;
begin
  inherited;
...
procedure Tfsrvsimples.ActDtmExcluirExecute(Sender: TObject);
var
  vlMensagem: String;
begin
  if dtm.IsEmpty then
...
procedure Tfsrvsimples.ActDtmIncluirExecute(Sender: TObject);
begin
...
procedure Tfsrvsimples.ActExcluirExecute(Sender: TObject);
begin
...
procedure Tfsrvsimples.ActImportarItensExecute(Sender: TObject);
var
  varq: string;
  sarq: TStringList;
  i: Integer;
...
procedure Tfsrvsimples.ImportaItensPedido(vite: string; vcod: string; vqtd: string; vvlr: string; vdes: string);
begin
...
procedure Tfsrvsimples.ActIncluirExecute(Sender: TObject);
...
Begin
  ...
  vlRetorno := CriaFormulario(Tfitmsrv, '', Self.meschave.Field.AsString);
...
procedure Tfsrvsimples.ActObsAlterarExecute(Sender: TObject);
begin
  inherited;
...
procedure Tfsrvsimples.ActObsExcluirExecute(Sender: TObject);
begin
  inherited;
...
procedure Tfsrvsimples.ActObsIncluirExecute(Sender: TObject);
var
  vch: string;
begin
  inherited;
...
procedure Tfsrvsimples.bActDtmExcluirClick(Sender: TObject);
begin
  inherited;
...
procedure Tfsrvsimples.bcancelaClick(Sender: TObject);
Var
  vnum: Integer;
Begin
...
procedure Tfsrvsimples.validatransporte;
var
  vch: string;
begin
  inherited;
...
procedure Tfsrvsimples.verificaentidade;
var
  pode: Boolean;
  vtpe: Integer;
begin
...
procedure Tfsrvsimples.bconfirmaClick(Sender: TObject);
Var
  vdif: Double;
...
Begin
  ...
  Begin
    ...
    vdif := 0;
    vdif := SimpleRoundTo(vTotalItens - (Self.mestotal.Field.AsFloat ), -2);
...
procedure Tfsrvsimples.bDFRAlterarClick(Sender: TObject);
begin
...
procedure Tfsrvsimples.bDRFExcluirClick(Sender: TObject);
begin
  if dfr.IsEmpty then
    Exit;
...
procedure Tfsrvsimples.bDRFIncluirClick(Sender: TObject);
begin
...
procedure Tfsrvsimples.btClonarItensClick(Sender: TObject);
var
  i: Integer;
  vlPode: Boolean;
  vlQuantidade: String;
...
procedure Tfsrvsimples.btImportaOracmentoClick(Sender: TObject);
var
  i, x: Integer;
  vlPode: Boolean;
  vlQuantidade: String;
...
procedure Tfsrvsimples.bvalidarClick(Sender: TObject);
begin
  inherited;
...
procedure Tfsrvsimples.DSRegistroDataChange(Sender: TObject; Field: TField);
begin
  inherited;
...
procedure Tfsrvsimples.edOrcChaveKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  If Key = ';' Then
  Begin
...
procedure Tfsrvsimples.edritemEnter(Sender: TObject);
begin
...
procedure Tfsrvsimples.edritemExit(Sender: TObject);
begin
...
procedure Tfsrvsimples.etdcodigoEnter(Sender: TObject);
begin
  inherited;
...
procedure Tfsrvsimples.etdcodigoExit(Sender: TObject);
var
  vlEndereco: String;
begin
  Self.ValidaSaida(Sender);
...
procedure Tfsrvsimples.etdcodigotransExit(Sender: TObject);
begin
  inherited;
  { Desativar ValidaSaida(); }
end;

procedure Tfsrvsimples.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  Self.salvacolunas(listaitens);
  inherited;
end;

procedure Tfsrvsimples.FormShow(Sender: TObject);
var
  psitu: string;
begin
  IdModulo := vPlIdMd;
...
procedure Tfsrvsimples.itmBeforeOpen(DataSet: TDataSet);
begin
  inherited;
  itm.Params[0].AsInteger := registromeschave.AsInteger;
end;

procedure Tfsrvsimples.itmCalcFields(DataSet: TDataSet);
begin
  inherited;
  itmitmtotalbru.AsFloat := Self.itmitmtotal.AsFloat - Self.itmitmdesconto.AsFloat;
end;

procedure Tfsrvsimples.itmrefAfterInsert(DataSet: TDataSet);
begin
  inherited;
  itmrefitmitem.AsInteger := itmref.RecordCount + 1;
end;

procedure Tfsrvsimples.listaitensDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  inherited;
  inherited gridzebrado(Sender, Rect, DataCol, Column, State);
end;

procedure Tfsrvsimples.mesbicmExit(Sender: TObject);
begin
  inherited;
  Self.ValidaSaida(Sender);
...
procedure Tfsrvsimples.mesbicmsExit(Sender: TObject);
begin
  inherited;
  Self.ValidaSaida(Sender);
  mescalculatotal(Sender);
...
Procedure Tfsrvsimples.mescalculatotal(Sender: TObject);
Begin
  Inherited;
  ...
  If (registro.State = dsInsert) Or (registro.State = dsEdit) Then
    Try
      ...
      Self.mestotal.Field.AsFloat := Self.mesvalor.Field.AsFloat - Self.mesdesconto.Field.AsFloat ;
    Except
    End;
End;

procedure Tfsrvsimples.mesdescontoExit(Sender: TObject);
begin
  inherited;
  Self.ValidaSaida(Sender);
  mescalculatotal(Sender);
...
procedure Tfsrvsimples.mesemissaoExit(Sender: TObject);
begin
  ValidaSaida(Sender);
...
procedure Tfsrvsimples.mesfreteExit(Sender: TObject);
begin
  inherited;
  Self.ValidaSaida(Sender);
  mescalculatotal(Sender);
...
procedure Tfsrvsimples.mesicmExit(Sender: TObject);
begin
  inherited;
  Self.ValidaSaida(Sender);
  mescalculatotal(Sender);
...
procedure Tfsrvsimples.mesipiExit(Sender: TObject);
begin
  inherited;
  Self.ValidaSaida(Sender);
  mescalculatotal(Sender);
...
procedure Tfsrvsimples.mesoutrasExit(Sender: TObject);
begin
  inherited;
  Self.ValidaSaida(Sender);
  mescalculatotal(Sender);
...
procedure Tfsrvsimples.mesvalorExit(Sender: TObject);
begin
  inherited;
  Self.ValidaSaida(Sender);
  mescalculatotal(Sender);
...
Procedure Tfsrvsimples.ajustatransporte(Sender: TObject);
Var
  retorno: String;
Begin
  ...
  CriaFormulario(tfdtmsrv, Self.dtmdtmchave.AsString, Self.registromeschave.AsString);

End;

Procedure Tfsrvsimples.ajustaobservacoes;
Begin
...
Procedure Tfsrvsimples.recalculatotais;
Begin
  If (registro.State <> dsEdit) Then
    registro.Edit;
...
procedure Tfsrvsimples.registroAfterInsert(DataSet: TDataSet);
begin
  inherited;
...
procedure Tfsrvsimples.tdfcodigoEnter(Sender: TObject);
begin
  inherited;
...
procedure Tfsrvsimples.tfpcodigoEnter(Sender: TObject);
begin
  inherited;
  Self.txtFiltro := 'tfpcodigo<>99';
end;

procedure Tfsrvsimples.toecodigoEnter(Sender: TObject);
begin
  inherited;
  Self.txtFiltro := '';
  ufs.Close;
  ...
  // 2 2 6 para fora do estado
   ...
   Self.txtFiltro := ' toeorigem = ''5'' and ttecodigo = 7';
...
procedure Tfsrvsimples.toecodigoExit(Sender: TObject);
begin
  inherited;
...
procedure Tfsrvsimples.VerificaToe;
var
  vlTpoCodigo: Integer;
begin
...
function Tfsrvsimples.SelecionaConta: string;
var
...
function Tfsrvsimples.RegistraLote: String;
type
...
procedure Tfsrvsimples.RecalculaProdutoseServicos;
var
  vlRecno: Integer;
begin
  if not itm.Active then
