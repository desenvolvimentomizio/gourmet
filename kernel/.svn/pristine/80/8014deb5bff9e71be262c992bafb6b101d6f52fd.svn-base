...
type
  ...
  Tfcpr = class(Tfrmbase)
    ...
    punpunbase: TFloatField;
    itmitmqtdcalculada: TFloatField;
    itmtdfcodigo: TStringField;
    puncusto: tuniquery;
    puncustopuncodigo: TIntegerField;
    puncustoprocodigo: TIntegerField;
    puncustounicodigo: TIntegerField;
    puncustopunidentificacao: TStringField;
    puncustounicodigobase: TIntegerField;
    puncustopunmultiplicador: TFloatField;
    puncustopunquantidade: TFloatField;
    puncustopunprecoav: TFloatField;
    puncustopunprecoap: TFloatField;
    puncustopuncusto: TFloatField;
    puncustopunmargem: TFloatField;
    puncustopunpesobruto: TFloatField;
    puncustopunpesoliq: TFloatField;
    puncustodgrcodigo: TIntegerField;
    puncustopunbarra: TStringField;
    puncustotuncodigo: TIntegerField;
    puncustopununitrib: TIntegerField;
    puncustopunqtdtrib: TFloatField;
    puncustopunbase: TFloatField;
  ...
  private
    ...
    function impnfe(chave: String; vmodo: Boolean): Boolean;
    procedure impitensnfe(chave: String; vmodo: Boolean);
  ...
  TImpNfe = function(AOwner: TComponent; conexao: tuniconnection; Acesso: TAcesso; vChave: string; vSoMes: Boolean): string;
...
Procedure Tfcpr.impitensnfe(chave: String; vmodo: Boolean);
var
  impnfe: TImpNfe;
...
function Tfcpr.impnfe(chave: String; vmodo: Boolean): Boolean;
var
  impnfe: TImpNfe;
...
begin
  ...
  if vPack <> 0 then
    try
      ...
      if Assigned(impnfe) then
      begin
        ch := impnfe(Application, Self.zcone, Acesso, chave, vmodo);
        if ch <> '' then
        begin
...
begin
  ...
  Begin
    ...
    itmtdfcodigo.AsString := tdfcodigo.Field.AsString;
    itm.Post;
  ...
  vlNovoArquivoNFe := ExtractFilePath(Application.ExeName) + 'xml-recebidos\' + FormatDateTime('yyyymm', registromesregistro.AsFloat) + '\' +
    chavenfe.Text + '.xml';
  ...
  begin
    ...
    begin
      Confirma := Application.MessageBox(PChar('Ajustar registros de Contas a Pagar desta Fatura ?'), 'Atenção',
  ...
  consulta.Close;
  consulta.SQL.Text := 'SELECT  etvcodigo, etdcodigo,  tvicodigo,  pcgcodigo FROM etv where tvicodigo=' + Inttostr(tviFornecedor) + ' and etdcodigo='
  ...
  begin
    consulta.Append;
    consulta.FieldByName('etdcodigo').AsString := etdcodigo.Field.AsString;
    consulta.FieldByName('tvicodigo').AsInteger := tviFornecedor;
    consulta.Post;
  end;

  consulta.Close;
  consulta.SQL.Text := 'SELECT  count(procodigo) qtd FROM itm where procodigo in (select tdsprocodigo from tds) and meschave=' +
    meschave.Field.AsString;
  consulta.Open;
  ...
  begin
    ...
    consulta.Close;
    consulta.SQL.Text := 'update mes set mesreclassicacao=1 where meschave=' + registromeschave.AsString;
    consulta.ExecSQL;
...
procedure Tfcpr.AjustaCamposAposToe;
begin
  if Self.registrotoecodigo.AsString <> '' then
  begin
    ...
    if Self.vimpnf = false then
      // vimpnf, significa que é uma nota importada e não lançada manual.
...
begin
  ...
  begin
    ...
    begin
      ...
      consulta.Close;
      consulta.SQL.Text := 'select punmultiplicador,puncodigo from pun where pun.procodigo=' + Self.itmprocodigo.AsString + ' and pun.unicodigo=' +
    ...
    puncusto.Close;
    puncusto.ParamByName('procodigo').AsString := Self.itmprocodigo.AsString;
    puncusto.Open;
    vlSql := pun.SQL.Text;

    puncusto.First;
    while not puncusto.Eof do
    begin
      ...
      begin
        ...
        puncusto.Edit;
        puncustopuncusto.AsFloat := itmitmcusto.AsFloat * Self.puncustopunmultiplicador.AsFloat;
        // punpunbase.AsFloat := itmitmcusto.AsFloat * Self.punpunmultiplicador.AsFloat;
        puncusto.Post;
      end;
...
begin
  ...
  begin
    if TamanhoArquivo(vlArquivo) < 2048 then
    begin
      ShowMessage
        ('O XML deste documento ainda não esta disponivel para importação, favor aguarde 15 minutos e faça uma nova consulta de documentos.');
  ...
  if (Self.impnfe(vlChave, True) = false) then
  begin
    chavenfe.SetFocus;
    Exit;
  end
  ...
  begin
...

    {
      ...
      if Principal.FindComponent('lspdbtoecodigo') <> nil then
      begin
      (Principal.FindComponent('lspdbtoecodigo') as TDBText).Visible := false;
      end;

      if Self.FindComponent('spdbtoecodigo') <> nil then
      begin
      (Self.FindComponent('spdbtoecodigo') as tbitbtn).Visible := false;
      end;
    ...
    if (toecodigo.Field.AsString = '') or (toecodigo.Field.AsString = cfgcfgtoeusointe.AsString) then
    begin
      toecodigo.Field.AsString := '';
      Label7.Visible := True;
      toecodigo.Visible := True;
      toecodigo.ReadOnly := false;
      toecodigo.tabstop := True;
      toecodigo.SetFocus;
    end
    else
    begin
      ...
      toecodigo.Visible := false;
      toecodigo.ReadOnly := True;
      toecodigo.tabstop := false;

      if Self.FindComponent('lspdbtoecodigo') <> nil then
      begin
        (Self.FindComponent('lspdbtoecodigo') as TDBText).Visible := false;
      end;
...
begin
  ...
  Begin
    ...
    Self.impitensnfe(chavenfe.Text, false);
...
procedure Tfcpr.itmCalcFields(DataSet: TDataSet);
begin
  inherited;
  itmitmqtdcalculada.AsFloat := itmitmcontendo.AsFloat * itmitmquantidade.AsFloat;
end;
...
Begin
  ...
  If (listaitens.SelectedIndex = 5) then
  begin
    consulta.Close;
  ...
  *)
  ...
  { If (listaitens.SelectedIndex = 4) then
  ...
  *)
  ...
  if (Self.itmitmcontendo.AsFloat <= 0) AND (listaitens.SelectedIndex = 13) then
  begin
    itm.Edit;
    listaitens.SelectedIndex := 12;
  end
  else
  begin
  ...
  if (listaitens.SelectedIndex = 12) then
  begin
    if (listaitens.Columns[6].Field.AsString = '') then
    begin
      listaitens.SelectedIndex := 6;
    end;
  end;
...
Begin
  ...
  If (listaitens.SelectedIndex = 4) And (itmprocodigo.AsInteger = 0) Then
  Begin
    BuscaProduto;
    listaitens.SetFocus;
  End
  else If (listaitens.SelectedIndex = 4) And (itmprocodigo.AsInteger <> 0) Then
  begin
    if itmpronome.AsString = '' then
    begin
      BuscaProduto;
  ...
  *)
  ...
  begin
    ...
    if Self.itmitmcontendo.AsString = '' then
    begin
        ...
        Begin
          ...
          Begin
            ...
            Begin
              ...
              ConsultaPun.ParamByName('punmultiplicador').AsFloat := Self.itmitmcontendo.AsFloat;
        ...
        begin
          ...
          *)
          ...
          begin
            ...
            consulta.Close;
            consulta.SQL.Text :=
              'SELECT itm.itmitem FROM itm WHERE itm.puncodigo = :puncodigo AND itm.itmchave <> :itmchave AND itm.meschave = :meschave;';
            consulta.Params[0].AsInteger := ConsultaPun.FieldByName('puncodigo').AsInteger;
          ...
          itm.Post;
          if Self.itmtoecodigo.AsString <> '' then
          begin
            consulta.Close;
            consulta.SQL.Text := 'select ttecodigo from toe where toecodigo=' + Self.itmtoecodigo.AsString;
            consulta.Open;
...
begin
  ...
  Begin
    ...
    Else
      listaitens.Canvas.Brush.Color := PEG_COR_VALORREQUERIDO;
  End
  Else
  Begin
    If Column.ReadOnly Then
...
begin
  ...
  ActiveControlCancela := listaitens;
  if itm.Active then
    itm.First;
end;
...
begin
  ...
  begin
    ...
    if not itm.IsEmpty then
      if Application.MessageBox(PChar('Você realmente deseja alterar a Operação de todos os produtos?'), 'Confirmação', MB_ICONQUESTION + MB_YESNO) = IDYES
      then
      begin
        itm.DisableControls;
        itm.First;
...
Begin
  ...
  Begin
    ...
    Try
      ...
      if Self.vimpnf = false then
      begin
        Self.mestotal.Field.AsFloat := Self.mesvalor.Field.AsFloat - Self.mesdesconto.Field.AsFloat + Self.mesfrete.Field.AsFloat +
          Self.mesicms.Field.AsFloat + Self.messeguro.Field.AsFloat + Self.mesipi.Field.AsFloat + Self.mesoutras.Field.AsFloat;
      end;
    Except
    End;
  End;
...
procedure Tfcpr.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
var
  vlNome: string;
begin

  vlControl := ActiveControl;
  vlNome := vlControl.Name;
  inherited;
...
begin
  ...
  chavenfe.Color := PEG_COR_VALORREQUERIDO;
...
var
  vlpunmultiplicador: Double;
  vlunicodigobase: Integer;
...
begin
  ...
  vlpunmultiplicador := 1;
  vlunicodigobase := 1;

  if consulta.RecordCount >= 1 then
  begin
    vlpunmultiplicador := Self.consulta.Fields[0].AsFloat;
    vlunicodigobase := Self.consulta.Fields[1].AsInteger;

  end;

  consulta.Close;
  consulta.SQL.Text := 'SELECT itmcontendo FROM itm,mes WHERE itm.meschave=mes.meschave and mes.etdcodigo=' + etdcodigo.Field.AsString + ' and ';
  consulta.SQL.Add('procodigo = ' + Self.itmprocodigo.AsString + ' AND ');
  consulta.SQL.Add('unicodigo = ' + Self.itmunicodigo.AsString);
  ...
  Self.itmitmcontendo.AsFloat := vlpunmultiplicador;
  Self.itmunicodigobase.AsInteger := vlunicodigobase;
...
begin
  ...
  begin
    listaitens.SelectedField := listaitens.Columns[6].Field;
  end;
end;
...
begin
  ...
  for i := 0 to listaitens.Columns.Count - 1 do
    case i of
      4, 6, 12:
        listaitens.Columns[i].ReadOnly := false;
    end;
end;
