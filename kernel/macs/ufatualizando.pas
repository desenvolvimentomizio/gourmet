...
const
  vpIpServidorRemoto = 'pegasussistemas.com.br';
...
begin
  try
    ...
    ConexaoWeb.Password := 'Pegasus973*';
    ConexaoWeb.Server := vpIpServidorRemoto;

    ConexaoWeb.Port := 2007;
    ConexaoWeb.Username := 'pegasuss_pegasus_publica';
    ConexaoWeb.Database := 'pegasuss_pegasus_publica';
...
begin
  ...
  begin
    try
      ...
      versoes.SQL.Text := 'select verarquivo,verdatahora,verpasta,vermd5 from versoes_pegasus where verativa=3';
      versoes.Open;
      ...
      versoes.close;
      versoes.SQL.Text := 'select verarquivo,verdatahora,verpasta,vermd5 from versoes_pegasus where verativa=1';
      versoes.Open;
  ...
  try
   ...
   { if lowercase(vlverarquivoLocal) <> lowercase(vlnomeapli) then
    begin}
      ...
      versoesarquivo.Connection := ConexaoWeb;
      versoesarquivo.SQL.Text := 'select verconteudoarquivo from versoes_pegasus where verarquivo=' + QuotedStr(versoes.FieldByName('verarquivo').AsString);
      versoesarquivo.Open;
