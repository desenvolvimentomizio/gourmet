...
type
  Tfrabol = class(Tfrabase)
    ...
    obloblvalor: TCurrencyField;
    obltobidentificacao: TStringField;
    obloblnumret: TStringField;
    dsobl: TUniDataSource;
    DBGridObl: TDBGrid;
...
procedure Tfrabol.ActAtualizarExecute(Sender: TObject);
begin

  uqtabela.FilterSQL:='v_rfi.srfcodigo = 0';
...
begin
  ...
  begin
    ...
    begin
      consulta.Close;
      consulta.SQL.Text := 'delete from bol where bolchave=' + uqtabelabolchave.AsString;
      consulta.ExecSQL;

      consulta.Close;
      consulta.SQL.Text := 'delete from gbr where rfichave=' + uqtabelarfichave.AsString;
      consulta.ExecSQL;
...
procedure Tfrabol.ActMostrarTodosExecute(Sender: TObject);
begin
  inherited;
  uqtabela.FilterSQL:='';
    PlQtdRecno.caption := 'Registros: ' + IntToStr(self.uqtabela.RecordCount);
...
procedure Tfrabol.DSTabelaDataChange(Sender: TObject; Field: TField);
begin
  inherited;
  if uqtabela.Active then
 begin
   if uqtabelabolchave.AsString<>'' then
   begin
     obl.Close;
     obl.ParamByName('bolchave').AsString:=uqtabelabolchave.AsString;
     obl.open;
