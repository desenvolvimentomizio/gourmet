...
type
  ...
  private
    ...
    function preparaemials: Boolean;
...
begin
  ...
  Titulo.NumeroDocumento := SoNumeros(BolRfirfinumero.AsString);
  ...
  case vlBcoCodigo of
    ...
    756:
      begin
        ...
        BancoLocalPag := 'PAGÁVEL PREFERENCIALMENTE NO SICOOB';
      end;
    399:
...
begin
  ...
  begin
    try
      ...
      begin
        mfi.Close;
        mfi.SQL.Text := 'update mfi set mfisituacao=9999 where rfichave=' + gbrrfichave.AsString;
        mfi.ExecSQL;
...
function Tfbol.preparaemials: Boolean;
var
  vletdcodigo: Integer;
begin
  result := False;
  ...
  consulta.SQL.Text := '';
  consulta.SQL.Add('SELECT etdcodigo  FROM tit where titcodigo=' + gbrtitcodigo.AsString);
  consulta.Open;

  if not consulta.IsEmpty then
  begin

    vletdcodigo := consulta.FieldByName('etdcodigo').AsInteger;

    consulta.Close;
    consulta.SQL.Text := '';
    consulta.SQL.Text := ' CREATE TEMPORARY TABLE IF NOT EXISTS tt_titulos_email_boleto ( ';
    consulta.SQL.Add(' titcodigo int(11) ');
    consulta.SQL.Add(',etdcodigo int(11) );');
    consulta.ExecSQL;
    consulta.Close;
    consulta.SQL.Text := '';
    consulta.SQL.Add('SELECT tteb.titcodigo , tteb.etdcodigo  FROM tt_titulos_email_boleto tteb');
    consulta.Open;

    consulta.Append;
    consulta.FieldByName('titcodigo').AsInteger := gbrtitcodigo.AsInteger;
    consulta.FieldByName('etdcodigo').AsInteger := vletdcodigo;
    consulta.Post;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        bolboldatadesconto.AsFloat := IncDay(gbrrfivencimento.AsFloat, IfThen(ecaecadiasdesc.AsInteger = 0, 0, ecaecadiasdesc.AsInteger));
...
Begin
  ...
  begin
    Application.MessageBox(PChar('Email do emitente não cadastrado.' + #13 + #13 + 'Entre em contato com suporte da Pégasus Sistemas - (66) 3544-2765.'), 'ATENÇÃO',
