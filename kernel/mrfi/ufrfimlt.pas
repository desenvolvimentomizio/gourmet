...
type
  ...
  public
    ...
    jaajustou: boolean;
    vpMesChave: String;
    function registraparcelas(vchave, vvalor, vparcelas: string; vDadosParcelas: string = ''): string;
...
function Tfrfimlt.registraparcelas(vchave: string; vvalor: string; vparcelas: string; vDadosParcelas: string = ''): string;
var
  ...
  if pack <> 0 then
    try
      ...
      if Assigned(registra) then
        Result := registra(Application, zcone, vvalor, vchave, '0', vparcelas, psituacao.caption, Acesso, vpMesChave.ToInteger(), vDadosParcelas);
