...
uses Winapi.Windows, Vcl.Forms, VirtualTable, Data.DB, MemDS, DBAccess, Uni,
  ...
  System.Diagnostics, comobj, System.ImageList, Vcl.ImgList, 
...
type
  TAcoesSemPermissao = array [0 .. 7] of string;

const
  AcoesSemPermissao: TAcoesSemPermissao = ('ActAtualizar', 'ActConfig', 'ActConfiguracoes', 'ActFiltrar', 'ActPesquisar', 'ActRelatorios', 'ActSair', 'ActVerFiltros');
...
type
  ...
  TFraBase = class(TFrame)
    ...
    procedure PlBotaoFiltroEsp3MouseWheelDown(Sender: TObject; Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
    procedure PlBotaoFiltroEsp3MouseWheelUp(Sender: TObject; Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
  ...
  private
    ...
    function AplicaFiltroEspecial3(vsqlorginal: string; tabe: TUniQuery): string;
    ...
    procedure ActFiltroEspExecute3(Sender: TObject);
  ...
  public
    ...
    procedure MontaFiltroEsp(tabe: TUniQuery; vChavePadrao: String = ''); overload;
    procedure MontaFiltroEsp(tabe: TUniQuery; vChavePadrao: TStringList); overload;
    procedure MontaFiltroEsp3(tabe: TUniQuery; vChavePadrao: String = '');
...
procedure TFraBase.PlBotaoFiltroEsp3MouseWheelDown(Sender: TObject; Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
begin
  TScrollBox(Sender).Perform(WM_VSCROLL, 1, 0);
end;

procedure TFraBase.PlBotaoFiltroEsp3MouseWheelUp(Sender: TObject; Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
begin
...
begin
  ...
  DBGrid2Excel.SaveDBGridAs('C:\pegasus1\excel.xls')
  // ExportRecordsetToMSExcel(self.DBGLista, 'C:\pegasus1\excel.xml', DSTabela);
end;
...
begin
  ...
  begin
    ...
    if (pos('fracco', lowercase(self.Name)) = 0) and (PlBotaoFiltroEsp2.Enabled) and (pos('mesreclassicacao=1', TxtFiltro) = 0) then
      PlBotaoFiltroEsp2.visible := true;
  end;

end;

procedure TFraBase.MontaFiltroEsp3(tabe: TUniQuery; vChavePadrao: String = '');
var
  ActExib: TAction;
  btamcr: TCheckBox;
...
begin
  ...
  begin
    ...
    begin
      if PlBotaoFiltroEsp3.FindChildControl('bAct' + SoLetras(tabe.Fields[1].AsString)) = nil then
      begin
        ...
        vntabfilesp3 := tabe.Name;
        ActExib.OnExecute := ActFiltroEspExecute3;
        ...
        btamcr := TCheckBox.Create(PlBotaoFiltroEsp3);
        btamcr.Parent := PlBotaoFiltroEsp3;
        ...
        PlBotaoFiltroEsp3.visible := true;
      end;
...
procedure TFraBase.MontaFiltroEsp(tabe: TUniQuery; vChavePadrao: TStringList);
var
  ActExib: TAction;
  btamcr: TCheckBox;
  i: Integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        for i := 0 to vChavePadrao.Count - 1 do
        begin

          if tabe.Fields[0].AsString = vChavePadrao[i] then
            ActExib.Checked := true;
        ...
        btamcr.Left := X;
        btamcr.Height := 22;
        btamcr.Width := length(btamcr.caption) + 80;
        if btamcr.Width > l then
          l := btamcr.Width + 70;
    ...
    PlBotaoFiltroEsp.Width := t + 60;
  end;
end;

procedure TFraBase.ActFiltroEspExecute3(Sender: TObject);
var
  vlNomeCB: string;
...
begin
  ...
  begin
    ...
    vlCheckBox := (self.PlBotaoFiltroEsp3.FindComponent('b' + vlNomeCB) as TCheckBox);
...
function TFraBase.AplicaFiltroEspecial3(vsqlorginal: string; tabe: TUniQuery): string;
var
  vlFiltro: string;
  vlNomeCB: string;
  vlCheckBox: TCheckBox;
begin
  ...
  begin
    vlNomeCB := semacento(SoLetras(tabe.Fields[1].AsString));
    vlCheckBox := (self.PlBotaoFiltroEsp3.FindComponent('bAct' + vlNomeCB) as TCheckBox);
...
begin
  ...
  begin
    ...
    if (vlNomeAcao = 'ActIncluirSelecionado') or (vlNomeAcao = 'ActIncluirTodos') or (vlNomeAcao = 'ActRemoverTodos') or (vlNomeAcao = 'ActRemoverSelecionado') or
      (vlNomeAcao = 'ActExcluirExecute') then
    begin
    end
    else
    begin
...
var
  vQuery, vSQLFiltroEsp, vSQLFiltroEsp3, vLinhaSQL, vLinhaSQL3: string;
  i: Integer;
  vRegAtual: String;
...
begin
  try
    ...
    try
      ...
      ztab3 := self.FindComponent(vntabfilesp3);

      vLinhaSQL3 := '';

      if ztab3 <> nil then
        vFieldFiltroEsp3 := (ztab3 as TUniQuery).Name + '.' + (ztab3 as TUniQuery).Fields[0].FieldName
      else
        vFieldFiltroEsp3 := '';

      if vFieldFiltroEsp3 <> '' then
      begin
        for i := 0 to self.uqtabela.SQL.Count - 1 do
        begin
          vLinhaSQL3 := self.uqtabela.SQL[i];

          if pos(vFieldFiltroEsp3 + '=', vLinhaSQL3) = 0 then
      ...
      begin
        ...
        begin
          vSQLFiltroEsp3 := AplicaFiltroEspecial3(vQuery, ztab3 as TUniQuery);
        end
        else
        begin
          vSQLFiltroEsp3 := AplicaFiltroEspecial3(vQuery, ztab3 as TUniQuery);
      ...
      if (vSQLFiltroEsp3 <> '') then
        if (pos('where', lowercase(vQuery)) > 0) then
          vSQLFiltroEsp3 := ' and ' + vSQLFiltroEsp3
        else
          vSQLFiltroEsp3 := ' WHERE ' + vSQLFiltroEsp3;
      ...
      if vSQLFiltroEsp = '' then
      begin
        if length(vQuery + vSQLFiltroEsp + vSQLFiltroEsp3) > 0 then
          self.uqtabela.SQL.text := vQuery + vSQLFiltroEsp + vSQLFiltroEsp3;
      ...
      { except
        ...
        self.uqtabela.SQL.text := SQLOriginal + vSQLFiltroEsp + vSQLFiltroEsp3;
...
Begin
  ...
  begin
    try
      ...
      ShowMessage('Este campo é uma composição, não é pesquisavel!');
      edbusca.text := '';
    end;
  end
  else
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if pos('where', vlsqlbase) > 0 then
            query := 'select distinct ' + vlCampos + ' ' + vlsqlbase + ' ' + 'and ' + filtro + ' order by ' + vlCampos + ' limit 200'
          else
            query := 'select distinct ' + vlCampos + ' ' + vlsqlbase + ' ' + 'where ' + filtro + ' order by ' + vlCampos + ' limit 200';
          ...
          begin
            ...
            if pos(' limit 200', uqtabela.SQL.text) = 0 then
              uqtabela.SQL.text := uqtabela.SQL.text + ' limit 200';
            ...
            begin
              uqtabela.SQL.text := StringReplace(uqtabela.SQL.text, ' limit 200', '', [rfReplaceAll, rfIgnoreCase]);
            end;
...
begin
  ...
  begin
    ...
    if (Parent is TForm) then
      if (Parent as TForm).BorderStyle = bsNone then
      begin
        (Parent as TForm).Height := Screen.Height - 270;
...
begin
  ...
  begin
    ...
    // vlGeracao := GetFileModDate(vlDirBPL);
    // decodedatetime(vlGeracao, ano, mes, dia, myHour, myMin, mySec, myMilli);

    // vlNomeBPL := vlNomeBPL + ' ' + formatfloat('00', mes) + '.' + formatfloat('00', dia) + '-' + formatfloat('00', myHour) + ':' + formatfloat('00', myMin);
    // datetimetostr(GetFileModDate(vlDirBPL));
    self.PlInfo.caption := 'R:' + vlNomeBPL;
