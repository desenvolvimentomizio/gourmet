...
Begin
  if not((Self.ActiveControl = bconfirma) or (Self.ActiveControl = bcancela) or (Self.ActiveControl = bfechar) or (TButton(Self.ActiveControl).caption = 'Chamado')) then
  begin
...
Begin
  ...
  Begin
    ...
    nb := nc;
    vc := Copy(Self.vdadosatuais.Strings[i], Pos('-', Self.vdadosatuais.Strings[i]) + 1, 270);
  ...
  begin
      ...
      begin
        ...
        *)
        ...
        if vIndexNext <= (vComponent As TDBEdit).DataSource.DataSet.FieldCount - 1 then
          if ((vComponent As TDBEdit).DataSource.DataSet.Fields[vIndexNext].FieldKind = fkLookup) and ((vComponent As TDBEdit).DataSource.DataSet.Fields[vIndexNext].Visible) Then
          begin
        ...
        *)
        ...
        vdadosatuais.Add((vComponent As TDBEdit).DataSource.DataSet.name + '=' + (vComponent As TDBEdit).Field.FullName + '-' + (vComponent As TDBEdit).Field.AsString);
      end;
    ...
    *)
    ...
    if (vComponent is TDBComboBox) then
      if ((vComponent as TDBComboBox).Parent is TTabSheet) or ((vComponent as TDBComboBox).Parent is TGroupBox) or ((vComponent as TDBComboBox).Parent is TPanel) then
      Begin
        ...
        vdadosatuais.Add((vComponent As TDBComboBox).DataSource.DataSet.name + '=' + (vComponent As TDBComboBox).Field.FullName + '-' + (vComponent As TDBComboBox).Field.AsString);
    ...
    *)
    ...
    begin
      ...
      begin
        ...
        vdadosatuais.Add((vComponent As TDBMemo).DataSource.DataSet.name + '=' + (vComponent As TDBMemo).Field.FullName + '-' + (vComponent As TDBMemo).Field.AsString);
      end;
...
Begin
  ...
  begin
    ...
    try
      ...
      begin
        if ((Sender As TDBEdit).Field.Required) or ((Sender As TDBEdit).Field.ConstraintErrorMessage <> 'Data inválida: ' + chr(39) + '  /  /    ' + chr(39)) then
        begin
  ...
  Begin
    ...
    Begin
      If ((Sender As TDBEdit).DataSource.DataSet.Fields[X].FieldKind = fkLookup) and ((Sender As TDBEdit).DataSource.DataSet.Fields[X].Visible) and
        ((Sender As TDBEdit).DataSource.DataSet.Fields[X].Required) Then
      Begin
...
Begin
  ...
  Begin
    ...
    Begin
      ...
      If (vtit = 'Transferência de Verbas') or (vtit = 'Registro de Verba') or (vtit = 'Contas a Pagar - Parcelado') Or (vtit = 'Entrada') Or (vtit = 'Entrada') Or
        (vtit = 'Contas a Pagar') Or (vtit = 'Conta de Consumo') or (vtit = 'Entrada por Devolução') or ((Self.name = 'frng') and (Pos('Pagar', Self.caption) > 0)) Then
      Begin
        ndll := 'mfrn';
        // nmodulo := 'modulofrn';
      End
      Else If (vtit = 'Licenciamento') or (vtit = 'Informação Tributária') or (vtit = 'Emissão de Recibo') or (vtit = 'Contrato') or (vtit = 'Chamado') or
        ...
        (vtit = 'Alteração da Entidade') Or (vtit = 'Contas a Receber - Parcelado') Or (vtit = 'Faturamento') or (vtit = 'Configurações Gerais') Or (vtit = 'Registro de Cheque') Or
        (vtit = 'Saída') Or (vtit = 'Contas a Receber') or (vtit = 'Saída por Devolução') or (Self.name = 'fgcb') or (Self.name = 'frng') or (Self.name = 'fdfr') or
        (Self.name = 'ffla') Then
      Begin
        ndll := 'mcli';
      ...
      End
        ...
        (vtit = 'Valor da Hora por Função') or (vtit = 'Lançamentos Mensais') or (vtit = 'Lançamento Mensal') or (vtit = 'Lançamento Fixos') or (vtit = 'Lançamento Fixo') or
        (vtit = 'Participação de Colaborador') then
      begin
        ndll := 'mcfh';
        pesq := txtFiltro;
      end;
...
Begin
  ...
  begin
    ...
    begin
      ...
      begin
        if ((Self.FindComponent(Self.Components[i].name) as TPanel).name <> 'pbotoes') and ((Self.FindComponent(Self.Components[i].name) as TPanel).name <> 'lbcodigojanela') and
...
begin
  ...
  for i := 0 to Self.registro.FieldCount - 1 do
    if Self.registro.Fields[i].Required then
      ...
      begin
        Application.MessageBox(Pchar('Campo "' + Self.registro.Fields[i].DisplayLabel + '" precisa ser preenchido!'), 'Preenchimento Obrigatório', MB_ICONWARNING + MB_OK);
        Result := False;
        break;
      end;
end;
