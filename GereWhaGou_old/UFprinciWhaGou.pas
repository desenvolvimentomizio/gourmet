...
uses
  ...
  Vcl.Imaging.pngimage, Data.DB, DBAccess, Uni, UniProvider, MySQLUniProvider,shellapi,
  MemDS;
...
type
  TFprinciWhaGou = class(TForm)
    ...
    procedure WPPConnect1AfterInjectJS(Sender: TObject);
    procedure WPPConnect1GetEnvrequire_auth(Response: TIsRequire_auth);
    procedure WPPConnect1Disconnected(Sender: TObject);
    procedure WPPConnect1GetMessages(const Response: TGetMessageClass);
    procedure WPPConnect1Get_ErrorResponse(const Response: TErrorResponseClass);
...
begin
  ...
  try
    ...
    WPPConnect1.markIsComposing('6692350049', '1001'); //Digitando 5 Segundos
    ...
    WPPConnect1.setKeepAlive('true'); //Marca como Online
    WPPConnect1.markIsComposing('6699749120', '1001'); //Digitando 5 Segundos
...
procedure TFprinciWhaGou.WPPConnect1AfterInjectJS(Sender: TObject);
begin
  WPPConnect1.GetMyNumber;
end;
...
procedure TFprinciWhaGou.WPPConnect1Disconnected(Sender: TObject);
begin
mmErros.Lines.Add( '412 Desconectou ');
end;
...
procedure TFprinciWhaGou.WPPConnect1GetEnvrequire_auth(
  Response: TIsRequire_auth);
begin
  Label3.Caption := 'Offline - AUTH';
end;
...
procedure TFprinciWhaGou.WPPConnect1GetMessages(
  const Response: TGetMessageClass);
begin
mmErros.Lines.Add( '504 Desconectou ');
end;
...
procedure TFprinciWhaGou.WPPConnect1Get_ErrorResponse(
  const Response: TErrorResponseClass);
begin
  mmErros.Lines.Add( '585  Erro '+Response.Error);
end;
